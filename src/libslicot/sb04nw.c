/* SB04NW.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_sb04nw (char *abschr, char *ul, int *n, int *m, double *c__,
	  ...
	  if (*indx > 1)
	    {
	      ...
	      C2F (dgemv) ("N", n, &i__1, &c_b7, &c__[c_offset], ldc,
			   &ab[*indx * ab_dim1 + 1], &c__1, &c_b9, &d__[1],
			   &c__1, 1L);
	    }
	}
      else
	  ...
	  if (*indx > 1)
	    {
	      ...
	      C2F (dgemv) ("T", &i__1, m, &c_b7, &c__[c_offset], ldc,
			   &ab[*indx + ab_dim1], ldab, &c_b9, &d__[1], &c__1,
			   1L);
	    }
	}
    }
