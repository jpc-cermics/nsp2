/* AB13MD.f -- translated by f2c (version 19961017).
 *   
...
static doubleC c_b1 = { 0., 0. };
static doubleC c_b2 = { 1., 0. };

static int c__1 = 1;
static double c_b11 = 1.;
static double c_b15 = 0.;
static double c_b136 = 2.;

/* Subroutine */ int
nsp_slicot_ab13md (char *fact, int *n, doubleC * z__, int *ldz,
		   int *m, int *nblock, int *itype, double *x,
		   double *bound, double *d__, double *g,
		   int *iwork, double *dwork, int *ldwork,
		   doubleC * zwork, int *lzwork, int *info,
		   long int fact_len)
{
  ...
  int iter;
  double prod, temp;
  int iwrk, isum, nsum, info2;
  double c__, e;
  int i__, j, k, l;
  double scale, delta;
  double rcond;
  int xfact;
  double hnorm, snorm, svlam;
  int gtest, bwork[1];
  double t1;
  int izwrk;
  double t2, t3, znorm;
  double hnorm1, dlambd, ynorm1, hn, ynorm2, znorm2;
  int mr;
  double pp;
  int mt;
  double regpar;
  ...
  int minwrk, minzrk;
  double stsize;
  int iw2, iw3, iw4, iw5, iw6, iw7, iw8, iw9, iz2, iz3, iz4, iz5, iz6,
    iz7, iz8, iz9;
  double rowsum;
  int iw10, iw11, iw12, iw13, iw14, iw15, iw16, iw17, iw18, iw19, iw20,
    iw21, iw22, iw23, iw24, iw25, iw26, iw27, iw28, iw29, iw30, iw31, iw32,
    iw33, iz10, iz11, iz12, iz13, iz14, iz15, iz16, iz17, iz18, iz19, iz20,
    iz21, iz22, iz23, iz24, lwa, lza;
  double eps, phi, rat, tau, tol;
  int pos;
  double tol2, tol3, tol4, tol5;
   ...
   *    Workspace 
   * 
   *    IWORK   INT array, dimension MAX(4*M-2,N) 
   * 
   ...
   *                  value; 
   *            = 1:  the block sizes must be positive ints; 
   ...
   */
  *info = 0;
  xfact = C2F (lsame) (fact, "F", 1L, 1L);
  if (!xfact && !C2F (lsame) (fact, "N", 1L, 1L))
    {
      *info = -1;
    }
  else if (*n < 0)
   ...
   * 
   */
  C2F (dlaset) ("Full", n, &c__1, &c_b11, &c_b11, &d__[1], n, 4L);
  C2F (dlaset) ("Full", n, &c__1, &c_b15, &c_b15, &g[1], n, 4L);
  /* 
   *    Quick return if possible. 
   * 
   */
  znorm = C2F (zlange) ("F", n, n, &z__[z_offset], ldz, &dwork[1], 1L);
  if (znorm == 0.)
    {
      *bound = 0.;
      dwork[1] = 1.;
   ...
   * 
   */
  C2F (zlacpy) ("Full", n, n, &z__[z_offset], ldz, &zwork[1], n, 4L);
  /* 
  ...
  if (nblock[1] == *n)
      ...
      else
	{
	   ...
	   * 
	   */
	  C2F (zgesvd) ("N", "N", n, n, &zwork[1], n, &dwork[1], &zwork[1],
			&c__1, &zwork[1], &c__1, &zwork[*n * *n + 1], lzwork,
			&dwork[*n + 1], &info2, 1L, 1L);
	  if (info2 > 0)
	    {
	      *info = 6;
	      return 0;
	  ...
	  *bound = dwork[1];
	  i__1 = *n * *n + 1;
	  lza = *n * *n + (int) zwork[i__1].r;
	  dwork[1] = (double) (*n * 5);
	  z__1.r = (double) lza, z__1.i = 0.;
  ...
  if (!xfact)
    {
      C2F (dlaset) ("Full", m, m, &c_b15, &c_b15, &dwork[1], m, 4L);
      C2F (dlaset) ("Full", m, &c__1, &c_b11, &c_b11, &dwork[iw2 + 1], m, 4L);
      znorm = C2F (zlange) ("F", n, n, &zwork[1], n, &dwork[1], 1L);
      i__1 = *m;
      for (j = 1; j <= i__1; ++j)
	{
	  i__2 = *m;
	  for (i__ = 1; i__ <= i__2; ++i__)
	      ...
	      if (i__ != j)
		{
		  ...
		  C2F (zlacpy) ("Full", &i__3, &i__4,
				&z__[iwork[i__] + 1 +
				     (iwork[j] + 1) * z_dim1], ldz,
				&zwork[iz2 + 1], n, 4L);
		  i__3 = iwork[i__ + 1] - iwork[i__];
		  i__4 = iwork[j + 1] - iwork[j];
		  i__5 = *lzwork - izwrk;
		  C2F (zgesvd) ("N", "N", &i__3, &i__4, &zwork[iz2 + 1], n,
				&dwork[iw3 + 1], &zwork[1], &c__1, &zwork[1],
				&c__1, &zwork[izwrk + 1], &i__5,
				&dwork[iwrk + 1], &info2, 1L, 1L);
		  if (info2 > 0)
		    {
		      *info = 6;
		      return 0;
      ...
      C2F (dlaset) ("Full", m, &c__1, &c_b15, &c_b15, &dwork[iw4 + 1], m, 4L);
    L50:
      i__1 = *m;
      for (i__ = 1; i__ <= i__1; ++i__)
	{
      ...
      hnorm = C2F (dlange) ("F", m, &c__1, &dwork[iw5 + 1], m, &dwork[1], 1L);
      if (hnorm <= tol2)
	{
	  goto L120;
	}
    ...
    L120:
      scale = 1. / dwork[iw2 + 1];
      C2F (dscal) (m, &scale, &dwork[iw2 + 1], &c__1);
    }
  else
    {
      dwork[iw2 + 1] = 1.;
	  ...
	  if (i__ != j)
	    {
	      i__3 = iwork[i__ + 1] - iwork[i__];
	      i__4 = iwork[j + 1] - iwork[j];
	      C2F (zlascl) ("G", m, m, &dwork[iw2 + j], &dwork[iw2 + i__],
			    &i__3, &i__4,
			    &zwork[iwork[i__] + 1 + iwork[j] * *n], n, &info2,
			    1L);
	    }
	  /* L140: */
	}
   ...
   * 
   */
  C2F (zlacpy) ("Full", n, n, &zwork[1], n, &zwork[iz2 + 1], n, 4L);
  i__1 = *lzwork - izwrk;
  C2F (zgesvd) ("N", "N", n, n, &zwork[iz2 + 1], n, &dwork[iw3 + 1],
		&zwork[1], &c__1, &zwork[1], &c__1, &zwork[izwrk + 1], &i__1,
		&dwork[iwrk + 1], &info2, 1L, 1L);
  if (info2 > 0)
    {
      *info = 6;
  ...
  i__1 = izwrk + 1;
  lza = (int) zwork[i__1].r;
  lzamax = Max (lza, lzamax);
  znorm = dwork[iw3 + 1];
  C2F (zlascl) ("G", m, m, &znorm, &c_b11, n, n, &zwork[1], n, &info2, 1L);
  /* 
   *    Set BB. 
   * 
   */
  i__1 = *n * *n;
  i__2 = *n * *n;
  C2F (dlaset) ("Full", &i__1, &mt, &c_b15, &c_b15, &dwork[iw9 + 1], &i__2,
		4L);
  /* 
   *    Set P. 
   * 
   ...
   * 
   */
  C2F (zgemm) ("C", "N", n, n, n, &c_b2, &zwork[1], n, &zwork[iz3 + 1], n,
	       &c_b1, &zwork[iz4 + 1], n, 1L, 1L);
  /* 
   *    Copy Z'*P*Z into A0. 
   * 
   */
  C2F (zlacpy) ("Full", n, n, &zwork[iz4 + 1], n, &zwork[iz5 + 1], n, 4L);
  /* 
   *    Copy diag(P) into B0d. 
   * 
   */
  C2F (dcopy) (n, &dwork[iw6 + 1], &c__1, &dwork[iw7 + 1], &c__1);
  /* 
   */
  i__1 = *m;
  for (k = 2; k <= i__1; ++k)
      ...
      for (j = 1; j <= i__2; ++j)
	{
	  ...
	  for (i__ = 1; i__ <= i__3; ++i__)
	    {
	      ...
	      i__6 = i__ + (j - 1) * *n;
	      z__1.r =
		z__2.r * zwork[i__6].r - z__2.i * zwork[i__6].i, z__1.i =
		z__2.r * zwork[i__6].i + z__2.i * zwork[i__6].r;
       ...
       */
      i__2 = *n * *n;
      C2F (zcopy) (&i__2, &zwork[iz4 + 1], &c__1,
		   &zwork[iz6 + 1 + (k - 2) * *n * *n], &c__1);
      /* 
       *       Copy diag(P) into the (k-1)-th column of BBd. 
       * 
       */
      C2F (dcopy) (n, &dwork[iw6 + 1], &c__1, &dwork[iw8 + 1 + (k - 2) * *n],
		   &c__1);
      /* 
      ...
      if (itype[k] == 1)
	{
	  ...
	  for (j = 1; j <= i__2; ++j)
	      ...
	      for (i__ = 1; i__ <= i__3; ++i__)
		{
		  ...
		  z__1.r = z__2.r * 0. - z__2.i * 1., z__1.i =
		    z__2.i * 0. + z__2.r * 1.;
		  zwork[i__4].r = z__1.r, zwork[i__4].i = z__1.i;
		  i__4 = iz4 + j + (i__ - 1) * *n;
		  d_cnjg (&z__3, &tempij);
   ...
   * 
   */
  C2F (dlaset) ("Full", &mt, &mt, &c_b15, &c_b11, &dwork[iw11 + 1], &mt, 4L);
  /* 
   */
  iter = -1;
  /* 
 ...
 L390:
  ...
  i__1 = *n * *n;
  C2F (zcopy) (&i__1, &zwork[iz5 + 1], &c__1, &zwork[iz7 + 1], &c__1);
  i__1 = *n * *n;
  i__2 = *n * *n;
  C2F (zgemv) ("N", &i__1, &mt, &c_b2, &zwork[iz6 + 1], &i__2,
	       &zwork[iz10 + 1], &c__1, &c_b2, &zwork[iz7 + 1], &c__1, 1L);
  /* 
   *       Compute diag( Binv ). 
   * 
   */
  C2F (dcopy) (n, &dwork[iw7 + 1], &c__1, &dwork[iw12 + 1], &c__1);
  i__1 = *m - 1;
  C2F (dgemv) ("N", n, &i__1, &c_b11, &dwork[iw8 + 1], n, &x[1], &c__1,
	       &c_b11, &dwork[iw12 + 1], &c__1, 1L);
  i__1 = *n;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
   ...
   */
  i__1 = *lzwork - izwrk;
  C2F (zgees) ("N", "N", (L_fp) nsp_slicot_select, n, &zwork[iz11 + 1], n,
	       &sdim, &zwork[iz12 + 1], &zwork[1], n, &zwork[izwrk + 1],
	       &i__1, &dwork[iwrk + 1], bwork, &info2, 1L, 1L);
  if (info2 > 0)
    {
      *info = 6;
      return 0;
  ...
  if (iter == 0)
    ...
    {
      dwork[iw13 + 1] = e;
      C2F (dcopy) (&mt, &x[1], &c__1, &dwork[iw13 + 2], &c__1);
      dlambd = dwork[iw13 + 1] * .98999999999999999 + dwork[iw14 + 1] * .01;
      C2F (dcopy) (&mt, &dwork[iw13 + 2], &c__1, &dwork[iw18 + 1], &c__1);
      C2F (dcopy) (&mt, &dwork[iw14 + 2], &c__1, &dwork[iw19 + 1], &c__1);
      l = 0;
    L460:
      i__1 = mt;
      for (i__ = 1; i__ <= i__1; ++i__)
	{
	  x[i__] =
	    (1. - .01 / nsp_pow_di (c_b136, l)) * dwork[iw18 + i__] +
	    .01 / nsp_pow_di (c_b136, l) * dwork[iw19 + i__];
	  /* L470: */
	}
      /* 
       *          Compute At(:) = A0 + AA*x. 
      ...
      for (i__ = 1; i__ <= i__1; ++i__)
	{
      ...
      i__1 = *n * *n;
      C2F (zcopy) (&i__1, &zwork[iz5 + 1], &c__1, &zwork[iz9 + 1], &c__1);
      i__1 = *n * *n;
      i__2 = *n * *n;
      C2F (zgemv) ("N", &i__1, &mt, &c_b2, &zwork[iz6 + 1], &i__2,
		   &zwork[iz10 + 1], &c__1, &c_b2, &zwork[iz9 + 1], &c__1,
		   1L);
      /* 
       *          Compute diag(Bt). 
       * 
       */
      C2F (dcopy) (n, &dwork[iw7 + 1], &c__1, &dwork[iw21 + 1], &c__1);
      i__1 = *m - 1;
      C2F (dgemv) ("N", n, &i__1, &c_b11, &dwork[iw8 + 1], n, &x[1], &c__1,
		   &c_b11, &dwork[iw21 + 1], &c__1, 1L);
      /* 
       *          Compute W. 
       * 
       */
      ...
      for (j = 1; j <= i__1; ++j)
	{
	  ...
	  for (i__ = 1; i__ <= i__2; ++i__)
	      ...
	      if (i__ == j)
		{
		  ...
		  d__1 =
		    (dwork[iw14 + 1] - dwork[iw13 + 1]) * 1e-4 / 2. -
		    dlambd * dwork[iw21 + i__];
		  z__2.r = d__1, z__2.i = 0.;
		  i__4 = iz9 + i__ + (i__ - 1) * *n;
		  z__1.r = z__2.r + zwork[i__4].r, z__1.i =
		    z__2.i + zwork[i__4].i;
		  zwork[i__3].r = z__1.r, zwork[i__3].i = z__1.i;
		}
	      else
		{
		  i__3 = iz13 + i__ + (j - 1) * *n;
		  i__4 = iz9 + i__ + (j - 1) * *n;
		  zwork[i__3].r = zwork[i__4].r, zwork[i__3].i =
		    zwork[i__4].i;
		}
	      /* L490: */
	    }
       ...
       */
      i__1 = *lzwork - izwrk;
      C2F (zgees) ("N", "N", (L_fp) nsp_slicot_select, n, &zwork[iz13 + 1], n,
		   &sdim, &zwork[iz14 + 1], &zwork[1], n, &zwork[izwrk + 1],
		   &i__1, &dwork[iwrk + 1], bwork, &info2, 1L, 1L);
      if (info2 > 0)
	{
	  *info = 6;
 ...
 L515:
  ...
  if (svlam - dlambd < tol)
      ...
      for (i__ = 1; i__ <= i__1; ++i__)
	{
       ...
       * 
       */
      C2F (dcopy) (n, &dwork[iw7 + 1], &c__1, &d__[1], &c__1);
      i__1 = *m - 1;
      C2F (dgemv) ("N", n, &i__1, &c_b11, &dwork[iw8 + 1], n,
		   &dwork[iw20 + 1], &c__1, &c_b11, &d__[1], &c__1, 1L);
      /* 
       *          Compute diag( G ). 
       * 
       */
      ...
      for (k = 1; k <= i__1; ++k)
	{
      ...
      C2F (dscal) (n, &znorm, &g[1], &c__1);
      dwork[1] = (double) (minwrk - *n * 5 + lwamax);
      i__1 = minzrk - *n * 3 + lzamax;
      z__1.r = (double) i__1, z__1.i = 0.;
      zwork[1].r = z__1.r, zwork[1].i = z__1.i;
      return 0;
    }
  svlam = dlambd;
  ...
  for (k = 1; k <= i__1; ++k)
    {
       ...
       */
      i__2 = *m - 1;
      C2F (dcopy) (&i__2, &x[1], &c__1, &dwork[iw22 + 1], &c__1);
      if (mr > 0)
	{
	  /* 
	   *             Store xG. 
	   * 
	   */
	  C2F (dcopy) (&mr, &x[*m], &c__1, &dwork[iw23 + 1], &c__1);
	}
      /* 
       *          Compute A(:) = A0 + AA*x. 
       * 
      ...
      for (i__ = 1; i__ <= i__2; ++i__)
	{
      ...
      i__2 = *n * *n;
      C2F (zcopy) (&i__2, &zwork[iz5 + 1], &c__1, &zwork[iz7 + 1], &c__1);
      i__2 = *n * *n;
      i__3 = *n * *n;
      C2F (zgemv) ("N", &i__2, &mt, &c_b2, &zwork[iz6 + 1], &i__3,
		   &zwork[iz10 + 1], &c__1, &c_b2, &zwork[iz7 + 1], &c__1,
		   1L);
      /* 
       *          Compute B = B0d + BBd*xD. 
       * 
       */
      C2F (dcopy) (n, &dwork[iw7 + 1], &c__1, &dwork[iw24 + 1], &c__1);
      i__2 = *m - 1;
      C2F (dgemv) ("N", n, &i__2, &c_b11, &dwork[iw8 + 1], n,
		   &dwork[iw22 + 1], &c__1, &c_b11, &dwork[iw24 + 1], &c__1,
		   1L);
      /* 
       *          Compute F. 
       * 
      ...
      for (j = 1; j <= i__2; ++j)
	{
	  ...
	  for (i__ = 1; i__ <= i__3; ++i__)
	      ...
	      if (i__ == j)
		{
		  ...
		  z__1.r = z__2.r - zwork[i__5].r, z__1.i =
		    z__2.i - zwork[i__5].i;
		  zwork[i__4].r = z__1.r, zwork[i__4].i = z__1.i;
		}
	      else
      ...
      C2F (zlacpy) ("Full", n, n, &zwork[iz15 + 1], n, &zwork[iz17 + 1], n,
		    4L);
      /* 
       *          Compute det( F ). 
       * 
       */
      i__2 = *lzwork - izwrk;
      C2F (zgees) ("N", "N", (L_fp) nsp_slicot_select, n, &zwork[iz15 + 1], n,
		   &sdim, &zwork[iz16 + 1], &zwork[1], n, &zwork[izwrk + 1],
		   &i__2, &dwork[iwrk + 1], bwork, &info2, 1L, 1L);
      if (info2 > 0)
	{
	  *info = 6;
	  return 0;
      ...
      for (i__ = 1; i__ <= i__2; ++i__)
	{
       ...
       * 
       */
      C2F (zgetrf) (n, n, &zwork[iz17 + 1], n, &iwork[1], &info2);
      if (info2 > 0)
	{
	  *info = 5;
	  return 0;
      ...
      i__2 = *ldwork - iwrk;
      C2F (zgetri) (n, &zwork[iz17 + 1], n, &iwork[1], &zwork[izwrk + 1],
		    &i__2, &info2);
      i__2 = izwrk + 1;
      lza = (int) zwork[i__2].r;
      lzamax = Max (lza, lzamax);
      ...
      for (j = 1; j <= i__2; ++j)
	{
      ...
      i__2 = *n * *n;
      i__3 = *n * *n;
      C2F (zgemv) ("C", &i__2, &mt, &c_b2, &zwork[iz18 + 1], &i__3,
		   &zwork[iz17 + 1], &c__1, &c_b1, &zwork[iz19 + 1], &c__1,
		   1L);
      i__2 = *m - 1;
      for (i__ = 1; i__ <= i__2; ++i__)
	{
	  dwork[iw26 + i__] =
	    1. / (dwork[iw22 + i__] - .01) - 1. / (100. - dwork[iw22 + i__]);
	  /* L620: */
	}
      if (mr > 0)
	{
      ...
      for (i__ = 1; i__ <= i__2; ++i__)
	{
       ...
       * 
       */
      C2F (dlacpy) ("Full", &mt, &mt, &dwork[iw11 + 1], &mt, &dwork[iw31 + 1],
		    &mt, 4L);
      C2F (dcopy) (&mt, &dwork[iw26 + 1], &c__1, &dwork[iw27 + 1], &c__1);
      i__2 = *ldwork - iwrk;
      C2F (dsysv) ("U", &mt, &c__1, &dwork[iw31 + 1], &mt, &iwork[1],
		   &dwork[iw27 + 1], &mt, &dwork[iwrk + 1], &i__2, &info2,
		   1L);
      if (info2 > 0)
	{
	  *info = 5;
       ...
       */
      i__2 = *m - 1;
      C2F (dcopy) (&i__2, &dwork[iw27 + 1], &c__1, &dwork[iw28 + 1], &c__1);
      /* 
       *          Determine stepsize. 
       * 
       */
      ...
      for (i__ = 1; i__ <= i__2; ++i__)
	{
	  if (dwork[iw28 + i__] > 0.)
	      ...
	      else
		{
		   ...
		   */
		  d__1 = temp, d__2 =
		    (dwork[iw22 + i__] - .01) / dwork[iw28 + i__];
		  temp = Min (d__1, d__2);
		}
	    }
	  /* L650: */
      ...
      for (i__ = 1; i__ <= i__2; ++i__)
	{
	  if (dwork[iw28 + i__] < 0.)
	      ...
	      else
		{
		   ...
		   */
		  d__1 = temp, d__2 =
		    (100. - dwork[iw22 + i__]) / (-dwork[iw28 + i__]);
		  temp = Min (d__1, d__2);
		}
	    }
	  /* L660: */
      ...
      if (mr > 0)
	{
	   ...
	   * 
	   */
	  C2F (dcopy) (&mr, &dwork[iw27 + *m], &c__1, &dwork[iw29 + 1],
		       &c__1);
	  /* 
	   *             Determine stepsize. 
	   * 
	      ...
	      if (dwork[iw29 + i__] > 0.)
		{
		  ...
		  if (l == 1)
		  ...
		  else
		    {
		       ...
		       */
		      d__1 = temp, d__2 =
			(dwork[iw23 + i__] + tau) / dwork[iw29 + i__];
		      temp = Min (d__1, d__2);
		    }
		}
	      /* L670: */
	      ...
	      if (dwork[iw29 + i__] < 0.)
		{
		  ...
		  if (l == 1)
		  ...
		  else
		    {
		       ...
		       */
		      d__1 = temp, d__2 =
			(tau - dwork[iw23 + i__]) / (-dwork[iw29 + i__]);
		      temp = Min (d__1, d__2);
		    }
		}
	      /* L680: */
      ...
      if (stsize >= tol4)
	{
	   ...
	   */
	  i__2 = *m - 1;
	  C2F (dcopy) (&i__2, &dwork[iw20 + 1], &c__1, &dwork[iw22 + 1],
		       &c__1);
	  if (mr > 0)
	    {
	      /* 
	       ...
	       * 
	       */
	      C2F (dcopy) (&mr, &dwork[iw20 + *m], &c__1, &dwork[iw23 + 1],
			   &c__1);
	    }
	  /* 
	  ...
	  i__2 = *m - 1;
	  C2F (dgemv) ("N", n, &i__2, &c_b11, &dwork[iw8 + 1], n,
		       &dwork[iw22 + 1], &c__1, &c_b11, &dwork[iw24 + 1],
		       &c__1, 1L);
	  /* 
	   ...
	   */
	  i__2 = *lzwork - izwrk;
	  C2F (zgees) ("N", "N", (L_fp) nsp_slicot_select, n,
		       &zwork[iz15 + 1], n, &sdim, &zwork[iz16 + 1],
		       &zwork[1], n, &zwork[izwrk + 1], &i__2,
		       &dwork[iwrk + 1], bwork, &info2, 1L, 1L);
	  if (info2 > 0)
	    {
	      *info = 6;
	      return 0;
	  ...
	  if (mr > 0)
	    {
	      ...
	      for (i__ = 1; i__ <= i__2; ++i__)
		{
		  dwork[iw30 + *n + (*m - 1 << 1) + i__] =
		    dwork[iw23 + i__] + tau;
		  dwork[iw30 + *n + (*m - 1 << 1) + mr + i__] =
		    tau - dwork[iw23 + i__];
		  /* L770: */
		}
	    }
	  ...
	  if (emin <= 0. || -log (prod) >= phi)
	    ...
	    {
	      C2F (dcopy) (&mt, &dwork[iw20 + 1], &c__1, &x[1], &c__1);
	    }
	}
      if (stsize < tol4)
	{
 ...
 L810:
   ...
   */
  i__1 = *m - 1;
  C2F (dcopy) (&i__1, &x[1], &c__1, &dwork[iw22 + 1], &c__1);
  if (mr > 0)
    {
      /* 
       *             Store xG. 
  ...
  C2F (dcopy) (n, &dwork[iw7 + 1], &c__1, &dwork[iw24 + 1], &c__1);
  i__1 = *m - 1;
  C2F (dgemv) ("N", n, &i__1, &c_b11, &dwork[iw8 + 1], n, &dwork[iw22 + 1],
	       &c__1, &c_b11, &dwork[iw24 + 1], &c__1, 1L);
  /* 
   *          Compute F. 
   * 
   */
  ...
  C2F (zlacpy) ("Full", n, n, &zwork[iz15 + 1], n, &zwork[iz17 + 1], n, 4L);
  /* 
   *          Compute det( F ). 
   * 
   */
  ...
  i__1 = *ldwork - iwrk;
  C2F (zgetri) (n, &zwork[iz17 + 1], n, &iwork[1], &zwork[izwrk + 1], &i__1,
		&info2);
  i__1 = izwrk + 1;
  lza = (int) zwork[i__1].r;
  lzamax = Max (lza, lzamax);
  ...
  i__1 = *n * *n;
  i__2 = *n * *n;
  C2F (zgemv) ("C", &i__1, &mt, &c_b2, &zwork[iz18 + 1], &i__2,
	       &zwork[iz17 + 1], &c__1, &c_b1, &zwork[iz19 + 1], &c__1, 1L);
  i__1 = *m - 1;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      dwork[iw26 + i__] =
   ...
   */
  i__1 = *n * mt;
  C2F (zgemm) ("N", "N", n, &i__1, n, &c_b2, &zwork[iz17 + 1], n,
	       &zwork[iz18 + 1], n, &c_b1, &zwork[iz20 + 1], n, 1L, 1L);
  C2F (dlaset) ("Full", &mt, &mt, &c_b15, &c_b15, &dwork[iw11 + 1], &mt, 4L);
  i__1 = mt;
  for (k = 1; k <= i__1; ++k)
    {
      i__2 = *n * *n;
      C2F (zcopy) (&i__2, &zwork[iz20 + 1 + (k - 1) * *n * *n], &c__1,
		   &zwork[iz22 + 1], &c__1);
      i__2 = *n;
      for (j = 1; j <= i__2; ++j)
	{
      ...
      i__2 = *n * *n;
      i__3 = *n * *n;
      C2F (zgemv) ("C", &i__2, &k, &c_b2, &zwork[iz20 + 1], &i__3,
		   &zwork[iz23 + 1], &c__1, &c_b1, &zwork[iz24 + 1], &c__1,
		   1L);
      i__2 = k;
      for (j = 1; j <= i__2; ++j)
	{
   ...
   */
 L1110:
  hnorm = C2F (dlange) ("F", &mt, &mt, &dwork[iw11 + 1], &mt, &dwork[1], 1L);
  /* 
   *          Compute rcond( H ). 
   * 
   */
  C2F (dlacpy) ("Full", &mt, &mt, &dwork[iw11 + 1], &mt, &dwork[iw31 + 1],
		&mt, 4L);
  hnorm1 = C2F (dlange) ("1", &mt, &mt, &dwork[iw31 + 1], &mt, &dwork[1], 1L);
  i__1 = *ldwork - iwrk;
  C2F (dsytrf) ("U", &mt, &dwork[iw31 + 1], &mt, &iwork[1], &dwork[iwrk + 1],
		&i__1, &info2, 1L);
  if (info2 > 0)
    {
      *info = 5;
  ...
  lwa = (int) dwork[iwrk + 1];
  lwamax = Max (lwa, lwamax);
  C2F (dsycon) ("U", &mt, &dwork[iw31 + 1], &mt, &iwork[1], &hnorm1, &rcond,
		&dwork[iwrk + 1], &iwork[mt + 1], &info2, 1L);
  if (rcond < tol3)
    {
      i__1 = mt;
      for (i__ = 1; i__ <= i__1; ++i__)
   ...
   */
  C2F (dcopy) (&mt, &dwork[iw26 + 1], &c__1, &dwork[iw27 + 1], &c__1);
  C2F (dsytrs) ("U", &mt, &c__1, &dwork[iw31 + 1], &mt, &iwork[1],
		&dwork[iw27 + 1], &mt, &info2, 1L);
  /* 
   ...
   */
  i__1 = *m - 1;
  C2F (dcopy) (&i__1, &dwork[iw20 + 1], &c__1, &dwork[iw22 + 1], &c__1);
  if (mr > 0)
    {
      /* 
       *             Store xG. 
       * 
       */
      C2F (dcopy) (&mr, &dwork[iw20 + *m], &c__1, &dwork[iw23 + 1], &c__1);
    }
  /* 
   *          Compute A(:) = A0 + AA*x_new. 
   * 
  ...
  if (emin <= 0.)
    ...
    {
      pp = C2F (ddot) (&mt, &dwork[iw26 + 1], &c__1, &dwork[iw27 + 1], &c__1);
      prod = 1.;
      i__1 = *n + (mt << 1);
      for (i__ = 1; i__ <= i__1; ++i__)
	{
 ...
 L1260:
  dwork[iw14 + 1] = dlambd;
  C2F (dcopy) (&mt, &x[1], &c__1, &dwork[iw14 + 2], &c__1);
  /* 
   *       Set yw. 
   * 
   */
  i__1 = mt + 1;
  C2F (dcopy) (&i__1, &dwork[iw14 + 1], &c__1, &dwork[iw15 + 1], &c__1);
  /* 
   *       Compute Fb. 
   * 
   */
  ...
  i__1 = *n * *n;
  i__2 = *n * *n;
  C2F (zgemv) ("C", &i__1, &mt, &c_b2, &zwork[iz20 + 1], &i__2,
	       &zwork[iz21 + 1], &c__1, &c_b1, &zwork[iz24 + 1], &c__1, 1L);
  i__1 = mt;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      i__2 = iz24 + i__;
  ...
  i__1 = *ldwork - iwrk;
  C2F (dsysv) ("U", &mt, &c__1, &dwork[iw31 + 1], &mt, &iwork[1],
	       &dwork[iw32 + 1], &mt, &dwork[iwrk + 1], &i__1, &info2, 1L);
  if (info2 > 0)
    {
      *info = 5;
      return 0;
   ...
   * 
   */
  hn = C2F (dlange) ("F", &mt, &c__1, &dwork[iw32 + 1], &mt, &dwork[1], 1L);
  /* 
   *       Compute y. 
   * 
   */
   ...
   */
  i__1 = *m - 1;
  C2F (dcopy) (&i__1, &dwork[iw13 + 2], &c__1, &dwork[iw22 + 1], &c__1);
  if (mr > 0)
    {
      /* 
       *          Store xG. 
       * 
       */
      C2F (dcopy) (&mr, &dwork[iw13 + *m + 1], &c__1, &dwork[iw23 + 1],
		   &c__1);
    }
  /* 
  ...
  if (pos)
    {
       ...
       */
      i__1 = mt + 1;
      C2F (dcopy) (&i__1, &dwork[iw13 + 1], &c__1, &dwork[iw17 + 1], &c__1);
      /* 
      ...
      for (i__ = 1; i__ <= i__1; ++i__)
	{
       ...
       */
      i__1 = mt + 1;
      C2F (dcopy) (&i__1, &dwork[iw17 + 1], &c__1, &dwork[iw15 + 1], &c__1);
      /* 
       *          Compute y(1)*diag(B) - A. 
       * 
       */
 ...
 L1490:
   ...
   */
  i__1 = *m - 1;
  C2F (dcopy) (&i__1, &dwork[iw16 + 2], &c__1, &dwork[iw22 + 1], &c__1);
  if (mr > 0)
    {
      /* 
       *          Store xG. 
       * 
       */
      C2F (dcopy) (&mr, &dwork[iw16 + *m + 1], &c__1, &dwork[iw23 + 1],
		   &c__1);
    }
  /* 
  ...
  if (pos)
    {
       ...
       */
      i__1 = mt + 1;
      C2F (dcopy) (&i__1, &dwork[iw16 + 1], &c__1, &dwork[iw15 + 1], &c__1);
    }
  else
    {
      /* 
       ...
       */
      i__1 = mt + 1;
      C2F (dcopy) (&i__1, &dwork[iw16 + 1], &c__1, &dwork[iw13 + 1], &c__1);
    }
  i__1 = mt + 1;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
  ...
  i__2 = mt + 1;
  ynorm1 =
    C2F (dlange) ("F", &i__1, &c__1, &dwork[iw33 + 1], &i__2, &dwork[1], 1L);
  i__1 = mt + 1;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
 ...
 L1600:
   ...
   * 
   */
  C2F (dcopy) (&mt, &dwork[iw15 + 2], &c__1, &x[1], &c__1);
  goto L390;
  /* 
**** Last line of AB13MD *** 
*/
