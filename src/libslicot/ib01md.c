/* IB01MD.f -- translated by f2c (version 19961017).
 *   
...
static double c_b207 = 0.;
static int c__0 = 0;

/* Subroutine */ int
nsp_slicot_ib01md (char *meth, char *alg, char *batch, char *conct, int *nobr,
		   int *m, int *l, int *nsmp, double *u,
		   int *ldu, double *y, int *ldy, double *r__,
		   int *ldr, int *iwork, double *dwork, int *ldwork,
		   int *iwarn, int *info, long int meth_len, long int alg_len,
		   long int batch_len, long int conct_len)
{
  /* System generated locals */
  int r_dim1, r_offset, u_dim1, u_offset, y_dim1, y_offset, i__1, i__2, i__3;
  double d__1;

  /* Local variables */
  int icol, ierr, itau, init;
  int last;
  double temp;
  int irev;
  int linr, n4sid;
  ...
  int nobr21;
  int qralg;
  int initi, lnobr, mnobr;
  int moesp;
  int lldrw, mldrw;
  int first;
  int jwork;
  int nobrm1, ishft2, id, jd, ii;
  int onebch;
  int nr, ns;
  int connec;
  static int icycle;
  ...
  int lmnobr, mmnobr;
  int interm;
  int ishftu, nslast, ldrwrk, ishfty, minwrk;
  static int maxwrk;
  int ldrwmx;
  static int nsmpsm;
  double dum[1];
  ...
  fqralg = C2F (lsame) (alg, "F", 1L, 1L);
  qralg = C2F (lsame) (alg, "Q", 1L, 1L);
  chalg = C2F (lsame) (alg, "C", 1L, 1L);
  ...
  *info = 0;
      ...
      else
	{
	  ...
	  if (chalg)
	  ...
	  else
	    {
	      minwrk = nr << 1;
	      maxwrk =
		nr + nr * C2F (ilaenv) (&c__1, "DGEQRF", " ", &ns, &nr, &c_n1,
					&c_n1, 6L, 1L);
	      if (first)
		{
		  if (*ldr < ns)
  ...
  if (chalg)
      ...
      if (!first && connec)
	{
	  ...
	  if (*m > 0)
	    {
	      C2F (dlacpy) ("Full", &nobr21, m, &u[u_offset], ldu,
			    &dwork[nobr2], &ldrwrk, 4L);
	    }
	  C2F (dlacpy) ("Full", &nobr21, l, &y[y_offset], ldy,
			&dwork[ldrwrk * *m + nobr2], &ldrwrk, 4L);
	}
      /* 
       */
      if (*m > 0)
	{
	   ...
	   *                                ... + quapro_u(i+NS-1)*quapro_u(j+NS-1)', 
	   ...
	   *                                quapro_u(i+NS)*quapro_u(j+NS)' - u_i*u_j'. 
	   * 
	   */
	  if (!first)
	    {
		...
		{
		  C2F (daxpy) (&i__, &c_b24, &r__[i__ * r_dim1 + 1], &c__1,
	  ...
	  if (!first && connec)
	    {
	      C2F (dsyrk) ("Upper", "Transpose", m, &nobr21, &c_b29,
			   &dwork[1], &ldrwrk, &upd, &r__[r_offset], ldr, 5L,
			   9L);
	    }
	  C2F (dsyrk) ("Upper", "Transpose", m, &ns, &c_b29, &u[u_offset],
		       ldu, &upd, &r__[r_offset], ldr, 5L, 9L);
	  /* 
	   */
	  jd = 1;
	  /* 
	  ...
	  if (first || !connec)
	      ...
	      for (j = 2; j <= i__1; ++j)
		{
		   ...
		   * 
		   */
		  C2F (dgemm) ("Transpose", "NoTranspose", m, m, &ns, &c_b29,
			       &u[u_offset], ldu, &u[j + u_dim1], ldu, &upd,
			       &r__[jd * r_dim1 + 1], ldr, 9L, 11L);
		  /* 
		  ...
		  if (first)
			...
			{
			  C2F (dcopy) (&i__, &r__[i__ * r_dim1 + 1], &c__1,
				       &r__[*m + 1 + (*m + i__) * r_dim1],
				       &c__1);
			  /* L30: */
			}
		      /* 
		  ...
		  else
		    {
		      ...
		      for (i__ = jd - *m; i__ <= i__2; ++i__)
			{
			  C2F (daxpy) (&i__, &c_b29, &r__[i__ * r_dim1 + 1],
				       &c__1,
				       &r__[*m + 1 + (*m + i__) * r_dim1],
				       &c__1);
			  /* L40: */
		  ...
		  for (i__ = 2; i__ <= i__2; ++i__)
		    {
		      C2F (dger) (m, m, &c_b29, &u[ns + i__ - 1 + u_dim1],
				  ldu, &u[ns + j - 1 + u_dim1], ldu,
				  &r__[id + jd * r_dim1], ldr);
		      C2F (dger) (m, m, &c_b24, &u[i__ - 1 + u_dim1], ldu,
				  &u[j - 1 + u_dim1], ldu,
				  &r__[id + jd * r_dim1], ldr);
		      id += *m;
		      /* L50: */
		  ...
		  for (i__ = 1; i__ <= i__2; ++i__)
		    {
		      C2F (daxpy) (&i__, &u[ns + j - 1 + i__ * u_dim1],
				   &u[ns + j - 1 + u_dim1], ldu,
				   &r__[jd + (jd + i__ - 1) * r_dim1], &c__1);
		      d__1 = -u[j - 1 + i__ * u_dim1];
		      C2F (daxpy) (&i__, &d__1, &u[j - 1 + u_dim1], ldu,
				   &r__[jd + (jd + i__ - 1) * r_dim1], &c__1);
		      /* L60: */
		    }
		  /* 
	      ...
	      for (j = 2; j <= i__1; ++j)
		{
		   ...
		   * 
		   */
		  C2F (dgemm) ("Transpose", "NoTranspose", m, m, &nobr21,
			       &c_b29, &dwork[1], &ldrwrk, &dwork[j], &ldrwrk,
			       &upd, &r__[jd * r_dim1 + 1], ldr, 9L, 11L);
		  C2F (dgemm) ("Transpose", "NoTranspose", m, m, &ns, &c_b29,
			       &u[u_offset], ldu, &u[j + u_dim1], ldu, &c_b29,
			       &r__[jd * r_dim1 + 1], ldr, 9L, 11L);
		  /* 
		  ...
		  for (i__ = 2; i__ <= i__2; ++i__)
		    {
		      C2F (dger) (m, m, &c_b29, &u[ns + i__ - 1 + u_dim1],
				  ldu, &u[ns + j - 1 + u_dim1], ldu,
				  &r__[id + jd * r_dim1], ldr);
		      C2F (dger) (m, m, &c_b24, &dwork[i__ - 1], &ldrwrk,
				  &dwork[j - 1], &ldrwrk,
				  &r__[id + jd * r_dim1], ldr);
		      id += *m;
		      /* L100: */
		  ...
		  for (i__ = 1; i__ <= i__2; ++i__)
		    {
		      ...
		      C2F (daxpy) (&i__, &d__1, &dwork[j - 1], &ldrwrk,
				   &r__[jd + (jd + i__ - 1) * r_dim1], &c__1);
		      /* L110: */
		    }
		  /* 
	  ...
	  if (last && moesp)
	      ...
	      for (j = 2; j <= i__1; ++j)
		{
		  C2F (dswap) (&j, &r__[j * r_dim1 + 1], &c__1,
			       &r__[mnobr + 1 + (mnobr + j) * r_dim1], &c__1);
		  i__2 = j - 1;
		  C2F (dswap) (&i__2, &r__[(mnobr + j) * r_dim1 + 1], &c__1,
	   ...
	   *                                ... + quapro_u(i+NS-1)*quapro_y(j+NS-1)', 
	  ...
	  if (!first)
		...
		{
		  C2F (daxpy) (&ii, &c_b24, &r__[i__ * r_dim1 + 1], &c__1,
	  ...
	  if (first || !connec)
		...
		{
		  C2F (dgemm) ("Transpose", "NoTranspose", m, l, &ns, &c_b29,
			       &u[i__ + u_dim1], ldu, &y[y_offset], ldy, &upd,
			       &r__[(i__ - 1) * *m + 1 +
				    (mmnobr + 1) * r_dim1], ldr, 9L, 11L);
		  /* L150: */
		}
	      /* 
	       */
	  ...
	  else
	    {
	      ...
	      for (i__ = 1; i__ <= i__1; ++i__)
		{
		  C2F (dgemm) ("Transpose", "NoTranspose", m, l, &nobr21,
			       &c_b29, &dwork[i__], &ldrwrk,
			       &dwork[ldrwrk * *m + 1], &ldrwrk, &upd,
			       &r__[(i__ - 1) * *m + 1 +
				    (mmnobr + 1) * r_dim1], ldr, 9L, 11L);
		  C2F (dgemm) ("Transpose", "NoTranspose", m, l, &ns, &c_b29,
			       &u[i__ + u_dim1], ldu, &y[y_offset], ldy,
			       &c_b29,
			       &r__[(i__ - 1) * *m + 1 +
				    (mmnobr + 1) * r_dim1], ldr, 9L, 11L);
		  /* L160: */
	  ...
	  if (first || !connec)
	      ...
	      for (j = 2; j <= i__1; ++j)
		{
		  ...
		  if (first)
			...
			{
			  C2F (dcopy) (&ii, &r__[i__ * r_dim1 + 1], &c__1,
		  ...
		  else
		    {
		      ...
		      for (i__ = jd - *l; i__ <= i__2; ++i__)
			{
			  C2F (daxpy) (&ii, &c_b29, &r__[i__ * r_dim1 + 1],
				       &c__1,
				       &r__[*m + 1 + (*l + i__) * r_dim1],
				       &c__1);
			  /* L180: */
		  ...
		  for (i__ = 2; i__ <= i__2; ++i__)
		    {
		      C2F (dger) (m, l, &c_b29, &u[ns + i__ - 1 + u_dim1],
				  ldu, &y[ns + j - 1 + y_dim1], ldy,
				  &r__[id + jd * r_dim1], ldr);
		      C2F (dger) (m, l, &c_b24, &u[i__ - 1 + u_dim1], ldu,
				  &y[j - 1 + y_dim1], ldy,
				  &r__[id + jd * r_dim1], ldr);
		      id += *m;
		      /* L190: */
	      ...
	      for (j = 2; j <= i__1; ++j)
		{
		   ...
		   */
		  C2F (dgemm) ("Transpose", "NoTranspose", m, l, &nobr21,
			       &c_b29, &dwork[1], &ldrwrk,
			       &dwork[ldrwrk * *m + j], &ldrwrk, &upd,
			       &r__[jd * r_dim1 + 1], ldr, 9L, 11L);
		  C2F (dgemm) ("Transpose", "NoTranspose", m, l, &ns, &c_b29,
			       &u[u_offset], ldu, &y[j + y_dim1], ldy, &c_b29,
			       &r__[jd * r_dim1 + 1], ldr, 9L, 11L);
		  /* 
		  ...
		  for (i__ = 2; i__ <= i__2; ++i__)
		    {
		      C2F (dger) (m, l, &c_b29, &u[ns + i__ - 1 + u_dim1],
				  ldu, &y[ns + j - 1 + y_dim1], ldy,
				  &r__[id + jd * r_dim1], ldr);
		      C2F (dger) (m, l, &c_b24, &dwork[i__ - 1], &ldrwrk,
				  &dwork[ldrwrk * *m + j - 1], &ldrwrk,
				  &r__[id + jd * r_dim1], ldr);
		      id += *m;
		      /* L230: */
		    }
	  ...
	  if (last && moesp)
		...
		{
		  C2F (dswap) (&mnobr, &r__[j * r_dim1 + 1], &c__1,
			       &r__[mnobr + 1 + j * r_dim1], &c__1);
		  /* L250: */
		}
	      /* 
	       */
       ...
       *                                   quapro_y(i+NS-1)*quapro_y(i+NS-1)', 
       ...
       *                             quapro_y(i+NS)*quapro_y(j+NS)' - y_i*y_j'. 
       * 
       */
      jd = mmnobr + 1;
      /* 
      ...
      if (!first)
	{
	  ...
	  for (i__ = nr - *l; i__ >= i__1; --i__)
	    {
	      i__2 = i__ - mmnobr;
	      C2F (daxpy) (&i__2, &c_b24, &r__[jd + i__ * r_dim1], &c__1,
      ...
      if (!first && connec)
	{
	  C2F (dsyrk) ("Upper", "Transpose", l, &nobr21, &c_b29,
		       &dwork[ldrwrk * *m + 1], &ldrwrk, &upd,
		       &r__[jd + jd * r_dim1], ldr, 5L, 9L);
	}
      C2F (dsyrk) ("Upper", "Transpose", l, &ns, &c_b29, &y[y_offset], ldy,
		   &upd, &r__[jd + jd * r_dim1], ldr, 5L, 9L);
      /* 
       */
      if (first || !connec)
	{
	  ...
	  for (j = 2; j <= i__1; ++j)
	    {
	       ...
	       * 
	       */
	      C2F (dgemm) ("Transpose", "NoTranspose", l, l, &ns, &c_b29,
			   &y[y_offset], ldy, &y[j + y_dim1], ldy, &upd,
			   &r__[mmnobr + 1 + jd * r_dim1], ldr, 9L, 11L);
	      /* 
	      ...
	      if (first)
		{
		  ...
		  for (i__ = jd - *l; i__ <= i__2; ++i__)
		    {
		      i__3 = i__ - mmnobr;
		      C2F (dcopy) (&i__3, &r__[mmnobr + 1 + i__ * r_dim1],
				   &c__1,
				   &r__[mmnobr + *l + 1 +
					(*l + i__) * r_dim1], &c__1);
		      /* L270: */
	      ...
	      else
		{
		  ...
		  for (i__ = jd - *l; i__ <= i__2; ++i__)
		    {
		      i__3 = i__ - mmnobr;
		      C2F (daxpy) (&i__3, &c_b29,
				   &r__[mmnobr + 1 + i__ * r_dim1], &c__1,
				   &r__[mmnobr + *l + 1 +
					(*l + i__) * r_dim1], &c__1);
		      /* L280: */
		    }
	      ...
	      for (i__ = 2; i__ <= i__2; ++i__)
		{
		  C2F (dger) (l, l, &c_b29, &y[ns + i__ - 1 + y_dim1], ldy,
			      &y[ns + j - 1 + y_dim1], ldy,
			      &r__[id + jd * r_dim1], ldr);
		  C2F (dger) (l, l, &c_b24, &y[i__ - 1 + y_dim1], ldy,
			      &y[j - 1 + y_dim1], ldy, &r__[id + jd * r_dim1],
			      ldr);
		  id += *l;
		  /* L290: */
		}
	      ...
	      for (i__ = 1; i__ <= i__2; ++i__)
		{
		  C2F (daxpy) (&i__, &y[ns + j - 1 + i__ * y_dim1],
			       &y[ns + j - 1 + y_dim1], ldy,
			       &r__[jd + (jd + i__ - 1) * r_dim1], &c__1);
		  d__1 = -y[j - 1 + i__ * y_dim1];
		  C2F (daxpy) (&i__, &d__1, &y[j - 1 + y_dim1], ldy,
			       &r__[jd + (jd + i__ - 1) * r_dim1], &c__1);
		  /* L300: */
		}
	      /* 
	  ...
	  for (j = 2; j <= i__1; ++j)
	    {
	       ...
	       * 
	       */
	      C2F (dgemm) ("Transpose", "NoTranspose", l, l, &nobr21, &c_b29,
			   &dwork[ldrwrk * *m + 1], &ldrwrk,
			   &dwork[ldrwrk * *m + j], &ldrwrk, &upd,
			   &r__[mmnobr + 1 + jd * r_dim1], ldr, 9L, 11L);
	      C2F (dgemm) ("Transpose", "NoTranspose", l, l, &ns, &c_b29,
			   &y[y_offset], ldy, &y[j + y_dim1], ldy, &c_b29,
			   &r__[mmnobr + 1 + jd * r_dim1], ldr, 9L, 11L);
	      /* 
	      ...
	      for (i__ = 2; i__ <= i__2; ++i__)
		{
		  C2F (dger) (l, l, &c_b29, &y[ns + i__ - 1 + y_dim1], ldy,
			      &y[ns + j - 1 + y_dim1], ldy,
			      &r__[id + jd * r_dim1], ldr);
		  C2F (dger) (l, l, &c_b24, &dwork[ldrwrk * *m + i__ - 1],
	      ...
	      for (i__ = 1; i__ <= i__2; ++i__)
		{
		  ...
		  C2F (daxpy) (&i__, &d__1, &dwork[ldrwrk * *m + j - 1],
			       &ldrwrk, &r__[jd + (jd + i__ - 1) * r_dim1],
			       &c__1);
		  /* L350: */
		}
	      /* 
	  ...
	  if (connec)
	    {
	      ...
	      if (*m > 0)
		{
		  C2F (dlacpy) ("Full", &nobr21, m, &u[ns + 1 + u_dim1], ldu,
				&dwork[1], &nobr21, 4L);
		}
	      C2F (dlacpy) ("Full", &nobr21, l, &y[ns + 1 + y_dim1], ldy,
			    &dwork[mmnobr - *m + 1], &nobr21, 4L);
	    }
	  /* 
      ...
      else
	{
	   ...
	   * 
	   */
	  C2F (dpotrf) ("Upper", &nr, &r__[r_offset], ldr, &ierr, 5L);
	  goto L370;
	}
    }
  else if (fqralg)
    {
       ...
       * 
       */
      nsp_slicot_ib01my (meth, batch, conct, nobr, m, l, nsmp, &u[u_offset],
			 ldu, &y[y_offset], ldy, &r__[r_offset], ldr,
			 &iwork[1], &dwork[1], ldwork, iwarn, &ierr, 1L, 1L,
			 1L);
      if (!last)
	{
	  return 0;
 ...
 L370:
  ...
  if (qralg)
      ...
      if (!first && connec)
	{
	  ...
	  if (moesp)
	    {
	      C2F (dlacpy) ("Full", &nobrm1, m, &dwork[inu + *nobr], &ldrwrk,
			    &dwork[1], &ldrwrk, 4L);
	    }
	}
      /* 
      ...
      if (first)
	{
	  ...
	  if (linr)
	      ...
	      if (*m > 0)
		{
		  if (moesp)
		    {
		      ...
		      for (i__ = 1; i__ <= i__1; ++i__)
			{
			  C2F (dlacpy) ("Full", &ns, m,
					&u[*nobr + i__ + u_dim1], ldu,
					&r__[(*m * (i__ - 1) + 1) * r_dim1 +
					     1], ldr, 4L);
			  /* L400: */
			}
		      /* 
		      ...
		      for (i__ = 1; i__ <= i__1; ++i__)
			{
			  C2F (dlacpy) ("Full", &ns, m, &u[i__ + u_dim1], ldu,
					&r__[(mnobr + *m * (i__ - 1) +
					      1) * r_dim1 + 1], ldr, 4L);
			  /* L410: */
			}
	      ...
	      for (i__ = 1; i__ <= i__1; ++i__)
		{
		  C2F (dlacpy) ("Full", &ns, l, &y[i__ + y_dim1], ldy,
				&r__[(mmnobr + *l * (i__ - 1) + 1) * r_dim1 +
				     1], ldr, 4L);
		  /* L430: */
		}
	      /* 
	      ...
	      jwork = itau + nr;
	      i__1 = *ldwork - jwork + 1;
	      C2F (dgeqrf) (&ns, &nr, &r__[r_offset], ldr, &dwork[itau],
	      ...
	      for (i__ = 1; i__ <= i__1; ++i__)
		{
	      ...
	      jwork = itau + nr;
	      i__1 = *ldwork - jwork + 1;
	      C2F (dgeqrf) (&ns, &nr, &dwork[1], &ldrwrk, &dwork[itau],
			    &dwork[jwork], &i__1, &ierr);
	      i__1 = Min (ns, nr);
	      C2F (dlacpy) ("Upper ", &i__1, &nr, &dwork[1], &ldrwrk,
			    &r__[r_offset], ldr, 6L);
	    }
	  /* 
	   */
	  if (ns < nr)
	    {
	      i__1 = nr - ns;
	      i__2 = nr - ns;
	      C2F (dlaset) ("Upper ", &i__1, &i__2, &c_b207, &c_b207,
			    &r__[ns + 1 + (ns + 1) * r_dim1], ldr, 6L);
	    }
	  initi += ns;
	}
      /* 
      ...
      if (ncycle > 1 || !first)
	{
	  ...
	  for (nicycl = inicyc; nicycl <= i__1; ++nicycl)
	      ...
	      if (*m > 0)
		{
		  ...
		  if (moesp)
		    {
		      ...
		      for (i__ = 1; i__ <= i__2; ++i__)
			{
			  C2F (dlacpy) ("Full", &nsf, m,
					&u[initi + *nobr + i__ + u_dim1], ldu,
					&dwork[ishftu], &ldrwrk, 4L);
			  ishftu += mldrw;
			  /* L480: */
			}
	      ...
	      for (i__ = 1; i__ <= i__2; ++i__)
		{
		  C2F (dlacpy) ("Full", &nsf, l, &y[initi + i__ + y_dim1],
				ldy, &dwork[ishfty], &ldrwrk, 4L);
		  ishfty += lldrw;
		  /* L510: */
		}
	      /* 
	      ...
	      if (init > 1)
		{
		  ...
		  if (moesp && *m > 0)
		    {
		      C2F (dlacpy) ("Full", nobr, m, &u[u_offset], ldu,
				    &dwork[*nobr], &ldrwrk, 4L);
		    }
		  /* 
		  ...
		  if (*m > 0)
		      ...
		      if (moesp)
			{
			  ...
			  for (i__ = 1; i__ <= i__2; ++i__)
			    {
			      C2F (dlacpy) ("Full", &nobr21, m,
					    &dwork[ishftu - mldrw + 1],
					    &ldrwrk, &dwork[ishftu], &ldrwrk,
					    4L);
			      ishftu += mldrw;
			      /* L520: */
			    }
			  ...
			  for (i__ = 1; i__ <= i__2; ++i__)
			    {
			      C2F (dlacpy) ("Full", &nobr21, m,
					    &dwork[ishft2 - mldrw + 1],
					    &ldrwrk, &dwork[ishft2], &ldrwrk,
					    4L);
			      ishft2 += mldrw;
			      /* L530: */
			    }
	       ...
	       * 
	       */
	      nsp_slicot_mb04od ("Full", &nr, &c__0, &nsl, &r__[r_offset],
				 ldr, &dwork[1], &ldrwrk, dum, &nr, dum, &nr,
	  ...
	  if (connec)
	    {
	      ...
	      if (*m > 0)
		{
		  C2F (dlacpy) ("Full", &nobr21, m, &u[initi + 1 + u_dim1],
				ldu, &dwork[1], &nobr21, 4L);
		}
	      C2F (dlacpy) ("Full", &nobr21, l, &y[initi + 1 + y_dim1], ldy,
			    &dwork[mmnobr - *m + 1], &nobr21, 4L);
	    }
	  /* 
	   *          Return to get new data. 
