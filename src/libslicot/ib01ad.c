/* IB01AD.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_ib01ad (char *meth, char *alg, char *jobd, char *batch,
		   char *conct, char *ctrl, int *nobr, int *m, int *l,
		   int *nsmp, double *u, int *ldu, double *y, int *ldy,
		   int *n, double *r__, int *ldr, double *sv, double *rcond,
		   double *tol, int *iwork, double *dwork, int *ldwork,
		   int *iwarn, int *info, long int meth_len, long int alg_len,
		   long int jobd_len, long int batch_len, long int conct_len,
		   long int ctrl_len)
{
  ...
  /* Local variables */
  int last, n4sid, chalg;
  int jobdm;
  int nobr21;
  int qralg;
  int lnobr, mnobr;
  int moesp, first, onebch;
  int nr, ns;
  int connec, fqralg;
  int lmnobr, iwarnl;
  int interm, contrl;
  ...
  *info = 0;
   ...
   * 
   */
  nsp_slicot_ib01md (meth, alg, batch, conct, nobr, m, l, nsmp, &u[u_offset],
		     ldu, &y[y_offset], ldy, &r__[r_offset], ldr, &iwork[1],
		     &dwork[1], ldwork, iwarn, info, 1L, 1L, 1L, 1L);
  /* 
   */
  if (*info == 1)
    {
   ...
   * 
   */
  nsp_slicot_ib01nd (meth, jobd, nobr, m, l, &r__[r_offset], ldr, &sv[1],
		     rcond, &iwork[1], &dwork[1], ldwork, &iwarnl, info, 1L,
		     1L);
  *iwarn = Max (*iwarn, iwarnl);
   ...
   * 
   */
  nsp_slicot_ib01od (ctrl, nobr, l, &sv[1], n, tol, &iwarnl, info, 1L);
  *iwarn = Max (*iwarn, iwarnl);
  /* 
