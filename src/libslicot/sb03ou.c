/* SB03OU.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_sb03ou (int *discr, int *ltrans, int *n, int *m,
		   double *a, int *lda, double *b, int *ldb,
		   double *tau, double *u, int *ldu, double *scale,
		   double *dwork, int *ldwork, int *info)
{
  /* System generated locals */
  int a_dim1, a_offset, b_dim1, b_offset, u_dim1, u_offset, i__1, i__2;
  ...
  *info = 0;
  ...
  if (*ltrans)
    {
       ...
       * 
       */
      C2F (dgerqf) (n, m, &b[b_offset], ldb, &tau[1], &dwork[1], ldwork,
		    info);
      /* 
      ...
      if (*m >= *n)
	{
	  C2F (dlacpy) ("Upper", &mn, n, &b[(*m - *n + 1) * b_dim1 + 1], ldb,
			&u[u_offset], ldu, 5L);
	}
      else
	{
	  ...
	  for (i__ = *m; i__ >= 1; --i__)
	    {
	      i__1 = *n - *m + i__;
	      C2F (dcopy) (&i__1, &b[i__ * b_dim1 + 1], &c__1,
			   &u[(*n - *m + i__) * u_dim1 + 1], &c__1);
	      /* L10: */
	    }
	  /* 
	   */
	  i__1 = *n - *m;
	  C2F (dlaset) ("Full", n, &i__1, &c_b8, &c_b8, &u[u_offset], ldu,
			4L);
	}
    }
  else
    {
       ...
       * 
       */
      C2F (dgeqrf) (m, n, &b[b_offset], ldb, &tau[1], &dwork[1], ldwork,
		    info);
      C2F (dlacpy) ("Upper", &mn, n, &b[b_offset], ldb, &u[u_offset], ldu,
		    5L);
      if (*m < *n)
	{
	  i__1 = *n - *m;
	  i__2 = *n - *m;
	  C2F (dlaset) ("Upper", &i__1, &i__2, &c_b8, &c_b8,
			&u[*m + 1 + (*m + 1) * u_dim1], ldu, 5L);
	}
    }
  wrkopt = (int) dwork[1];
  /* 
   ...
   * 
   */
  nsp_slicot_sb03ot (discr, ltrans, n, &a[a_offset], lda, &u[u_offset], ldu,
		     scale, &dwork[1], info);
  if (*info != 0 && *info != 1)
    {
      return 0;
