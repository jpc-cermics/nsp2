/* SB03OR.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_sb03or (int *discr, int *ltrans, int *n, int *m,
		   double *s, int *lds, double *a, int *lda,
		   double *c__, int *ldc, double *scale, int *info)
{
  /* System generated locals */
  int a_dim1, a_offset, c_dim1, c_offset, s_dim1, s_offset, i__1, i__2;
  ...
  int dl;
  double at[4] /* was [2][2] */ , scaloc;
  double vec[4] /* was [2][2] */ ;
  int l2p1;
	  ...
	  if (*discr)
	    {
	      ...
	      i__1 = *n - l2;
	      g11 =
		-C2F (ddot) (&i__1, &s[l1 + l2p1 * s_dim1], lds,
			     &c__[l2p1 + c_dim1], &c__1);
	      if (tbyt)
		{
		  i__1 = *n - l2;
		  g12 =
		    -C2F (ddot) (&i__1, &s[l1 + l2p1 * s_dim1], lds,
				 &c__[l2p1 + (c_dim1 << 1)], &c__1);
		  vec[0] = c__[l1 + c_dim1] + g11 * at[0] + g12 * at[1];
		  vec[2] =
		    c__[l1 + (c_dim1 << 1)] + g11 * at[2] + g12 * at[3];
		}
	      else
		{
	      ...
	      if (dl != 1)
		{
		  i__1 = *n - l2;
		  g21 =
		    -C2F (ddot) (&i__1, &s[l2 + l2p1 * s_dim1], lds,
				 &c__[l2p1 + c_dim1], &c__1);
		  if (tbyt)
		    {
		      i__1 = *n - l2;
			...
			-C2F (ddot) (&i__1, &s[l2 + l2p1 * s_dim1], lds,
		      ...
		      vec[3] =
			c__[l2 + (c_dim1 << 1)] + g21 * at[2] + g22 * at[3];
		    }
		  else
		    {
	      ...
	      i__1 = -isgn;
	      nsp_slicot_sb04px (&c_false, &c_false, &i__1, &dl, m,
				 &s[l1 + l1 * s_dim1], lds, at, &c__2, vec,
				 &c__2, &scaloc, x, &c__2, &xnorm, info);
	    }
	  else
	    {
	      /* 
	      ...
	      i__1 = *n - l2;
	      vec[0] =
		c__[l1 + c_dim1] - C2F (ddot) (&i__1, &s[l1 + l2p1 * s_dim1],
					       lds, &c__[l2p1 + c_dim1],
					       &c__1);
	      if (tbyt)
		{
	      ...
	      if (dl != 1)
		{
	      ...
	      C2F (dlasy2) (&c_false, &c_false, &isgn, &dl, m,
			    &s[l1 + l1 * s_dim1], lds, at, &c__2, vec, &c__2,
			    &scaloc, x, &c__2, &xnorm, info);
	    }
	  ...
	  if (scaloc != 1.)
	    {
	      ...
	      for (j = 1; j <= i__1; ++j)
		{
		  C2F (dscal) (n, &scaloc, &c__[j * c_dim1 + 1], &c__1);
		  /* L10: */
		}
	      /* 
	       */
	  ...
	  if (*discr)
	    {
	      ...
	      i__2 = l1 - 1;
	      g11 =
		-C2F (ddot) (&i__2, &c__[c_offset], &c__1,
			     &s[l1 * s_dim1 + 1], &c__1);
	      if (tbyt)
		{
		  i__2 = l1 - 1;
		  g21 =
		    -C2F (ddot) (&i__2, &c__[(c_dim1 << 1) + 1], &c__1,
				 &s[l1 * s_dim1 + 1], &c__1);
		  vec[0] = c__[l1 + c_dim1] + at[0] * g11 + at[2] * g21;
		  vec[1] =
		    c__[l1 + (c_dim1 << 1)] + at[1] * g11 + at[3] * g21;
		}
	      else
		{
	      ...
	      if (dl != 1)
		{
		  ...
		  if (tbyt)
			...
			-C2F (ddot) (&i__2, &c__[(c_dim1 << 1) + 1], &c__1,
		      ...
		      vec[3] =
			c__[l2 + (c_dim1 << 1)] + at[1] * g12 + at[3] * g22;
		    }
		  else
		    {
	      ...
	      i__2 = -isgn;
	      nsp_slicot_sb04px (&c_false, &c_false, &i__2, m, &dl, at, &c__2,
				 &s[l1 + l1 * s_dim1], lds, vec, &c__2,
				 &scaloc, x, &c__2, &xnorm, info);
	    }
	  else
	    {
	      ...
	      i__2 = l1 - 1;
	      vec[0] =
		c__[l1 + c_dim1] - C2F (ddot) (&i__2, &c__[c_offset], &c__1,
					       &s[l1 * s_dim1 + 1], &c__1);
	      if (tbyt)
		{
		  i__2 = l1 - 1;
		  vec[1] =
		    c__[l1 + (c_dim1 << 1)] - C2F (ddot) (&i__2,
							  &c__[(c_dim1 << 1) +
							       1], &c__1,
							  &s[l1 * s_dim1 + 1],
							  &c__1);
	      ...
	      if (dl != 1)
		{
		  i__2 = l1 - 1;
		  vec[2] =
		    c__[l2 + c_dim1] - C2F (ddot) (&i__2, &c__[c_offset],
						   &c__1, &s[l2 * s_dim1 + 1],
						   &c__1);
		  if (tbyt)
		    {
	      ...
	      C2F (dlasy2) (&c_false, &c_false, &isgn, m, &dl, at, &c__2,
			    &s[l1 + l1 * s_dim1], lds, vec, &c__2, &scaloc, x,
			    &c__2, &xnorm, info);
	    }
	  infom = Max (*info, infom);
	  if (scaloc != 1.)
