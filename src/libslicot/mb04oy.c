/* MB04OY.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_mb04oy (int *m, int *n, double *v, double *tau,
		   double *a, int *lda, double *b, int *ldb, double *dwork)
{
   ...
   * 
   */
  C2F (dcopy) (n, &a[a_offset], lda, &dwork[1], &c__1);
  C2F (dgemv) ("Transpose", m, n, &c_b14, &b[b_offset], ldb, &v[1], &c__1,
	       &c_b14, &dwork[1], &c__1, 9L);
  d__1 = -(*tau);
  C2F (daxpy) (n, &d__1, &dwork[1], &c__1, &a[a_offset], lda);
  d__1 = -(*tau);
  C2F (dger) (m, n, &d__1, &v[1], &c__1, &dwork[1], &c__1, &b[b_offset], ldb);
  goto L210;
 L10:
  /* 
 ...
 L50:
  ...
  for (j = 1; j <= i__1; ++j)
    {
      sum =
	a[j * a_dim1 + 1] + v1 * b[j * b_dim1 + 1] + v2 * b[j * b_dim1 + 2];
      a[j * a_dim1 + 1] -= sum * *tau;
      b[j * b_dim1 + 1] -= sum * t1;
      b[j * b_dim1 + 2] -= sum * t2;
      /* L60: */
  ...
  for (j = 1; j <= i__1; ++j)
    {
      sum =
	a[j * a_dim1 + 1] + v1 * b[j * b_dim1 + 1] + v2 * b[j * b_dim1 + 2] +
	v3 * b[j * b_dim1 + 3] + v4 * b[j * b_dim1 + 4];
      a[j * a_dim1 + 1] -= sum * *tau;
      b[j * b_dim1 + 1] -= sum * t1;
      b[j * b_dim1 + 2] -= sum * t2;
      b[j * b_dim1 + 3] -= sum * t3;
  ...
  for (j = 1; j <= i__1; ++j)
    {
      sum =
	a[j * a_dim1 + 1] + v1 * b[j * b_dim1 + 1] + v2 * b[j * b_dim1 + 2] +
	v3 * b[j * b_dim1 + 3] + v4 * b[j * b_dim1 + 4] + v5 * b[j * b_dim1 +
								 5];
      a[j * a_dim1 + 1] -= sum * *tau;
      b[j * b_dim1 + 1] -= sum * t1;
      b[j * b_dim1 + 2] -= sum * t2;
 ...
 L170:
  ...
  for (j = 1; j <= i__1; ++j)
	...
	v3 * b[j * b_dim1 + 3] + v4 * b[j * b_dim1 + 4] + v5 * b[j * b_dim1 +
								 5] +
	v6 * b[j * b_dim1 + 6] + v7 * b[j * b_dim1 + 7] + v8 * b[j * b_dim1 +
								 8];
      a[j * a_dim1 + 1] -= sum * *tau;
      b[j * b_dim1 + 1] -= sum * t1;
      b[j * b_dim1 + 2] -= sum * t2;
