/* SB03OY.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_sb03oy (int *discr, int *ltrans, int *isgn, double *s,
		   int *lds, double *r__, int *ldr, double *a,
		   int *lda, double *scale, int *info)
{
  /* System generated locals */
  int a_dim1, a_offset, r_dim1, r_offset, s_dim1, s_offset;
  ...
  /* Local variables */
  double absb, absg, abst, temp[2], smin, g[2], gamma[2], alpha, t[2],
    delta[2], y[2];
  double tempi, e1, e2, tempr, p1, p2[2], p3, v1, v2[2], v3;
  double dp[2], s11, s12, s21, s22, dt[2], x11[2], x12[2], x21[2], x22[2];
  double scaloc, bignum, smlnum, p3i, p3r, eta, csp[2], csq[2], eps, sgn,
    cst[2], snp, snq, snt;
  ...
  *info = 0;
   ...
   *Computing MAX 
   */
  d__3 = Abs (s11), d__4 = Abs (s12), d__3 = Max (d__3, d__4), d__4 =
    Abs (s21), d__3 = Max (d__3, d__4), d__4 = Abs (s22);
  d__1 = smlnum, d__2 = eps * Max (d__3, d__4);
  smin = Max (d__1, d__2);
  *scale = 1.;
  /* 
   */
  C2F (dlanv2) (&s11, &s12, &s21, &s22, &tempr, &tempi, &e1, &e2, csp, csq);
  if (tempi == 0.)
    {
      *info = 4;
      return 0;
    }
  absb = C2F (dlapy2) (&e1, &e2);
  if (*discr)
    {
      if (sgn * (absb - 1.) >= 0.)
	{
  ...
  nsp_slicot_sb03ov (temp, &s[s_dim1 + 2], csq, &snq);
  /* 
  ...
  if (*ltrans)
    {
       ...
       */
      temp[0] =
	csq[0] * r__[(r_dim1 << 1) + 2] - snq * r__[(r_dim1 << 1) + 1];
      temp[1] = -csq[1] * r__[(r_dim1 << 1) + 2];
      d__1 = -snq * r__[r_dim1 + 1];
      nsp_slicot_sb03ov (temp, &d__1, csp, &snp);
      /* 
      ...
      p1 = temp[0];
      temp[0] =
	csq[0] * r__[(r_dim1 << 1) + 1] + snq * r__[(r_dim1 << 1) + 2];
  ...
  if (*ltrans)
    {
       ...
       */
      x22[1] = -x22[1];
      nsp_slicot_sb03ov (x22, x21, cst, &snt);
      r__[(r_dim1 << 1) + 2] = x22[0];
      r__[(r_dim1 << 1) + 1] =
	cst[0] * x12[0] - cst[1] * x12[1] + snt * x11[0];
	...
	{
	  r__[r_dim1 + 1] = C2F (dlapy2) (&tempr, &tempi);
  ...
  else
    {
       ...
       * 
       */
      nsp_slicot_sb03ov (x11, x21, cst, &snt);
      r__[r_dim1 + 1] = x11[0];
      r__[(r_dim1 << 1) + 1] =
	cst[0] * x12[0] + cst[1] * x12[1] + snt * x22[0];
	...
	{
	  r__[(r_dim1 << 1) + 2] = C2F (dlapy2) (&tempr, &tempi);
  ...
  if (*ltrans)
    {
      ...
      s[(s_dim1 << 1) + 2] = cst[0] * x22[0] - cst[1] * x22[1] + snt * x21[0];
      /* 
  ...
  else
    {
      ...
      a[a_dim1 + 2] = 0.;
      a[(a_dim1 << 1) + 1] = cst[0] * x12[0] + cst[1] * x12[1] - snt * x11[0];
