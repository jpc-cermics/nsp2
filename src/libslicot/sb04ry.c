/* SB04RY.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_sb04ry (char *rc, char *ul, int *m, double *a, int *lda,
  ...
  *info = 0;
  ...
  if (C2F (lsame) (ul, "U", 1L, 1L))
      ...
      for (j = 1; j <= i__1; ++j)
	{
	  ...
	  i__3 = j + 1;
	  i__2 = Min (i__3, *m);
	  C2F (dscal) (&i__2, lambda, &dwork[j * dwork_dim1 + 1], &c__1);
      ...
      for (j = 1; j <= i__1; ++j)
	{
	  ...
	  i__2 = *m - j1 + 1;
	  C2F (dscal) (&i__2, lambda, &dwork[j1 + j * dwork_dim1], &c__1);
	  dwork[j + j * dwork_dim1] += 1.;
	  /* L80: */
	}
      /* 
