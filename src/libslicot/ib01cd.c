/* IB01CD.f -- translated by f2c (version 19961017).
 *   
...
static double c_b58 = 0.;

/* Subroutine */ int
nsp_slicot_ib01cd (char *jobx0, char *comuse, char *job, int *n, int *m,
		   int *l, int *nsmp, double *a, int *lda,
		   double *b, int *ldb, double *c__, int *ldc,
		   double *d__, int *ldd, double *u, int *ldu,
		   double *y, int *ldy, double *x0, double *v,
		   int *ldv, double *tol, int *iwork, double *dwork,
		   int *ldwork, int *iwarn, int *info, long int jobx0_len,
		   long int comuse_len, long int job_len)
{
  /* System generated locals */
  int a_dim1, a_offset, b_dim1, b_offset, c_dim1, c_offset, d_dim1,
    d_offset, u_dim1, u_offset, v_dim1, v_offset, y_dim1, y_offset, i__1,
    i__2, i__3, i__4, i__5;

  /* Local variables */
  char jobd[1];
  int ncol, ierr, itau, mtmp, i__;
  int usebd;
  double rcond;
  int withb;
  int withd;
  int isize, nsmpl, jwork;
  int ia, ib, ic;
  int withx0;
  int lm, iq, ln, nm, nn;
  int maxdia, compbd;
  int maxdim;
  double rcondu;
  int iwarnl, minsmp, minwrk, maxwrk, minwls, n2m, ldw;
  double dum[1];
  int iwi, iwr, ncp1, ldw2, ldw3;
  ...
  compbd = C2F (lsame) (comuse, "C", 1L, 1L);
  usebd = C2F (lsame) (comuse, "U", 1L, 1L);
  ...
  *info = 0;
  ...
  else if (!(compbd || usebd || C2F (lsame) (comuse, "N", 1L, 1L)))
    {
      *info = -2;
    }
  else if (!withb)
      ...
      if (*info == 0 && *ldwork >= minwrk)
	{
	  ...
	  if (compbd)
	      ...
	      if (*m > 0 && withd)
		{
		  ...
		  i__5 = *nsmp - *m;
		  i__3 =
		    *m * C2F (ilaenv) (&c__1, "DGEQRF", " ", nsmp, m, &c_n1,
				       &c_n1, 6L, 1L), i__4 =
		    ncol + ncol * C2F (ilaenv) (&c__1, "DGEQRF", " ", &i__5,
						&ncol, &c_n1, &c_n1, 6L, 1L);
		  ...
		  i__5 = *nsmp - *m;
		  i__3 =
		    ncp1 * C2F (ilaenv) (&c__1, "DORMQR", "LT", nsmp, &ncp1,
					 m, &c_n1, 6L, 2L), i__4 =
		    ncol + C2F (ilaenv) (&c__1, "DORMQR", "LT", &i__5, &c__1,
					 &ncol, &c_n1, 6L, 2L);
		  i__1 = maxwrk, i__2 = isize + *n + *m + Max (i__3, i__4);
		  maxwrk = Max (i__1, i__2);
		}
	      else
		{
		   ...
		   */
		  i__3 =
		    ncol * C2F (ilaenv) (&c__1, "DGEQRF", " ", &nsmpl, &ncol,
					 &c_n1, &c_n1, 6L, 1L), i__4 =
		    C2F (ilaenv) (&c__1, "DORMQR", "LT", &nsmpl, &c__1, &ncol,
				  &c_n1, 6L, 2L);
	  ...
	  else
	    {
	       ...
	       */
	      i__3 =
		*n * C2F (ilaenv) (&c__1, "DGEQRF", " ", &nsmpl, n, &c_n1,
				   &c_n1, 6L, 1L), i__4 =
		C2F (ilaenv) (&c__1, "DORMQR", "LT", &nsmpl, &c__1, n, &c_n1,
			      6L, 2L);
  ...
  ic = ia + nn;
  ib = ic + ln;
  C2F (dlacpy) ("Full", n, n, &a[a_offset], lda, &dwork[ia], &ldw, 4L);
  C2F (dlacpy) ("Full", l, n, &c__[c_offset], ldc, &dwork[ic], l, 4L);
  /* 
   */
  if (usebd)
    {
      mtmp = *m;
      C2F (dlacpy) ("Full", n, m, &b[b_offset], ldb, &dwork[ib], &ldw, 4L);
    }
  else
    {
      mtmp = 0;
   ...
   */
  i__1 = *ldwork - jwork + 1;
  nsp_slicot_tb01wd (n, &mtmp, l, &dwork[ia], &ldw, &dwork[ib], &ldw,
		     &dwork[ic], l, &v[v_offset], ldv, &dwork[iwr],
		     &dwork[iwi], &dwork[jwork], &i__1, &ierr);
  if (ierr > 0)
    {
      *info = 1;
      return 0;
  ...
  for (i__ = iwr; i__ <= i__1; ++i__)
    {
      if (C2F (dlapy2) (&dwork[i__], &dwork[i__ + *n]) >= 1.)
	{
	  *iwarn = 6;
	}
      /* L10: */
  ...
  if (compbd)
    {
      i__1 = *ldwork - jwork + 1;
      nsp_slicot_ib01qd (jobx0, job, n, m, l, nsmp, &dwork[ia], &ldw,
			 &dwork[ic], l, &u[u_offset], ldu, &y[y_offset], ldy,
			 &x0[1], &dwork[ib], &ldw, &d__[d_offset], ldd, tol,
			 &iwork[1], &dwork[jwork], &i__1, &iwarnl, info, 1L,
			 1L);
      /* 
       */
      if (*info == 0)
	{
	   ...
	   * 
	   */
	  C2F (dgemm) ("NoTranspose", "NoTranspose", n, m, n, &c_b57,
		       &v[v_offset], ldv, &dwork[ib], &ldw, &c_b58,
      ...
      else
	{
	  *(unsigned char *) jobd = 'Z';
       ...
       */
      i__1 = *ldwork - jwork + 1;
      nsp_slicot_ib01rd (jobd, n, &mtmp, l, nsmp, &dwork[ia], &ldw,
			 &dwork[ib], &ldw, &dwork[ic], l, &d__[d_offset], ldd,
			 &u[u_offset], ldu, &y[y_offset], ldy, &x0[1], tol,
			 &iwork[1], &dwork[jwork], &i__1, &iwarnl, info, 1L);
    }
  *iwarn = Max (*iwarn, iwarnl);
  ...
  if (*info == 0)
      ...
      if (withx0)
	{
	   ...
	   * 
	   */
	  C2F (dgemv) ("NoTranspose", n, n, &c_b57, &v[v_offset], ldv, &x0[1],
		       &c__1, &c_b58, &dwork[jwork], &c__1, 11L);
	  C2F (dcopy) (n, &dwork[jwork], &c__1, &x0[1], &c__1);
	}
      /* 
       */
