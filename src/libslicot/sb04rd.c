/* SB04RD.f -- translated by f2c (version 19961017).
 *   
...
/* Subroutine */ int
nsp_slicot_sb04rd (char *abschu, char *ula, char *ulb, int *n, int *m,
		   ...
		   long int abschu_len, long int ula_len, long int ulb_len)
{
  ...
  *info = 0;
  ...
  else if (*ldwork < *n << 1 || *ldwork < (maxmn << 1) * ((maxmn << 1) + 4)
	   && !(labscs && lula && lulb))
    {
      *info = -15;
    }
  ...
  if (labscs && lula && lulb)
    {
       ...
       * 
       */
      nsp_slicot_sb04py ("NoTranspose", "NoTranspose", &c__1, n, m,
			 &a[a_offset], lda, &b[b_offset], ldb, &c__[c_offset],
			 ldc, &scale, &dwork[1], info, 11L, 11L);
      if (scale != 1.)
	{
	  *info = 1;
	}
  ...
  *(unsigned char *) abschr = *(unsigned char *) abschu;
  ...
  if (C2F (lsame) (abschr, "B", 1L, 1L))
      ...
      if ((iend - i__) * fwd >= 0)
	{
	  ...
	  if (istep == 1)
	    {
	      nsp_slicot_sb04rw (abschr, ulb, n, m, &c__[c_offset], ldc, &i__,
				 &b[b_offset], ldb, &a[a_offset], lda,
				 &dwork[jwork], &dwork[1], 1L, 1L);
	      nsp_slicot_sb04ry ("R", ula, n, &a[a_offset], lda,
	    ...
	    {
	      ipincr = i__ + incr;
	      nsp_slicot_sb04rv (abschr, ulb, n, m, &c__[c_offset], ldc,
				 &ipincr, &b[b_offset], ldb, &a[a_offset],
				 lda, &dwork[jwork], &dwork[1], 1L, 1L);
	      nsp_slicot_sb04rx ("R", ula, n, &a[a_offset], lda,
      ...
      if ((iend - i__) * fwd >= 0)
	{
	  ...
	  if (istep == 1)
	    {
	      nsp_slicot_sb04rw (abschr, ula, n, m, &c__[c_offset], ldc, &i__,
				 &a[a_offset], lda, &b[b_offset], ldb,
				 &dwork[jwork], &dwork[1], 1L, 1L);
	      nsp_slicot_sb04ry ("C", ulb, m, &b[b_offset], ldb,
	    ...
	    {
	      ipincr = i__ + incr;
	      nsp_slicot_sb04rv (abschr, ula, n, m, &c__[c_offset], ldc,
				 &ipincr, &a[a_offset], lda, &b[b_offset],
				 ldb, &dwork[jwork], &dwork[1], 1L, 1L);
	      nsp_slicot_sb04rx ("C", ulb, m, &b[b_offset], ldb,
