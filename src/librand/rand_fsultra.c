...
{ 
  ...
  if (    (s1 == floor(s1) && 0.0 <= s1 && s1 <= 4294967295.0)
    ...
    {
      Scierror("bd seed for fsultra, must be integers in [0, 2^32-1]\n");
      return OK;
    }
}
...
{ 
  ...
  if ( floor(try) != try || try < 0.0  ||  try > (double) N)
    {
      Scierror("the first component of the fsultra state, must be an integer in [0, %d] \n\r",N);
      return FAIL;
    }
  swb_index = (int) try;
  ...
  if ( try != 0.0  &&  try != 1.0)
    {
      Scierror("the second component of the fsultra state, must be 0 or 1 \n\r");
      return FAIL;
    }
  swb_flag = (int) try;
  ...
  if ( floor(try) != try  ||  try <= 0 ||  try > 4294967295.0 )
    {
      Scierror("the third component of the fsultra state, must be an integer in [1, 2^32-1] \n\r");
      return FAIL;
    }
