...
struct _ast_wrap {
  ...
  void* (*get_args)(ast_wrap *);
  int (*get_arg)(ast_wrap *ast,int j,ast_wrap *arg); // (ast_wrap*) nsp_list_get_element(astel->get_args(astel),j);
  int (*get_length_args)(ast_wrap *ast); // nsp_list_length(astel->get_length_args(astel));
...
void set_use_html_color_class(int flag);
void set_use_gtk_color_class(int flag);
...
void nsp_ast_print_asread(NspAst *ast, int indent)
{
  ast_wrap loc;
...
static int _nsp_ast_pprint(ast_wrap *ast, int indent, int pos, int posret)
{
...
#else
      if ( ast->get_op(ast) != OBJECT ) pos=Sciprintf2(posret-pos, (use_html_color_class == TRUE) ? "&nbsp;": " ","");
#endif
    }
  ...
  if ( ast->get_op(ast) > 0 ) 
	...
	case 0: /* 0-ary operators */
	  return nsp_ast_pprint_opname(ast->get_op(ast),indent,pos);
	  break;
	case 1:
	  switch ( ast->get_op(ast) ) 
	    ...
	    case  SEMICOLON_OP  :
	      ...
	      newpos = nsp_ast_pprint_opname(ast->get_op(ast),0,newpos);
	      return newpos;
	      break;
	    case  COMMA_RET_OP : 
	    ...
	    case TILDE_OP : 
	    default:
	      newpos =nsp_ast_pprint_opname(ast->get_op(ast),indent,pos);
	...
	case 2:
	  newpos =_nsp_ast_pprint_arg(ast,1,indent,pos,posret);
	  newpos =nsp_ast_pprint_opname(ast->get_op(ast),1,newpos);
	  newpos += Sciprintf(" ");
	...
	case DOTARGS :
	  {
	    ...
	    newpos = pos + Sciprintf2(0,(use_html_color_class == TRUE) ? "&nbsp;": " ",".%s",(char *) ast1.get_str(&ast1));
	    return newpos;
	  }
	case CALLEVAL:
	case LISTEVAL :
	...
	case PLIST :
	  ...
	  break;
	case COMMENT:
	  return pos+ nsp_ast_pprint_comment(indent,(const char *) ast->get_str(ast));
	  break;
	case NAME :
#ifdef WITH_SYMB_TABLE_DEBUG
	  return pos+Sciprintf2(indent,(use_html_color_class == TRUE) ? "&nbsp;": " ","%s<%d>",(char *) ast->get_str(ast),ast->get_arity(ast));
#else 
	  return pos+Sciprintf2(indent,(use_html_color_class == TRUE) ? "&nbsp;": " ","%s",(char *) ast->get_str(ast));
#endif 
	  break;
	case OPNAME :
	  return pos+Sciprintf2(indent,(use_html_color_class == TRUE) ? "&nbsp;": " ","'%s'",(char *) ast->get_str(ast));
	  break;
	case NUMBER:
	  return pos + nsp_ast_pprint_number(indent,(const char *) ast->get_str(ast));
	  break;
	case STRING:
	  return pos + nsp_ast_pprint_string(indent,(const char *) ast->get_str(ast));
	  break;
	case INUMBER32 :
	case INUMBER64 :
	case UNUMBER32 :
...
static int _nsp_ast_printlength(ast_wrap *ast, int indent, int pos, int posret)
{
  ...
  if ( ast->get_op(ast) > 0 ) 
	...
	{
	case 0: /* 0-ary operators */
	  return nsp_ast_printlength_opname(ast->get_op(ast),indent,pos);
	  break;
	case 1:
	  switch ( ast->get_op(ast) ) 
	    {
	    ...
	    case  SEMICOLON_OP  :
	      ...
	      newpos = nsp_ast_printlength_opname(ast->get_op(ast),0,newpos);
	      return newpos;
	      break;
	    case  COMMA_RET_OP : 
	    case  SEMICOLON_RET_OP  :
	    ...
	    case TILDE_OP : 
	    default:
	      newpos =nsp_ast_printlength_opname(ast->get_op(ast),indent,pos);
	...
	case 2:
	  newpos =_nsp_ast_printlength_arg(ast,1,indent,pos,posret);
	  newpos =nsp_ast_printlength_opname(ast->get_op(ast),1,newpos);
	  newpos += 1;
...
static void *nsp_ast_get_args(ast_wrap *ast)
{
  return ((NspAst *) (ast->ast))->args;
}
...
static void nsp_ast_to_ast_wrap(NspAst *ast, ast_wrap *astwrap)
{
  ...
  astwrap->get_args = nsp_ast_get_args;
