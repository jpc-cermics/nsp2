...
cpexpr(register tagptr p)
{
  ...
  e = cpblock(blksize[(int)p->tag], (char *)p);
...
mkfunct(expptr p0)
{
  ...
  switch(np->vprocclass)
      ...
      if(np->vtype == TYUNKNOWN)
	{
	  ...
	  np->vtype = dflttype[(int)letter((int)np->fvarname[0])];
	}
      ap = mkaddr(np);
  ...
  if( (type = np->vtype) == TYUNKNOWN)
    {
      ...
      type = np->vtype = dflttype[(int)letter((int)np->fvarname[0])];
    }
...
mkaddr(register Namep p)
{
  ...
  switch( p->vstg)
	...
	return (Addrp) cpexpr((expptr)xretslot[(int)p->vtype]);
      goto other;
...
vardcl(register Namep v)
{
  ...
  if(v->vstg==STGUNKNOWN) {
    ...
    v->vstg = implstg[(int) letter((int)v->fvarname[0]) ];
    v->vimplstg = 1;
  }
