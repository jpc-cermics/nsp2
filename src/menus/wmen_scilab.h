...
EXPORT BOOL CALLBACK TestDlgProc  (HWND , UINT , WPARAM , LPARAM );
   
extern int ExposeChooseWindow (ChooseMenu *PCh);

/*  "wmen_dialog-n.c.X1" */
EXPORT int CALLBACK  SciDialogDlgProc  (HWND hdlg, UINT wmsg, WPARAM wparam, LPARAM lparam);  
extern BOOL DialogWindow  (void);  

/*  "wmen_getfile-n.c.X1" */

extern int GetFileWindow  (char *filemask, char **szFile, char *dirname, int flag, int *ierr,char *title);  
...
EXPORT BOOL CALLBACK SciMessageDlgProc  (HWND hdlg, UINT wmsg, WPARAM wparam, LPARAM lparam);
  
extern int ExposeMessageWindow  (void);  

/*  "wmen_print-n.c.X1" */

extern int ExposePrintdialogWindow  (int flag, int *colored, int *orientation);  
