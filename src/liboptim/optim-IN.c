...
int int_optim (Stack stack, int rhs, int opt, int lhs)
{ 
...
static OpTab Optim_func[] = {
  {"optim", int_optim},
  {"optim_test", int_n1qn1d_test},
  {(char *) 0, NULL},
};

int Optim_Interf (int i, Stack stack, int rhs, int opt, int lhs)
{
  return (*(Optim_func[i].fonc)) (stack, rhs, opt, lhs);
}

void Optim_Interf_Info (int i, char **fname, function (**f))
{
 *fname = Optim_func[i].name;
 *f = Optim_func[i].fonc;
}
