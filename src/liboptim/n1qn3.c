/* m1qn3
 * Copyright (C) 1998-2010 Jean Charles Gilbert, Claude Lemarechal, INRIA. 
 *
		 ...
		 int *nsim, double *dz, int *ndz, opt_simul_data *optim_data)
{
  ...
  if (*impres >= 1)
    {
      ...
      Sciprintf("\tabsolute precision on x (dxmin)%9.2f\n",*dxmin);
      Sciprintf("\texpected decrease for f (df1):%9.2f\n",*df1);
      Sciprintf("\trelative precision on g (epsg):%9.2f\n",*epsg);
  ...
  if (*impres >= 1)
    {
      Sciprintf("n1qn1:  allocated memory (nrz) :%d\n",*ndz);
      Sciprintf("n1qn1:  used memory: %d\n",ntravu);
      Sciprintf("n1qn1:  number of updates: %d\n",m);
    }
  id = 1;
  igg = id + *n;
  iprec = igg + *n;
 ...
 L904:
  if (*impres >= 1)
    {
      ...
      Sciprintf("\trealized relative precision on g: %9.2f",*epsg);
    }
  ...
  if (*impres >= 1)
    {
      Sciprintf("\tnorm of x = %15.8f",r1);
      Sciprintf("\tf         = %15.8f",*f);
      Sciprintf("\tnorm of g = %15.8f",r2);
    }
  return 0;
}
			 ...
			 opt_simul_data *optim_data)
{
  ...
  if (*impres >= 5)
    {
      Sciprintf("n1qn3a: descent direction -g: precon = %10.3f\n",precos);
    }
  if (*impres == 3)
    {
    }
 ...
 L100:
  ...
  if (*impres >= 3)
    {
      Sciprintf("n1qn3: iter %d, simul %d, f= %15.8f,  h'(0)= %12.5f\n",iter,isim,*f,hp0);
    }
  i__1 = *n;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
  ...
  if (*impres >= 5)
    {
      Sciprintf("n1qn3: stopping criterion on g: %12.5f\n",eps1);
    }
  if (eps1 < *epsg)
    {
      *mode = 1;
