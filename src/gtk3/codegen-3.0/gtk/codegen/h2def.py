...
(?P<available>(GDK_|G_|GLIB_|cairo_pub|CAIRO_BEGIN_DECLS\ cairo_)\w+) # tag
\s+
...
class DefsWriter:
    ...
    def _write_method(self, obj, name, ret, args, available):
        ...
        self.fp.write('  (c-name "' + name + '")\n')
        if available != 'GDK_AVAILABLE_IN_ALL' and available != 'G_AVAILABLE_IN_ALL' and  available != 'GLIB_AVAILABLE_IN_ALL':
