...
int int_obj2gboxed(Stack stack,int rhs,int opt,int lhs)
{
  ...
  if ( nsp_type_gboxednsp == NULL)
     nsp_type_gboxednsp = new_type_gboxednsp(T_BASE);
  nsp_ret = (NspObject *) gboxed_create(NVOID,type,Obj, TRUE, TRUE,
					(NspTypeBase *)	nsp_type_gboxednsp);
...
/* -----------NspBoxedNsp -----------
 * A sub class of NspGBoxed which is used to
 * store a GBoxed objects which contains an NspObject
 */
...
int nsp_type_gboxednsp_id=0;
NspTypeBoxedNsp *nsp_type_gboxednsp=NULL;

/*
 * Type object for NspBoxedNsp
 * all the instance of NspTypeBoxedNsp share the same id.
 * nsp_type_gboxednsp: is an instance of NspTypeBoxedNsp
 *    used for objects of NspBoxedNsp type (i.e built with new_gboxednsp)
 * other instances are used for derived classes
 */
NspTypeBoxedNsp *new_type_gboxednsp(type_mode mode)
{
  NspTypeBoxedNsp *type= NULL;
  NspTypeObject *top;
  if (  nsp_type_gboxednsp != 0 && mode == T_BASE )
    {
      /* initialization performed and T_BASE requested */
      return nsp_type_gboxednsp;
    }
  if (( type =  malloc(sizeof(NspTypeGBoxed))) == NULL) return NULL;
  type->interface = NULL;
  type->surtype = (NspTypeBase *) new_type_gboxed(T_DERIVED);
  if ( type->surtype == NULL) return NULL;
  type->attrs = gboxednsp_attrs;
  type->get_attrs = (attrs_func *) int_get_attribute;
  type->set_attrs = (attrs_func *) int_set_attribute;
  type->methods = gboxednsp_get_methods;
  type->gtk_methods = TRUE;
  type->new = (new_func *) new_gboxednsp;
  ...
  top->s_type =  (s_type_func *) nsp_gboxednsp_type_as_string;
  top->sh_type = (sh_type_func *) nsp_gboxednsp_type_short_string;
  /* top->create = (create_func*) int_gboxednsp_create;*/

  /* specific methods for gboxednsp */

  type->init = (init_func *) init_gboxednsp;

  /*
   * NspBoxedNsp interfaces can be added here
   * type->interface = (NspTypeBase *) new_type_b();
   * type->interface->interface = (NspTypeBase *) new_type_C()
   * ....
   */
  if ( nsp_type_gboxednsp_id == 0 )
    {
      /*
       * the first time we get here we initialize the type id and
       * an instance of NspTypeBoxedNsp called nsp_type_gboxednsp
       */
      type->id =  nsp_type_gboxednsp_id = nsp_new_type_id();
      nsp_type_gboxednsp = type;
      if ( nsp_register_type(nsp_type_gboxednsp) == FALSE) return NULL;
      /* add a ref to nsp_type in the gtype */
      register_nsp_type_in_gtype((NspTypeBase *)nsp_type_gboxednsp,
				 gdk_nspobj_get_type ());
      return ( mode == T_BASE ) ? type : new_type_gboxednsp(mode);
    }
  else
    {
      type->id = nsp_type_gboxednsp_id;
      return type;
    }
}

/*
 * initialize NspBoxedNsp instances
 * locally and by calling initializer on parent class
 */

static int init_gboxednsp(NspBoxedNsp *Obj,NspTypeBoxedNsp *type)
{
...
NspBoxedNsp *new_gboxednsp()
{
  ...
  /* type must exists */
  nsp_type_gboxednsp = new_type_gboxednsp(T_BASE);
  if ( (loc = malloc(sizeof(NspBoxedNsp)))== NULLBOXEDNSP) return loc;
  /* initialize object */
  if ( init_gboxednsp(loc,nsp_type_gboxednsp) == FAIL) return NULLBOXEDNSP;
  return loc;
}

/*----------------------------------------------
 * Object method redefined for NspBoxedNsp
 *-----------------------------------------------*/
/*
 * type as string
 */

static char gboxednsp_type_name[]="BoxedNsp";
static char gboxednsp_short_type_name[]="BoxedNsp";

static char *nsp_gboxednsp_type_as_string(void)
{
  return(gboxednsp_type_name);
}

static char *nsp_gboxednsp_type_short_string(NspObject *v)
{
  return(gboxednsp_short_type_name);
}

/*-----------------------------------------------------
 * a set of functions used when writing interfaces
 * for NspBoxedNsp objects
 * Note that some of these functions could become MACROS
 *-----------------------------------------------------*/

NspBoxedNsp   *nsp_gboxednsp_object(NspObject *O)
{
  /* Follow pointer */
  HOBJ_GET_OBJECT(O,NULL);
  /* Check type */
  if ( check_cast (O,nsp_type_gboxednsp_id)  == TRUE  ) return ((NspBoxedNsp *) O);
  else
    Scierror("Error:	Argument should be a %s\n",type_get_name(nsp_type_gboxednsp));
  return NULL;
}

int IsBoxedNspObj(Stack stack, int i)
{
  return nsp_object_type(NthObj(i),nsp_type_gboxednsp_id);
}

int IsBoxedNsp(NspObject *O)
{
  return nsp_object_type(O,nsp_type_gboxednsp_id);
}

NspBoxedNsp  *GetBoxedNspCopy(Stack stack, int i)
{
  if (  GetBoxedNsp(stack,i) == NULL ) return NULL;
  return MaybeObjCopy(&NthObj(i));
}

NspBoxedNsp  *GetBoxedNsp(Stack stack, int i)
{
  NspBoxedNsp *M;
  if (( M = nsp_gboxednsp_object(NthObj(i))) == NULLBOXEDNSP)
     ArgMessage(stack,i);
  return M;
}
...
NspBoxedNsp *gboxednsp_copy(NspBoxedNsp *self)
{
...
static int gboxednsp_get_nspobj(NspBoxedNsp *self,Stack stack,int rhs,int opt,int lhs)
{
  CheckRhs(0,0);
  CheckLhs(0,1);
  NspObject *value = NSP_GBOXED_GET(self, NspObject );
  if ( value == NULL) return RET_BUG;
  if ((value=nsp_object_copy(value)) == NULLOBJ) return RET_BUG;
  MoveObj(stack,1,value);
  return 1;
}

static NspMethods gboxednsp_methods[] = {
  {"get_obj",(nsp_method *)gboxednsp_get_nspobj },
  { NULL, NULL}
};

static NspMethods *gboxednsp_get_methods(void) { return gboxednsp_methods;};

/*-------------------------------------------
 * Attributes
 *-------------------------------------------*/

static AttrTab gboxednsp_attrs[]={{NULL,NULL,NULL,NULL,NULL}} ;
