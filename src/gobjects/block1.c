...
void gridblock_draw(NspGridBlock *B)
{
  ...
  Nsetscale2d(Xgc,WRect1,NULL,B->obj->scale,"nn");
  Sciprintf("drawing inside the frame \n");
  nsp_gframe_draw(B->obj);
  /* scale back */
  Nsetscale2d(Xgc,WRect,ARect,FRect,"nn");
#else 
...
int gridblock_translate(NspGridBlock *B,const double tr[2])
{
  ...
  Sciprintf("Translate a gridblock \n");
#ifdef WITH_GRID_FRAME
