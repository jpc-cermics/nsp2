		       ...
		       NspTypeBase *type )
{
  ...
  if ((NSP_OBJECT(H)->name = NewString(name))== NULLSTRING) return NULLBLOCK;
  ...
  switch (draw_script)
    ...
    case 0: 
      sprintf(str,"Matplot1(rand(1,1)*32,[%5.2f,%5.2f,%5.2f,%5.2f]);",B->r[0],B->r[1]-B->r[3],
	      B->r[0]+B->r[2],B->r[1]);
      ParseEvalFromStr(str,FALSE,FALSE,FALSE);
      break;
    case 1: 
...
void block_move_control(NspGFrame *F, NspBlock *B,const double pt[2], int cp,double ptc[2])
{
