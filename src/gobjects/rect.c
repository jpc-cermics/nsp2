...
static int rect_xdr_save(NspFile  *F, NspRect *M)
{
  ...
  if ( XdrSaveI(F,M->color) == FAIL) return FAIL;
  if ( XdrSaveI(F,M->thickness) == FAIL) return FAIL;
  if ( XdrSaveI(F,M->background) == FAIL) return FAIL;
  if ( XdrSaveArrayD(F,M->r,4) == FAIL) return FAIL;
  return OK;
}
...
static NspRect  *rect_xdr_load(NspFile  *F)
{
  ...
  char name[NAME_MAXL];
  double rect[4];
  if ( XdrLoadString(F,name,NAME_MAXL) == FAIL) return NULLRECT;
  if ( XdrLoadI(F,&color) == FAIL) return NULLRECT;
