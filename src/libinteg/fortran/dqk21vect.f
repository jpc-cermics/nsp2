...
c***modified by bruno pincon to call the integrand only one time
c            over the 21 integration points
...
c           parameters
c            on entry
c              fvect    subroutine subprogram defining the integrand
c                       function fvect(x, allf). the actual name for 
c                       fvect needs to be declared e x t e r n a l 
...
c
c***references  (none)
c***routines called  d1mach ** replaced by a dlamch call 
