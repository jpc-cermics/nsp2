     ...
     1            istate, iopt, rwork, lrw, iwork, liw, jac, jt, param)
      external f, jac
...
c              and is normally 500. => changed to 10000 (bruno))  to continue, the user may
...
*     (change by bp): new behavior: we return istate = -8 when t=t+h occurs
      call xerrwvb('lsoda:', 6, 1101, 1, 0, 0, 0, 2, tn, h)
      istate = -8
      goto 580  ! ou return
*      nhnil = nhnil + 1
*      if (nhnil .gt. mxhnil) go to 290
C$$$      call xerrwv('lsoda--  caution... t (=r1) and h (=r2) are',
...
C$$$      call xerrwv('lsoda--  previous message precedent given i1 times',
C$$$     1   50, 102, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0)
C$$$      call xerrwv('         will no more be repeated',
...
C$$$ 500  call xerrwv('lsoda--  at t (=r1), mxstep (=i1) steps   ',
C$$$     1   50, 201, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0)
C$$$      call xerrwv('needed before reaching tout',
C$$$     1   50, 201, 1, 1, mxstep, 0, 1, tn, 0.0d+0)
 500  call xerrwvb('lsoda:', 6, 201, 1, 1, mxstep, 0, 1, tn, 0)
      istate = -1
      go to 580
c ewt(i) .le. 0.0 for some i (not at start of problem). ----------------
 510  ewti = rwork(lewt+i-1)
C$$$      call xerrwv('lsoda--  at t (=r1), ewt(i1) is r2 .le. 0.',
C$$$     1   50, 202, 1, 1, i, 0, 2, tn, ewti)
      call xerrwvb('lsoda:', 6, 202, 1, 1, i, 0, 2, tn, ewti) 
      istate = -6
      go to 580
c too much accuracy requested for machine precision. -------------------
C$$$ 520  call xerrwv('lsoda--  a t (=r1),  too much precision required',
C$$$     1   50, 203, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0)
C$$$      call xerrwv(' with respect to epsilon  tolsf (=r2) ',
C$$$     1   50, 203, 1, 0, 0, 0, 2, tn, tolsf)
 520  call xerrwvb('lsoda:', 6, 203, 1, 0, 0, 0, 2, tn, tolsf)
      rwork(14) = tolsf
      istate = -2
      go to 580
...
C$$$ 540  call xerrwv('lsoda--  at t (=r1) and step h (=r2), the'    ,
...
C$$$     1   30, 205, 1, 0, 0, 0, 2, tn, h)
 540  call xerrwvb('lsoda:', 6, 205, 1, 0, 0, 0, 2, tn, h)
      istate = -5
      go to 560
