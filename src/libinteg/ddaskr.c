...
static int
nsp_ode_dnedd (double *x, double *y, double *yprime, int *neq, Dres res,
...
static int
nsp_ode_dnedk (double *x, double *y, double *yprime, int *neq, Dres res,
...
static int
nsp_ode_dlinsd (int *neq, double *y, double *t, double *yprime, double *cj,
		   double *tscale, double *p, double *pnrm, double *wt,
		   int *lsoff, double *stptol, int *iret, Dres res, int *ires,
		   double *wm, int *iwm, double *fnrm, int *icopt, int *id,
		   double *r__, double *ynew, double *ypnew, int *icnflg,
		   int *icnstr, double *rlx, double *rpar, int *ipar);

static int
nsp_ode_dfnrmd (int *neq, double *y, double *t, double *yprime,
		   double *r__, double *cj, double *tscale, double *wt,
		   Dres res, int *ires, double *fnorm, double *wm, int *iwm,
		   double *rpar, int *ipar);

static int
nsp_ode_dnsd (double *x, double *y, double *yprime, int *neq, Dres res,
		 double *pdum, double *wt, double *rpar, int *ipar,
		 double *dumsvr, double *delta, double *e, double *wm,
		 int *iwm, double *cj, double *dums, double *dumr,
		 double *dume, double *epcon, double *s, double *confac,
		 double *tolnew, int *muldel, int *maxit, int *ires,
		 int *idum, int *iernew);


static int
nsp_ode_dnsik (double *x, double *y, double *yprime, int *neq, int *icopt,
		  int *id, Dres res, Psol psol, double *wt, double *rpar,
		  int *ipar, double *savr, double *delta, double *r__,
		  double *yic, double *ypic, double *pwk, double *wm,
		  int *iwm, double *cj, double *tscale, double *sqrtn,
		  double *rsqrtn, double *eplin, double *epcon,
		  double *ratemx, int *maxit, double *stptol, int *icnflg,
		  int *icnstr, int *iernew);

static int
nsp_ode_dfnrmk (int *neq, double *y, double *t, double *yprime,
		   double *savr, double *r__, double *cj, double *tscale,
		   double *wt, double *sqrtn, double *rsqrtn, Dres res,
		   int *ires, Psol psol, int *irin, int *ier, double *fnorm,
		   double *eplin, double *wp, int *iwp, double *pwk,
		   double *rpar, int *ipar);

static int
nsp_ode_dlinsk (int *neq, double *y, double *t, double *yprime,
		   double *savr, double *cj, double *tscale, double *p,
		   double *pnrm, double *wt, double *sqrtn, double *rsqrtn,
		   int *lsoff, double *stptol, int *iret, Dres res, int *ires,
		   Psol psol, double *wm, int *iwm, double *rhok,
		   double *fnrm, int *icopt, int *id, double *wp, int *iwp,
		   double *r__, double *eplin, double *ynew, double *ypnew,
		   double *pwk, int *icnflg, int *icnstr, double *rlx,
		   double *rpar, int *ipar);

static int
nsp_ode_dnsk (double *x, double *y, double *yprime, int *neq, Dres res,
		 Psol psol, double *wt, double *rpar, int *ipar, double *savr,
		 double *delta, double *e, double *wm, int *iwm, double *cj,
		 double *sqrtn, double *rsqrtn, double *eplin, double *epcon,
		 double *s, double *confac, double *tolnew, int *muldel,
		 int *maxit, int *ires, int *iersl, int *iernew);

static int
nsp_ode_dslvk (int *neq, double *y, double *tn, double *yprime,
		  double *savr, double *x, double *ewt, double *wm, int *iwm,
		  Dres res, int *ires, Psol psol, int *iersl, double *cj,
		  double *eplin, double *sqrtn, double *rsqrtn, double *rhok,
		  double *rpar, int *ipar);

static int
nsp_ode_dspigm (int *neq, double *tn, double *y, double *yprime,
		   double *savr, double *r__, double *wght, int *maxl,
		   int *maxlp1, int *kmp, double *eplin, double *cj, Dres res,
		   int *ires, int *nre, Psol psol, int *npsl, double *z__,
		   double *v, double *hes, double *q, int *lgmr, double *wp,
		   int *iwp, double *wk, double *dl, double *rhok, int *iflag,
		   int *irst, int *nrsts, double *rpar, int *ipar);

static int
nsp_ode_datv (int *neq, double *y, double *tn, double *yprime,
		 double *savr, double *v, double *wght, double *yptem,
		 Dres res, int *ires, Psol psol, double *z__, double *vtem,
		 double *wp, int *iwp, double *cj, double *eplin, int *ier,
		 int *nre, int *npsl, double *rpar, int *ipar);


static int
nsp_ode_dmatd (int *neq, double *x, double *y, double *yprime,
		  double *delta, double *cj, double *h__, int *ier,
		  double *ewt, double *e, double *wm, int *iwm, Dres res,
		  int *ires, double *uround,Jacd jacd, double *rpar,
		  int *ipar);


static int
nsp_ode_ddasid (double *x, double *y, double *yprime, int *neq, int *icopt,
...
static int
nsp_ode_ddasik (double *x, double *y, double *yprime, int *neq, int *icopt,
		   ...
		   double *sqrtn, double *rsqrtn, double *epcon,double *ratemx,
		   double *stptol, int *jflg, int *icnflg, int *icnstr, 
		   int *iernls);


static int
nsp_ode_ddasic (double *x, double *y, double *yprime, int *neq, int *icopt,
		   int *id, Dres res, UJac jac, UPsol psol, double *h__,
		   double *tscale, double *wt, int *nic, int *idid,
		   double *rpar, int *ipar, double *phi, double *savr,
		   double *delta, double *e, double *yic, double *ypic,
		   double *pwk, double *wm, int *iwm, double *uround,
		   double *epli, double *sqrtn, double *rsqrtn,
		   double *epconi, double *stptol, int *jflg, int *icnflg,
		   int *icnstr,  int *ntype, Nsik nlsic);

static int
nsp_ode_drchek2 (int *job, Rt rt, int *nrt, int *neq, double *tn,
		    double *tout, double *y, double *yp, double *phi,
		    double *psi, int *kold, double *r0, double *r1,
		    double *rx, int *jroot, int *irt, double *uround,
		    int *info3, double *rwork, int *iwork, double *rpar,
		    int *ipar);


static int
nsp_ode_ddstp (double *x, double *y, double *yprime, int *neq, Dres res,
		  UJac jac, UPsol psol, double *h__, double *wt, double *vt,
		  int *jstart, int *idid, double *rpar, int *ipar,
		  double *phi, double *savr, double *delta, double *e,
		  double *wm, int *iwm, double *alpha, double *beta,
		  double *gamma, double *psi, double *sigma, double *cj,
		  double *cjold, double *hold, double *s, double *hmin,
		  double *uround, double *epli, double *sqrtn, double *rsqrtn,
		  double *epcon, int *iphase, int *jcalc, int *jflg, int *k,
		  int *kold, int *ns, int *nonneg, int *ntype, Dned_dk nls);


static int
nsp_ode_dnsid (double *x, double *y, double *yprime, int *neq, int *icopt,
		  int *id, Dres res, double *wt, double *rpar, int *ipar,
		  double *delta, double *r__, double *yic, double *ypic,
		  double *wm, int *iwm, double *cj, double *tscale,
		  double *epcon, double *ratemx, int *maxit, double *stptol,
		  int *icnflg, int *icnstr, int *iernew);
...
static double c_b770 = 2.;
static double c_b771 = 1.;
...
*  031201  stuck root masking 
*  040615  Removing Hmin requirement 
*  040615  Separating the error message of Singular Jacobian in DDASID 
* 
****CATEGORY NO.  I1A2 
...
*            If INFO(10) = 1 or 3, add NEQ to the base value. 
*            If INFO(11) = 1 or INFO(16) =1, add NEQ to the base value. 
*           >> Due to introduction of Mask in DASKR, NRT has been added 
*            to the  LIW 
* 
* 
* RPAR, IPAR -- These are arrays of double precision and int type, 
...
* 
*   SUBROUTINE DDAWTS1 (NEQ, IWT, RTOL, ATOL, Y, EWT, RPAR, IPAR) 
...
*          IDID = 6 -- The integration was successfully completed 
*                  by finding A ROOT, LIFTED FROM ZERO. 
* 
* 
*                   *** TASK INTERRUPTED *** 
...
int
C2F(ddaskr) (Dres res, int *neq, double *t, double *y, double *yprime,
	     ...
	     int *nrt, int *jroot)
{
 ...
 L30:
  ...
  if (info[12] == 0)
	  ...
	  if (info[5] == 0)
	    {
	      ...
	      msave = *neq / mband + 1;
	      lenrw =
		*nrt * 3 + 60 + (ncphi + 3) * *neq + lenpd + (msave << 1);
	    }
	  else
	    {
	      iwork[4] = 4;
  ...
  else if (info[12] == 1)
    {
      ...
      lenpd =
	(maxl + 3 + Min (i__1, i__2)) * *neq + (maxl + 3) * maxl + 1 + lenwp;
  ...
  if (lenic > 0)
    {
      nsp_ode_dcnst0 (neq, &y[1], &iwork[41], &iret);
      if (iret != 0)
	{
	  goto L727;
	}
 ...
 L100:
   ...
   * 
   */
  s_copy (msg, "DASKR--  THE LAST STEP TERMINATED WITH A NEGATIVE", 80L, 49L);
  nsp_ode_xerrwd (msg, &c__49, &c__201, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  VALUE (=I1) OF IDID AND NO APPROPRIATE", 80L, 47L);
  nsp_ode_xerrwd (msg, &c__47, &c__202, &c__0, &c__1, idid, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  ACTION WAS TAKEN. RUN TERMINATED", 80L, 41L);
  nsp_ode_xerrwd (msg, &c__41, &c__203, &c__1, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  return 0;
 L110:
  /* 
 ...
 L200:
  ...
  lrx = lr1 + *nrt;
  lwm = lrx + *nrt;
  /*    LXX = LWM+taille defined in cossimdaskr for Jacobian 
   */
  if (info[1] == 1)
    {
      goto L400;
  ...
  *idid = 1;
   ...
   * 
   */
  nsp_ode_ddawts1 (neq, &info[2], &rtol[1], &atol[1], &y[1], &rwork[lwt],
		      &rpar[1], &ipar[1]);
  nsp_ode_dinvwt (neq, &rwork[lwt], &ier);
  if (ier != 0)
    {
      goto L713;
    }
  ...
  /* 
   *    Compute unit roundoff and HMIN.  instead of D1MACH(4) we use 
   *    DLAMCH, because the optimized compiler affects the D1MACH. 
   *       UROUND = D1MACH(4) 
   */
  uround = nsp_dlamch ("p");
  rwork[9] = uround;
 ...
 L310:
  ...
  h0 = tdist * .001;
  ypnorm =
    nsp_ode_ddwnrm (neq, &yprime[1], &rwork[lvt], &rpar[1], &ipar[1]);
  if (ypnorm > .5 / h0)
    {
      h0 = .5 / ypnorm;
    }
  ...
  if (info[12] == 0)
    {
      ...
      lypic = lyic + *neq;
      lpwk = lypic;
      fun.d = nsp_ode_ddasid;
      nsp_ode_ddasic (&tn, &y[1], &yprime[1], neq, &info[11], &iwork[lid],
			 res,  jac,  psol, &h0, &tscale,
			 &rwork[lwt], &nwt, idid, &rpar[1], &ipar[1],
			 &rwork[lphi], &rwork[lsavr], &rwork[61], &rwork[le],
			 &rwork[lyic], &rwork[lypic], &rwork[lpwk],
			 &rwork[lwm], &iwork[1], &rwork[9], &rwork[10],
			 &rwork[11], &rwork[12], &epconi, &rwork[14],
			 &info[15], &icnflg, &iwork[41],&info[12],
			 fun);
    }
  else if (info[12] == 1)
    {
 ...
 L370:
  ...
  /*    -------------- masking -----------------
   */
  nsp_ode_drchek2 (&c__1,  rt, nrt, neq, t, tout, &y[1], &yprime[1],
		      &rwork[lphi], &rwork[39], &iwork[8], &rwork[lr0],
		      &rwork[lr1], &rwork[lrx], jroot, &irt, &rwork[9],
		      &info[3], &rwork[1], &iwork[1], &rpar[1], &ipar[1]);
  if (irt < 0)
    {
      goto L731;
    }
 ...
 L400:
   ...
   * 
   */
  nsp_ode_drchek2 (&c__2,  rt, nrt, neq, &tn, tout, &y[1],
		      &yprime[1], &rwork[lphi], &rwork[39], &iwork[8],
		      &rwork[lr0], &rwork[lr1], &rwork[lrx], jroot, &irt,
		      &rwork[9], &info[3], &rwork[1], &iwork[1], &rpar[1],
		      &ipar[1]);
  if (irt < 0)
    {
      goto L731;
 ...
 L410:
  ...
  nsp_ode_ddatrp1 (&tn, tout, &y[1], &yprime[1], neq, &iwork[8],
		      &rwork[lphi], &rwork[39]);
  *t = *tout;
  *idid = 3;
  done = TRUE;
 ...
 L420:
  ...
  nsp_ode_ddatrp1 (&tn, &tn, &y[1], &yprime[1], neq, &iwork[8],
		      &rwork[lphi], &rwork[39]);
  *t = tn;
  *idid = 1;
  done = TRUE;
 ...
 L450:
  ...
  nsp_ode_ddatrp1 (&tn, &tstop, &y[1], &yprime[1], neq, &iwork[8],
		      &rwork[lphi], &rwork[39]);
  *idid = 2;
  *t = tstop;
  done = TRUE;
 ...
 L505:
  ...
  avlin = (double) (iwork[20] - nli0) / (double) nnid;
  rcfn = (double) (iwork[15] - ncfn0) / (double) nstd;
  rcfl = (double) (iwork[16] - ncfl0) / (double) nnid;
  ...
  if (lavl)
    {
      s_copy (msg, "DASKR-- Warning. Poor iterative algorithm performance   ",
	      80L, 56L);
      nsp_ode_xerrwd (msg, &c__56, &c__501, &c__0, &c__0, &c__0, &c__0,
			 &c__0, &c_b38, &c_b38, 80L);
  ...
  if (lcfn)
    {
      s_copy (msg, "DASKR-- Warning. Poor iterative algorithm performance   ",
	      80L, 56L);
      nsp_ode_xerrwd (msg, &c__56, &c__502, &c__0, &c__0, &c__0, &c__0,
			 &c__0, &c_b38, &c_b38, 80L);
  ...
  if (lcfl)
    {
      s_copy (msg, "DASKR-- Warning. Poor iterative algorithm performance   ",
	      80L, 56L);
      nsp_ode_xerrwd (msg, &c__56, &c__503, &c__0, &c__0, &c__0, &c__0,
			 &c__0, &c_b38, &c_b38, 80L);
   ...
   */
 L510:
  nsp_ode_ddawts1 (neq, &info[2], &rtol[1], &atol[1], &rwork[lphi],
   ...
   */
  r__ =
    nsp_ode_ddwnrm (neq, &rwork[lphi], &rwork[lwt], &rpar[1],
		       &ipar[1]) * 100. * uround;
  if (r__ <= 1.)
    {
      goto L525;
 ...
 L525:
   ...
   *    ----------------------- Hmin Change---------------------- 
   * 
   *    HMIN is intended to be a value slightly above the roundoff level 
   *    in the current T.  As such it is appropriate that it varies with 
   *    T. In DASKR, HMIN is used in two ways: 
   * 
   *    1. At the start, ABS(TOUT - T) is required to be at least HMIN, to 
   *    guarantee that the user has provided the direction of the 
   *    integration reliably.  For this test, it would be sufficient to 
   *    ignore HMIN and simply require that TOUT - T is nonzero on the 
   *    machine. 
   * 
   *    2. If the integration has difficulty passing the convergence or 
   *    the error test with step size H of size ABS(H) = HMIN, it stops 
   *    with an error message saying that.  In all of these uses, it would 
   *    not hurt to use HMIN = 0, in my opinion.  There are cases where 
   *    the appropriate step size is temporarily below the roundoff level 
   *    in T.  The only negative impact of using HMIN = 0 then is that 
   *    some steps may be taken in which T + H = T on the machine.  In 
   *    contrast to the DASSL family, the ODEPACK solvers have HMIN = 0 as 
   *    the default in these uses of HMIN, but they issue a warning when T 
   *    + H = T, because in most cases this is the result of a user 
   *    program bug or input error of some kind. On the other hand, the 
   *    value HMIN = 4*UROUND makes no sense, because it ignores the scale 
   *    of the T variable completely.  Thus it could ause invalid error 
   *    halts, when values of ABS(H) smaller than that may well be 
   *    appropriate.  If the current HMIN is bothersome, I suggest using 
   *    HMIN = 0, and removing HMIN from the initial test on TOUT - T 
   * 
   * 
   ...
   * 
   *    info(12): 0-> dierct case  1->Krylov 
   */
  if (info[12] == 0)
    {
      Dned_dk fun;
      fun.d = nsp_ode_dnedd;
      nsp_ode_ddstp (&tn, &y[1], &yprime[1], neq,  res,  jac,
			psol, &h__, &rwork[lwt], &rwork[lvt], &info[1],
			idid, &rpar[1], &ipar[1], &rwork[lphi], &rwork[lsavr],
			&rwork[61], &rwork[le], &rwork[lwm], &iwork[1],
			&rwork[21], &rwork[27], &rwork[33], &rwork[39],
			&rwork[45], &rwork[5], &rwork[6], &rwork[7],
			&rwork[8], &hmin, &rwork[9], &rwork[10], &rwork[11],
			&rwork[12], &rwork[13], &iwork[6], &iwork[5],
			&info[15], &iwork[7], &iwork[8], &iwork[9], &nonneg,
			&info[12], fun);
    }
  else if (info[12] == 1)
    {
 ...
 L527:
   ...
   * 
   */
  nsp_ode_drchek2 (&c__3,  rt, nrt, neq, &tn, tout, &y[1],
		      &yprime[1], &rwork[lphi], &rwork[39], &iwork[8],
		      &rwork[lr0], &rwork[lr1], &rwork[lrx], jroot, &irt,
		      &rwork[9], &info[3], &rwork[1], &iwork[1], &rpar[1],
		      &ipar[1]);
 ...
 L600:
   ...
   */
 L610:
  s_copy (msg, "DASKR--  AT CURRENT T (=R1)  500 STEPS", 80L, 38L);
  nsp_ode_xerrwd (msg, &c__38, &c__610, &c__0, &c__0, &c__0, &c__0, &c__1,
		     &tn, &c_b38, 80L);
  s_copy (msg, "DASKR--  TAKEN ON THIS CALL BEFORE REACHING TOUT", 80L, 48L);
  nsp_ode_xerrwd (msg, &c__48, &c__611, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
   ...
   */
 L620:
  s_copy (msg, "DASKR--  AT T (=R1) TOO MUCH ACCURACY REQUESTED", 80L, 47L);
  nsp_ode_xerrwd (msg, &c__47, &c__620, &c__0, &c__0, &c__0, &c__0, &c__1,
		     &tn, &c_b38, 80L);
  s_copy (msg, "DASKR--  FOR PRECISION OF MACHINE. RTOL AND ATOL", 80L, 48L);
  nsp_ode_xerrwd (msg, &c__48, &c__621, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  WERE INCREASED BY A FACTOR R (=R1)", 80L, 43L);
  nsp_ode_xerrwd (msg, &c__43, &c__622, &c__0, &c__0, &c__0, &c__0, &c__1,
		     &r__, &c_b38, 80L);
  goto L700;
  /* 
   ...
   */
 L630:
  s_copy (msg, "DASKR--  AT T (=R1) SOME ELEMENT OF WT", 80L, 38L);
  nsp_ode_xerrwd (msg, &c__38, &c__630, &c__0, &c__0, &c__0, &c__0, &c__1,
		     &tn, &c_b38, 80L);
  s_copy (msg, "DASKR--  HAS BECOME .LE. 0.0", 80L, 28L);
  nsp_ode_xerrwd (msg, &c__28, &c__631, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
   ...
   */
 L640:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__640, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  ERROR TEST FAILED REPEATEDLY OR WITH ABS(H)=HMIN",
	  80L, 57L);
  nsp_ode_xerrwd (msg, &c__57, &c__641, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
 ...
 L650:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__650, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  NONLINEAR SOLVER FAILED TO CONVERGE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__651, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  REPEATEDLY OR WITH ABS(H)=HMIN", 80L, 39L);
  nsp_ode_xerrwd (msg, &c__40, &c__652, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
 ...
 L655:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__655, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  PRECONDITIONER HAD REPEATED FAILURES.", 80L, 46L);
  nsp_ode_xerrwd (msg, &c__46, &c__656, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
 ...
 L660:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__660, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  ITERATION MATRIX IS SINGULAR.", 80L, 38L);
  nsp_ode_xerrwd (msg, &c__38, &c__661, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
 ...
 L670:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__670, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  NONLINEAR SOLVER COULD NOT CONVERGE.", 80L, 45L);
  nsp_ode_xerrwd (msg, &c__45, &c__671, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  ALSO, THE ERROR TEST FAILED REPEATEDLY.", 80L, 48L);
  nsp_ode_xerrwd (msg, &c__49, &c__672, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
 ...
 L675:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__675, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  NONLINEAR SYSTEM SOLVER COULD NOT CONVERGE", 80L,
	  51L);
  nsp_ode_xerrwd (msg, &c__51, &c__676, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  BECAUSE IRES WAS EQUAL TO MINUS ONE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__677, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
   ...
   */
 L680:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2)", 80L, 40L);
  nsp_ode_xerrwd (msg, &c__40, &c__680, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  IRES WAS EQUAL TO MINUS TWO", 80L, 36L);
  nsp_ode_xerrwd (msg, &c__36, &c__681, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
 ...
 L685:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__685, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  INITIAL (Y,YPRIME) COULD NOT BE COMPUTED", 80L, 49L);
  nsp_ode_xerrwd (msg, &c__49, &c__686, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h0, 80L);
  goto L700;
  /* 
 ...
 L690:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2)", 80L, 40L);
  nsp_ode_xerrwd (msg, &c__40, &c__690, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  IER WAS NEGATIVE FROM PSOL", 80L, 35L);
  nsp_ode_xerrwd (msg, &c__35, &c__691, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
 ...
 L695:
  s_copy (msg, "DASKR--  AT T (=R1) AND STEPSIZE H (=R2) THE", 80L, 44L);
  nsp_ode_xerrwd (msg, &c__44, &c__695, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h__, 80L);
  s_copy (msg, "DASKR--  LINEAR SYSTEM SOLVER COULD NOT CONVERGE.", 80L, 49L);
  nsp_ode_xerrwd (msg, &c__50, &c__696, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L700;
  /* 
   */
 L696:
  ...
  nsp_ode_xerrwd (msg, &c__44, &c__685, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "DASKR--  INITIAL Jacobian COULD NOT BE COMPUTED", 80L, 47L);
  nsp_ode_xerrwd (msg, &c__49, &c__686, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tn, &h0, 80L);
  goto L700;
  /* 
  ...
  *t = tn;
   ...
   */
 L701:
  s_copy (msg, "DASKR--  ELEMENT (=I1) OF INFO VECTOR IS NOT VALID", 80L,
	  50L);
  nsp_ode_xerrwd (msg, &c__50, &c__1, &c__0, &c__1, &itemp, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L702:
  s_copy (msg, "DASKR--  NEQ (=I1) .LE. 0", 80L, 25L);
  nsp_ode_xerrwd (msg, &c__25, &c__2, &c__0, &c__1, neq, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L703:
  s_copy (msg, "DASKR--  MAXORD (=I1) NOT IN RANGE", 80L, 34L);
  nsp_ode_xerrwd (msg, &c__34, &c__3, &c__0, &c__1, &mxord, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L704:
  s_copy (msg, "DASKR--  RWORK LENGTH NEEDED, LENRW (=I1), EXCEEDS LRW (=I2)",
	  80L, 60L);
  nsp_ode_xerrwd (msg, &c__60, &c__4, &c__0, &c__2, &lenrw, lrw, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L705:
  s_copy (msg, "DASKR--  IWORK LENGTH NEEDED, LENIW (=I1), EXCEEDS LIW (=I2)",
	  80L, 60L);
  nsp_ode_xerrwd (msg, &c__60, &c__5, &c__0, &c__2, &leniw, liw, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L706:
  s_copy (msg, "DASKR--  SOME ELEMENT OF RTOL IS .LT. 0", 80L, 39L);
  nsp_ode_xerrwd (msg, &c__39, &c__6, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L707:
  s_copy (msg, "DASKR--  SOME ELEMENT OF ATOL IS .LT. 0", 80L, 39L);
  nsp_ode_xerrwd (msg, &c__39, &c__7, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L708:
  s_copy (msg, "DASKR--  ALL ELEMENTS OF RTOL AND ATOL ARE ZERO", 80L, 47L);
  nsp_ode_xerrwd (msg, &c__47, &c__8, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L709:
  s_copy (msg, "DASKR--  INFO(4) = 1 AND TSTOP (=R1) BEHIND TOUT (=R2)", 80L,
	  54L);
  nsp_ode_xerrwd (msg, &c__54, &c__9, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tstop, tout, 80L);
  goto L750;
 L710:
  s_copy (msg, "DASKR--  HMAX (=R1) .LT. 0.0", 80L, 28L);
  nsp_ode_xerrwd (msg, &c__28, &c__10, &c__0, &c__0, &c__0, &c__0, &c__1,
		     &hmax, &c_b38, 80L);
  goto L750;
 L711:
  s_copy (msg, "DASKR--  TOUT (=R1) BEHIND T (=R2)", 80L, 34L);
  nsp_ode_xerrwd (msg, &c__34, &c__11, &c__0, &c__0, &c__0, &c__0, &c__2,
		     tout, t, 80L);
  goto L750;
 L712:
  s_copy (msg, "DASKR--  INFO(8)=1 AND H0=0.0", 80L, 29L);
  nsp_ode_xerrwd (msg, &c__29, &c__12, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L713:
  s_copy (msg, "DASKR--  SOME ELEMENT OF WT IS .LE. 0.0", 80L, 39L);
  nsp_ode_xerrwd (msg, &c__39, &c__13, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
   ...
   */
 L715:
  s_copy (msg, "DASKR--  INFO(4)=1 AND TSTOP (=R1) BEHIND T (=R2)", 80L, 49L);
  nsp_ode_xerrwd (msg, &c__49, &c__15, &c__0, &c__0, &c__0, &c__0, &c__2,
		     &tstop, t, 80L);
  goto L750;
 L717:
  s_copy (msg, "DASKR--  ML (=I1) ILLEGAL. EITHER .LT. 0 OR .GT. NEQ", 80L,
	  52L);
  nsp_ode_xerrwd (msg, &c__52, &c__17, &c__0, &c__1, &iwork[1], &c__0,
		     &c__0, &c_b38, &c_b38, 80L);
  goto L750;
 L718:
  s_copy (msg, "DASKR--  MU (=I1) ILLEGAL. EITHER .LT. 0 OR .GT. NEQ", 80L,
	  52L);
  nsp_ode_xerrwd (msg, &c__52, &c__18, &c__0, &c__1, &iwork[2], &c__0,
		     &c__0, &c_b38, &c_b38, 80L);
  goto L750;
 L719:
  s_copy (msg, "DASKR--  TOUT (=R1) IS EQUAL TO T (=R2)", 80L, 39L);
  nsp_ode_xerrwd (msg, &c__39, &c__19, &c__0, &c__0, &c__0, &c__0, &c__2,
		     tout, t, 80L);
  goto L750;
 L720:
  s_copy (msg, "DASKR--  MAXL (=I1) ILLEGAL. EITHER .LT. 1 OR .GT. NEQ", 80L,
	  54L);
  nsp_ode_xerrwd (msg, &c__54, &c__20, &c__0, &c__1, &iwork[24], &c__0,
		     &c__0, &c_b38, &c_b38, 80L);
  goto L750;
 L721:
  s_copy (msg, "DASKR--  KMP (=I1) ILLEGAL. EITHER .LT. 1 OR .GT. MAXL", 80L,
	  54L);
  nsp_ode_xerrwd (msg, &c__54, &c__21, &c__0, &c__1, &iwork[25], &c__0,
		     &c__0, &c_b38, &c_b38, 80L);
  goto L750;
 L722:
  s_copy (msg, "DASKR--  NRMAX (=I1) ILLEGAL. .LT. 0", 80L, 36L);
  nsp_ode_xerrwd (msg, &c__36, &c__22, &c__0, &c__1, &iwork[26], &c__0,
		     &c__0, &c_b38, &c_b38, 80L);
  goto L750;
 L723:
  s_copy (msg, "DASKR--  EPLI (=R1) ILLEGAL. EITHER .LE. 0.D0 OR .GE. 1.D0",
	  80L, 58L);
  nsp_ode_xerrwd (msg, &c__58, &c__23, &c__0, &c__0, &c__0, &c__0, &c__1,
		     &rwork[10], &c_b38, 80L);
  goto L750;
 L724:
  s_copy (msg, "DASKR--  ILLEGAL IWORK VALUE FOR INFO(11) .NE. 0", 80L, 48L);
  nsp_ode_xerrwd (msg, &c__48, &c__24, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L725:
  s_copy (msg, "DASKR--  ONE OF THE INPUTS FOR INFO(17) = 1 IS ILLEGAL", 80L,
	  54L);
  nsp_ode_xerrwd (msg, &c__54, &c__25, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L726:
  s_copy (msg, "DASKR--  ILLEGAL IWORK VALUE FOR INFO(10) .NE. 0", 80L, 48L);
  nsp_ode_xerrwd (msg, &c__48, &c__26, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L727:
  s_copy (msg, "DASKR--  Y(I) AND IWORK(40+I) (I=I1) INCONSISTENT", 80L, 49L);
  nsp_ode_xerrwd (msg, &c__49, &c__27, &c__0, &c__1, &iret, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L730:
  s_copy (msg, "DASKR--  NRT (=I1) .LT. 0", 80L, 25L);
  nsp_ode_xerrwd (msg, &c__25, &c__30, &c__1, &c__1, nrt, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  goto L750;
 L731:
  s_copy (msg, "DASKR--  R IS ILL-DEFINED.  ZERO VALUES WERE FOUND AT TWO",
	  80L, 57L);
  nsp_ode_xerrwd (msg, &c__57, &c__31, &c__1, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  s_copy (msg, "         VERY CLOSE T VALUES, AT T = R1", 80L, 39L);
  nsp_ode_xerrwd (msg, &c__39, &c__31, &c__1, &c__0, &c__0, &c__0, &c__1,
		     &rwork[51], &c_b38, 80L);
  /* 
   */
 L750:
  ...
  *idid = -33;
  return 0;
 L760:
  s_copy (msg, "DASKR--  REPEATED OCCURRENCES OF ILLEGAL INPUT", 80L, 46L);
  nsp_ode_xerrwd (msg, &c__46, &c__701, &c__0, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  /*    770 CONTINUE 
   */
  s_copy (msg, "DASKR--  RUN TERMINATED. APPARENT INFINITE LOOP", 80L, 47L);
  nsp_ode_xerrwd (msg, &c__47, &c__702, &c__1, &c__0, &c__0, &c__0, &c__0,
		     &c_b38, &c_b38, 80L);
  return 0;
  /* 
...
*/
   ...
   *         IRT = 2  A zero-crossing surface has detached from zero 
   * 
 ...
 L100:
  ...
  nsp_ode_ddatrp1 (tn, &rwork[51], &y[1], &yp[1], neq, kold,
		      &phi[phi_offset], &psi[1]);
 ...
 L330:
  nsp_ode_ddatrp1 (tn, &t1, &y[1], &yp[1], neq, kold, &phi[phi_offset],
		      &psi[1]);
 ...
 L350:
  nsp_ode_droots2 (nrt, &hminr, &jflag, &rwork[51], &t1, &r0[1], &r1[1],
		      &rx[1], &x, &jroot[1]);
  if (jflag > 1)
    {
      goto L360;
    }
  nsp_ode_ddatrp1 (tn, &x, &y[1], &yp[1], neq, kold, &phi[phi_offset],
		      &psi[1]);
 ...
 L360:
  if (jflag == 2)
      ...
      for (i__ = 1; i__ <= i__1; ++i__)
	{
	  if (iwork[lmask + i__] == 1)
		...
		{
		  jroot[i__] = (int) d_sign (&c_b770, &r1[i__]);
		  mroot = TRUE;
		}
	      else
		{
	  ...
	  else
	    {
	      if ((d__1 = r1[i__], Abs (d__1)) == zero)
		{
		  jroot[i__] = (int) (-d_sign (&c_b771, &r0[i__]));
		  zroot = TRUE;
		}
	      else
		{
		  if (d_sign (&c_b771, &r0[i__]) !=
		      d_sign (&c_b771, &r1[i__]))
		    {
		      d__1 = r1[i__] - r0[i__];
		      jroot[i__] = (int) d_sign (&c_b771, &d__1);
		      zroot = TRUE;
		    }
		  else
		    {
  ...
  rwork[51] = x;
  C2F (dcopy) (nrt, &rx[1], &c__1, &r0[1], &c__1);
  return 0;
  /*----------------------END OF SUBROUTINE DRCHEk2 ----------------------- 
   */
}				/* drchek2_ */
 ...
 */
int
nsp_ode_droots2 (int *nrt, double *hmin, int *jflag, double *x0,
		    double *x1, double *r0, double *r1, double *rx, double *x,
		    int *jroot)
{
  /* Initialized data */
...
*/
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      if (jroot[i__] == 1 && (d__1 = r1[i__], Abs (d__1)) > zero)
	{
	  iunstuck = i__;
	}
      if ((d__1 = r1[i__], Abs (d__1)) > zero)
      ...
      if (jroot[i__] == 1)
	{
       ...
       */
    L110:
      if (d_sign (&c_b771, &r0[i__]) == d_sign (&c_b771, &r1[i__]))
	{
	  goto L120;
	}
 ...
 L170:
  alpha *= 2.;
 L180:
  if ((d__1 = r0[imax], Abs (d__1)) == zero
      || (d__2 = r1[imax], Abs (d__2)) == zero)
    {
      x2 = (*x0 + alpha * *x1) / (alpha + 1);
    }
  else
    {
 ...
 L200:
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      if (jroot[i__] == 1 && (d__1 = rx[i__], Abs (d__1)) > zero)
	{
	  iunstuck = i__;
	}
      ...
      if (jroot[i__] == 1)
	{
       ...
       */
    L210:
      if (d_sign (&c_b771, &r0[i__]) == d_sign (&c_b771, &rx[i__]))
	{
	  goto L220;
	}
 ...
 L240:
   ...
   */
  *x1 = x2;
  C2F (dcopy) (nrt, &rx[1], &c__1, &r1[1], &c__1);
  last = 1;
  xroot = FALSE;
  goto L270;
 L260:
  ...
  *jflag = 2;
   ...
   */
  *x = *x1;
  C2F (dcopy) (nrt, &r1[1], &c__1, &rx[1], &c__1);
  return 0;
...
int
nsp_ode_drchek1 (int *job, Rt rt, int *nrt, int *neq, double *tn,
		    double *tout, double *y, double *yp, double *phi,
		    double *psi, int *kold, double *r0, double *r1,
		    double *rx, int *jroot, int *irt, double *uround,
		    int *info3, double *rwork, int *iwork, double *rpar,
...
*/
 ...
 L100:
  ...
  (*rt) (neq, &rwork[51], &y[1], &yp[1], nrt, &r0[1], rpar, ipar);
  ++iwork[36];
  /*    ---------   MASKING THE STUCK ZEROS IN COLD-RESTARTS 
   */
  *irt = 0;
  i__1 = *nrt;
  for (i__ = 1; i__ <= i__1; ++i__)
      ...
      if (jroot[i__] == 1)
	{
	  if ((d__1 = r0[i__], Abs (d__1)) == zero)
	    ...
	    {
	      /*    .        to take one step through DDSTP then then in the next arrival->exit! 
	       */
	      *irt = 2;
	      jroot[i__] = (int) d_sign (&c_b770, &r0[i__]);
	    }
	}
      /* L130: */
    }
 ...
 L240:
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
      ...
      if ((d__1 = r0[i__], Abs (d__1)) > zero)
	{
      ...
      if (jroot[i__] == 1)
	{
	  /*    .      MASKING THE STUCK ZEROS IN HOT-RESTARTS 
	   */
	  iwork[lmask + i__] = 1;
	  jroot[i__] = 0;
	}
 ...
 L260:
   ...
   */
 L300:
  /*    AT THE BEGINING OF THE PREVIOUS STEP THERE WERE A MASK-LIFTING 
   */
  if (iwork[37] == 2)
    {
      iwork[37] = 0;
 ...
 L330:
  ...
  (*rt) (neq, &t1, &y[1], &yp[1], nrt, &r1[1], rpar, ipar);
  ++iwork[36];
  /*    .   PASSING THE MASK THROUGH DROOT2 VIA JROOT(I) 
   */
  i__1 = *nrt;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
  ...
  jflag = 0;
 L350:
  nsp_ode_droots1 (nrt, &hminr, &jflag, &rwork[51], &t1, &r0[1], &r1[1],
		      &rx[1], &x, &jroot[1]);
  if (jflag > 1)
    {
      goto L360;
 ...
 L360:
  ...
  return 0;
  /*---------------------- END OF SUBROUTINE DRCHE ----------------------- 
   */
}				/* drchek1_ */

int
nsp_ode_droots1 (int *nrt, double *hmin, int *jflag, double *x0,
		    double *x1, double *r0, double *r1, double *rx, double *x,
		    int *jroot)
{
  /* Initialized data */
...
*/
 ...
 L180:
  ...
  /*----------------------- Hindmarsh ---------------- 
   *    I recently studied the rootfinding algorithm in some detail, and 
   *    found that there is a high potential for an infinite loop within 
   *    the subroutine DROOTS/SROOTS.  This is caused by an adjustment to 
   *    the new computed iterate, called X2 there at statement 180.  The 
   *    adjustment following 180 is faulty, and should be replaced as 
   *    follows. This logic moves X2 away from one endpoint of the current 
   *    interval bracketing the root if it is too close, but in a way that 
   *    cannot result in an infinite loop.  Even if you have not 
   *    encountered any trouble at this spot in DASKR, I recommend you 
   *    make the change. 
   *c      IF ((ABS(X2-X0) .LT. HMIN) .AND. 
   *c     1   (ABS(X1-X0) .GT. 10.0D0*HMIN)) X2 = X0 + 0.1D0*(X1-X0) 
   */
  if ((d__1 = x2 - *x0, Abs (d__1)) < half * *hmin)
    {
      fracint = (d__1 = *x1 - *x0, Abs (d__1)) / *hmin;
...
int
nsp_ode_drchek0 (int *job, Rt rt, int *nrt, int *neq, double *tn,
		    double *tout, double *y, double *yp, double *phi,
		    double *psi, int *kold, double *r0, double *r1,
		    double *rx, int *jroot, int *irt, double *uround,
		    int *info3, double *rwork, int *iwork, double *rpar,
...
*/
 ...
 L190:
   ...
   */
 L200:
  /*    if in the previous step a z-crossing or mask lifting has occured... 
   */
  if (iwork[37] == 0)
    {
      goto L260;
 ...
 L330:
  ...
  jflag = 0;
 L350:
  nsp_ode_droots0 (nrt, &hminr, &jflag, &rwork[51], &t1, &r0[1], &r1[1],
		      &rx[1], &x, &jroot[1]);
  if (jflag > 1)
    {
      goto L360;
...
int
nsp_ode_droots0 (int *nrt, double *hmin, int *jflag, double *x0,
		    double *x1, double *r0, double *r1, double *rx, double *x,
		    int *jroot)
{
  /* Initialized data */
...
*/
  ...
  *idid = 1;
  ...
  C2F (dcopy) (neq, &y[1], &c__1, &phi[phi_dim1 + 1], &c__1);
  C2F (dcopy) (neq, &yprime[1], &c__1, &phi[(phi_dim1 << 1) + 1], &c__1);
  /* 
   */
  if (*icopt == 2)
    {
 ...
 L200:
  if ( *ntype == 0 ) 
    (*nlsic.d) (x, &y[1], &yprime[1], neq, icopt, &id[1], res, jac.d,
		psol.d, h__, tscale, &wt[1], &jskip, &rpar[1], &ipar[1],
		&savr[1], &delta[1], &e[1], &yic[1], &ypic[1], &pwk[1], &wm[1],
		&iwm[1], &cj, uround, epli, sqrtn, rsqrtn, epconi, &ratemx,
		stptol, jflg, icnflg, &icnstr[1], &iernls);
  else 
    (*nlsic.k) (x, &y[1], &yprime[1], neq, icopt, &id[1], res, jac.k,
		psol.k, h__, tscale, &wt[1], &jskip, &rpar[1], &ipar[1],
		&savr[1], &delta[1], &e[1], &yic[1], &ypic[1], &pwk[1], &wm[1],
		&iwm[1], &cj, uround, epli, sqrtn, rsqrtn, epconi, &ratemx,
		stptol, jflg, icnflg, &icnstr[1], &iernls);
  ...
  *h__ *= rhcut;
  ...
  /* 
   */
  C2F (dcopy) (neq, &phi[phi_dim1 + 1], &c__1, &y[1], &c__1);
  C2F (dcopy) (neq, &phi[(phi_dim1 << 1) + 1], &c__1, &yprime[1], &c__1);
  goto L200;
  /* 
   */
 L350:
...
int
nsp_ode_dyypnw (int *neq, double *y, double *yprime, double *cj,
		   double *rl, double *p, int *icopt, int *id, double *ynew,
		   double *ypnew)
{
  /* System generated locals */
  int i__1;
...
*/
  ...
  *idid = 1;
   ...
   */
  if (*ntype == 0) 
    (*nls.d) (x, &y[1], &yprime[1], neq, res, jac.d, psol.d, h__,
	      &wt[1], jstart, idid, &rpar[1], &ipar[1], &phi[phi_offset],
	      &gamma[1], &savr[1], &delta[1], &e[1], &wm[1], &iwm[1], cj, cjold,
	      &cjlast, s, uround, epli, sqrtn, rsqrtn, epcon, jcalc, jflg, &kp1,
	      nonneg, ntype, &iernls);
  else
    (nls.k) (x, &y[1], &yprime[1], neq, res, jac.k, psol.k, h__,
	     &wt[1], jstart, idid, &rpar[1], &ipar[1], &phi[phi_offset],
	     &gamma[1], &savr[1], &delta[1], &e[1], &wm[1], &iwm[1], cj, cjold,
	     &cjlast, s, uround, epli, sqrtn, rsqrtn, epcon, jcalc, jflg, &kp1,
	     nonneg, ntype, &iernls);
   ...
   * 
   */
  enorm = nsp_ode_ddwnrm (neq, &e[1], &vt[1], &rpar[1], &ipar[1]);
  /* 
   */
  erk = sigma[*k + 1] * enorm;
  terk = (*k + 1) * erk;
  ...
  erkm1 =
    sigma[*k] * nsp_ode_ddwnrm (neq, &delta[1], &vt[1], &rpar[1],
				   &ipar[1]);
  terkm1 = *k * erkm1;
  if (*k > 2)
    {
 ...
 L410:
  ...
  erkm2 =
    sigma[*k - 1] * nsp_ode_ddwnrm (neq, &delta[1], &vt[1], &rpar[1],
				       &ipar[1]);
  terkm2 = (*k - 1) * erkm2;
  ...
  *hold = *h__;
  ...
  erkp1 =
    1. / (*k + 2) * nsp_ode_ddwnrm (neq, &delta[1], &vt[1], &rpar[1],
				       &ipar[1]);
  terkp1 = (*k + 2) * erkp1;
  if (*k > 1)
    {
  ...
  *idid = -6;
   ...
   */
 L675:
  nsp_ode_ddatrp1 (x, x, &y[1], &yprime[1], neq, k, &phi[phi_offset],
		      &psi[1]);
  *jstart = 1;
  if (*idid >= 0)
    {
...
int
nsp_ode_dcnstr (int *neq, double *y, double *ynew, int *icnstr,
...
int nsp_ode_dcnst0 (int *neq, double *y, int *icnstr, int *iret)
{
  /* Initialized data */
...
int
nsp_ode_ddawts1 (int *neq, int *iwt, double *rtol, double *atol, double *y,
...
int nsp_ode_dinvwt (int *neq, double *wt, int *ier)
{
  /* System generated locals */
  int i__1;
...
int
nsp_ode_ddatrp1 (double *x, double *xout, double *yout, double *ypout,
...
double
nsp_ode_ddwnrm (int *neq, double *v, double *rwt, double *rpar, int *ipar)
{
  /* System generated locals */
  int i__1;
...
*/
 ...
 L300:
  ...
  ++nj;
  ++iwm[13];
  nsp_ode_dmatd (neq, x, &y[1], &yprime[1], &delta[1], cj, h__, &ierj,
		    &wt[1], &r__[1], &wm[1], &iwm[1], res, &ires,
		    uround,  jacd, &rpar[1], &ipar[1]);
  /*    assigning two different error message for singular-Jacobian and 
   *    internal error 
   */
  if (ires < 0)
    {
   ...
   */
  nsp_ode_dnsid (x, &y[1], &yprime[1], neq, icopt, &id[1],  res,
		    &wt[1], &rpar[1], &ipar[1], &delta[1], &r__[1], &yic[1],
		    &ypic[1], &wm[1], &iwm[1], cj, tscale, epcon, ratemx,
		    &mxnit, stptol, icnflg, &icnstr[1], &iernew);
  if (iernew == 1 && nj < mxnj)
    {
      /* 
   ...
   * 
   *    unrecoverable error in nonlinear solver. 
   */
 L370:
  *iernls = -1;
  return 0;
...
*/
   ...
   * 
   */
  nsp_ode_dslvd (neq, &delta[1], &wm[1], &iwm[1]);
  /* 
   *    Get norm of DELTA.  Return now if norm(DELTA) .le. EPCON. 
   * 
   */
  delnrm = nsp_ode_ddwnrm (neq, &delta[1], &wt[1], &rpar[1], &ipar[1]);
  fnrm = delnrm;
  if (*tscale > 0.)
    {
 ...
 L300:
  ...
  /* 
   */
  nsp_ode_dlinsd (neq, &y[1], x, &yprime[1], cj, tscale, &delta[1],
		     &delnrm, &wt[1], &lsoff, stptol, &iret,  res,
		     &ires, &wm[1], &iwm[1], &fnrm, icopt, &id[1], &r__[1],
		     &yic[1], &ypic[1], icnflg, &icnstr[1], &rlx, &rpar[1],
		     &ipar[1]);
  /* 
   */
  rate = fnrm / oldfnm;
  ...
  *iernew = 1;
   ...
   * 
   */
  C2F (dcopy) (neq, &r__[1], &c__1, &delta[1], &c__1);
  delnrm = fnrm;
  goto L300;
  /* 
...
*/
  ...
  if (kprin >= 2)
    {
      s_copy (msg, "------ IN ROUTINE DLINSD-- PNRM = (R1)", 80L, 38L);
      nsp_ode_xerrwd (msg, &c__38, &c__901, &c__0, &c__0, &c__0, &c__0,
			 &c__1, pnrm, &c_b38, 80L);
    }
  tau = *pnrm;
  rl = one;
  ...
  if (*icnflg != 0)
    {
    L10:
      nsp_ode_dyypnw (neq, &y[1], &yprime[1], cj, &rl, &p[1], icopt,
			 &id[1], &ynew[1], &ypnew[1]);
      nsp_ode_dcnstr (neq, &y[1], &ynew[1], &icnstr[1], &tau, rlx, iret,
			 &ivar);
      if (*iret == 1)
	{
	  ratio1 = tau / *pnrm;
	  ...
	  if (kprin >= 2)
	    {
	      s_copy (msg,
		      ...
		      80L, 50L);
	      nsp_ode_xerrwd (msg, &c__50, &c__902, &c__0, &c__1, &ivar,
				 &c__0, &c__1, pnrm, &c_b38, 80L);
	    }
	  if (*pnrm <= *stptol)
	    {
	      *iret = 1;
  ...
  if (*lsoff == 0 && kprin >= 2)
    {
      s_copy (msg, "------ MIN. LAMBDA = (R1)", 80L, 25L);
      nsp_ode_xerrwd (msg, &c__25, &c__903, &c__0, &c__0, &c__0, &c__0,
			 &c__1, &rlmin, &c_b38, 80L);
    }
  /*----------------------------------------------------------------------- 
 ...
 L100:
  /*    ---------------------------------------------------------- 
   */
  nsp_ode_dyypnw (neq, &y[1], &yprime[1], cj, &rl, &p[1], icopt, &id[1],
		     &ynew[1], &ypnew[1]);
  nsp_ode_dfnrmd (neq, &ynew[1], t, &ypnew[1], &r__[1], cj, tscale, &wt[1],
		     res, ires, &fnrmp, &wm[1], &iwm[1], &rpar[1],
		     &ipar[1]);
  ++iwm[12];
  if (*ires != 0)
    {
  ...
  if (kprin >= 2)
    {
      s_copy (msg, "------ LAMBDA = (R1)", 80L, 20L);
      nsp_ode_xerrwd (msg, &c__20, &c__904, &c__0, &c__0, &c__0, &c__0,
			 &c__1, &rl, &c_b38, 80L);
      s_copy (msg, "------ NORM(F1) = (R1),  NORM(F1NEW) = (R2)", 80L, 43L);
      nsp_ode_xerrwd (msg, &c__43, &c__905, &c__0, &c__0, &c__0, &c__0,
			 &c__2, &f1nrm, &f1nrmp, 80L);
    }
 ...
 L150:
  *iret = 0;
  C2F (dcopy) (neq, &ynew[1], &c__1, &y[1], &c__1);
  C2F (dcopy) (neq, &ypnew[1], &c__1, &yprime[1], &c__1);
  *fnrm = fnrmp;
  if (kprin >= 1)
    {
      s_copy (msg, "------ LEAVING ROUTINE DLINSD, FNRM = (R1)", 80L, 42L);
      nsp_ode_xerrwd (msg, &c__42, &c__906, &c__0, &c__0, &c__0, &c__0,
			 &c__1, fnrm, &c_b38, 80L);
    }
  return 0;
  /*----------------------------------------------------------------------- 
...
*/
  ...
  *ires = 0;
   ...
   *----------------------------------------------------------------------- 
   */
  nsp_ode_dslvd (neq, &r__[1], &wm[1], &iwm[1]);
  /*----------------------------------------------------------------------- 
   *    Calculate norm of R. 
   *----------------------------------------------------------------------- 
   */
  *fnorm = nsp_ode_ddwnrm (neq, &r__[1], &wt[1], &rpar[1], &ipar[1]);
  if (*tscale > 0.)
    {
...
*/
 ...
 L300:
  ...
  pnorm = nsp_ode_ddwnrm (neq, &y[1], &wt[1], &rpar[1], &ipar[1]);
   ...
   */
  temp1 = 2. / (*cj / *cjold + 1.);
  nsp_ode_dnsd (x, &y[1], &yprime[1], neq,  res, pdum, &wt[1],
		   &rpar[1], &ipar[1], dumsvr, &delta[1], &e[1], &wm[1],
		   &iwm[1], cj, dums, dumr, dume, epcon, s, &temp1, &tolnew,
		   &muldel, &maxit, &ires, &idum, &iernew);
  /* 
   */
  if (iernew > 0 && *jcalc != 0)
    {
		  ...
		  int *ipar)
{
 ...
 L100:
  ...
  /*    for mixed-model's Jacobian we need to pass some parameters 
   */
  wm[*neq * *neq + 1] = *h__;
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      /*Computing MAX 
       */
      d__4 = 1., d__5 = (d__1 = y[i__], Abs (d__1)), d__4 =
	Max (d__4, d__5), d__5 = (d__2 =
				  *h__ * yprime[i__], Abs (d__2)), d__4 =
	Max (d__4, d__5), d__5 = (d__3 = 1. / ewt[i__], Abs (d__3));
  ...
  *ires = 0;
  ...
  for (j = 1; j <= i__1; ++j)
      ...
      for (n = j; i__3 < 0 ? n >= i__2 : n <= i__2; n += i__3)
	{
	  ...
	  d__4 = (d__1 = y[n], Abs (d__1)), d__5 = (d__2 =
						    *h__ * yprime[n],
						    Abs (d__2)), d__4 =
	    Max (d__4, d__5), d__5 = (d__3 = 1. / ewt[n], Abs (d__3));
	  del = squr * Max (d__4, d__5);
	  d__1 = *h__ * yprime[n];
	  del = d_sign (&del, &d__1);
	  del = y[n] + del - y[n];
   ...
   */
 L550:
  nsp_ode_dgbfa (&wm[1], &meband, neq, &iwm[1], &iwm[2], &iwm[lipvt], ier);
  return 0;
  /* 
...
int nsp_ode_dslvd (int *neq, double *delta, double *wm, int *iwm)
{
  int lipvt, mtype, meband;
...
*/
 ...
 L400:
  meband = (iwm[1] << 1) + iwm[2] + 1;
  nsp_ode_dgbsl (&wm[1], &meband, neq, &iwm[1], &iwm[2], &iwm[lipvt],
		    &delta[1], &c__0);
  return 0;
  /* 
...
*/
  ...
  if (*jflg == 1 && *jskip == 0)
    {
      ++nj;
      ++iwm[13];
      (*jack) ( res, &ires, neq, x, &y[1], &yprime[1], &wt[1],
		&delta[1], &r__[1], h__, cj, &wm[lwp], &iwm[liwp], &ierpj,
		&rpar[1], &ipar[1]);
      if (ires < 0 || ierpj != 0)
	{
	  goto L370;
  ...
  *jskip = 0;
   ...
   * 
   */
  nsp_ode_dnsik (x, &y[1], &yprime[1], neq, icopt, &id[1],  res,
		    psol, &wt[1], &rpar[1], &ipar[1], &savr[1],
		    &delta[1], &r__[1], &yic[1], &ypic[1], &pwk[1], &wm[1],
		    &iwm[1], cj, tscale, sqrtn, rsqrtn, &eplin, epcon, ratemx,
		    &mxnit, stptol, icnflg, &icnstr[1], &iernew);
  /* 
   */
  if (iernew == 1 && nj < mxnj && *jflg == 1)
    {
       ...
       * 
       */
      C2F (dcopy) (neq, &savr[1], &c__1, &delta[1], &c__1);
      goto L300;
    }
  /* 
   */
...
*/
   ...
   * 
   */
  C2F (dcopy) (neq, &delta[1], &c__1, &savr[1], &c__1);
  /* 
   *    Compute norm of (P-inverse)*(residual). 
   * 
   */
  nsp_ode_dfnrmk (neq, &y[1], x, &yprime[1], &savr[1], &r__[1], cj, tscale,
		     &wt[1], sqrtn, rsqrtn,  res, &ires,  psol,
		     &c__1, &ier, &fnrm, eplin, &wm[lwp], &iwm[liwp], &pwk[1],
		     &rpar[1], &ipar[1]);
  ++iwm[21];
  if (ier != 0)
    {
 ...
 L300:
   ...
   * 
   */
  nsp_ode_dslvk (neq, &y[1], x, &yprime[1], &savr[1], &delta[1], &wt[1],
		    &wm[1], &iwm[1],  res, &ires,  psol, &iersl,
		    cj, eplin, sqrtn, rsqrtn, &rhok, &rpar[1], &ipar[1]);
  if (ires != 0 || iersl != 0)
    {
      goto L390;
    }
  ...
  /* 
   */
  nsp_ode_dlinsk (neq, &y[1], x, &yprime[1], &savr[1], cj, tscale,
		     &delta[1], &delnrm, &wt[1], sqrtn, rsqrtn, &lsoff,
		     stptol, &iret,  res, &ires,  psol, &wm[1],
		     &iwm[1], &rhok, &fnrm, icopt, &id[1], &wm[lwp],
		     &iwm[liwp], &r__[1], eplin, &yic[1], &ypic[1], &pwk[1],
...
*/
  ...
  if (kprin >= 2)
    {
      s_copy (msg, "------ IN ROUTINE DLINSK-- PNRM = (R1)", 80L, 38L);
      nsp_ode_xerrwd (msg, &c__38, &c__921, &c__0, &c__0, &c__0, &c__0,
			 &c__1, pnrm, &c_b38, 80L);
    }
  tau = *pnrm;
  rl = one;
	  ...
	  if (kprin >= 2)
	    {
	      s_copy (msg,
		      "------ CONSTRAINT VIOL., PNRM = (R1), INDEX = (I1)",
		      80L, 50L);
	      nsp_ode_xerrwd (msg, &c__50, &c__922, &c__0, &c__1, &ivar,
				 &c__0, &c__1, pnrm, &c_b38, 80L);
	    }
	  if (*pnrm <= *stptol)
	    {
  ...
  if (*lsoff == 0 && kprin >= 2)
    {
      s_copy (msg, "------ MIN. LAMBDA = (R1)", 80L, 25L);
      nsp_ode_xerrwd (msg, &c__25, &c__923, &c__0, &c__0, &c__0, &c__0,
			 &c__1, &rlmin, &c_b38, 80L);
    }
  /*----------------------------------------------------------------------- 
   *Begin iteration to find RL value satisfying alpha-condition. 
 ...
 L100:
  nsp_ode_dyypnw (neq, &y[1], &yprime[1], cj, &rl, &p[1], icopt, &id[1],
		     &ynew[1], &ypnew[1]);
  nsp_ode_dfnrmk (neq, &ynew[1], t, &ypnew[1], &savr[1], &r__[1], cj,
		     tscale, &wt[1], sqrtn, rsqrtn,  res, ires,
		     psol, &c__0, &ier, &fnrmp, eplin, &wp[1], &iwp[1],
		     &pwk[1], &rpar[1], &ipar[1]);
  ++iwm[12];
  if (*ires >= 0)
    {
  ...
  if (kprin >= 2)
    {
      s_copy (msg, "------ LAMBDA = (R1)", 80L, 20L);
      nsp_ode_xerrwd (msg, &c__20, &c__924, &c__0, &c__0, &c__0, &c__0,
			 &c__1, &rl, &c_b38, 80L);
      s_copy (msg, "------ NORM(F1) = (R1),  NORM(F1NEW) = (R2)", 80L, 43L);
      nsp_ode_xerrwd (msg, &c__43, &c__925, &c__0, &c__0, &c__0, &c__0,
			 &c__2, &f1nrm, &f1nrmp, 80L);
    }
  if (f1nrmp > f1nrm + alpha * slpi * rl)
    {
  ...
  *fnrm = fnrmp;
  if (kprin >= 1)
    {
      s_copy (msg, "------ LEAVING ROUTINE DLINSK, FNRM = (R1)", 80L, 42L);
      nsp_ode_xerrwd (msg, &c__42, &c__926, &c__0, &c__0, &c__0, &c__0,
			 &c__1, fnrm, &c_b38, 80L);
    }
  return 0;
  /*----------------------------------------------------------------------- 
		   ...
		   double *rpar, int *ipar)
{
   ...
   *----------------------------------------------------------------------- 
   */
  C2F (dcopy) (neq, &savr[1], &c__1, &r__[1], &c__1);
  C2F (dscal) (neq, rsqrtn, &wt[1], &c__1);
  *ier = 0;
  (*psol) (neq, t, &y[1], &yprime[1], &savr[1], &pwk[1], cj, &wt[1], &wp[1],
	   &iwp[1], &r__[1], eplin, ier, &rpar[1], &ipar[1]);
  C2F (dscal) (neq, sqrtn, &wt[1], &c__1);
  if (*ier != 0)
    {
      return 0;
    }
		  ...
		  int *jflg, int *kp1, int *nonneg, int *ntype, int *iernls)
{
 ...
 L300:
  ...
  if (*jcalc == -1)
    {
      ...
      *jcalc = 0;
      (*jack) ( res, &ires, neq, x, &y[1], &yprime[1], &wt[1],
		&delta[1], &e[1], h__, cj, &wm[lwp], &iwm[liwp], &ierpj,
		&rpar[1], &ipar[1]);
      *cjold = *cj;
      *s = 100.;
      if (ires < 0)
   ...
   * 
   */
  nsp_ode_dnsk (x, &y[1], &yprime[1], neq,  res,  psol, &wt[1],
		   &rpar[1], &ipar[1], &savr[1], &delta[1], &e[1], &wm[1],
		   &iwm[1], cj, sqrtn, rsqrtn, &eplin, epcon, s, &temp1,
		   &tolnew, &muldel, &maxit, &ires, &iersl, &iernew);
  /* 
   */
  if (iernew > 0 && *jcalc != 0)
    {
		 ...
		 int *maxit, int *ires, int *iersl, int *iernew)
{
 ...
 L300:
   ...
   */
  nsp_ode_dslvk (neq, &y[1], x, &yprime[1], &savr[1], &delta[1], &wt[1],
		    &wm[1], &iwm[1],  res, ires,  psol, iersl, cj,
		    eplin, sqrtn, rsqrtn, &rhok, &rpar[1], &ipar[1]);
  if (*ires != 0 || *iersl != 0)
    {
      goto L380;
    }
		  ...
		  double *rpar, int *ipar)
{
  ...
  *ires = 0;
  ...
  lz = ldl + *neq;
  C2F (dscal) (neq, rsqrtn, &ewt[1], &c__1);
  C2F (dcopy) (neq, &x[1], &c__1, &wm[lr], &c__1);
  i__1 = *neq;
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      /* L110: */
 ...
 L115:
  ...
  if (nrsts > 0)
    {
      C2F (dcopy) (neq, &wm[ldl], &c__1, &wm[lr], &c__1);
    }
  nsp_ode_dspigm (neq, tn, &y[1], &yprime[1], &savr[1], &wm[lr], &ewt[1],
		     &maxl, &maxlp1, &kmp, eplin, cj,  res, ires, &nres,
		     psol, &npsl, &wm[lz], &wm[lv], &wm[lhes], &wm[lq],
		     &lgmr, &wm[lwp], &iwm[liwp], &wm[lwk], &wm[ldl], rhok,
		     &iflag, &irst, &nrsts, &rpar[1], &ipar[1]);
  nli += lgmr;
  nps += npsl;
  nre += nres;
  i__1 = *neq;
  ...
  iwm[16] = ncfl;
  iwm[12] = nre;
  C2F (dscal) (neq, sqrtn, &ewt[1], &c__1);
  return 0;
}
		   ...
		   int *irst, int *nrsts, double *rpar, int *ipar)
{
  ...
  *nre = 0;
   ...
   *----------------------------------------------------------------------- 
   */
  rnrm = C2F (dnrm2) (neq, &v[v_offset], &c__1);
  if (rnrm <= *eplin)
    {
      *rhok = rnrm;
      return 0;
  ...
  tem = 1. / rnrm;
  C2F (dscal) (neq, &tem, &v[v_dim1 + 1], &c__1);
  /*----------------------------------------------------------------------- 
  ...
  for (ll = 1; ll <= i__1; ++ll)
    {
      *lgmr = ll;
       ...
       *----------------------------------------------------------------------- 
       */
      nsp_ode_datv (neq, &y[1], tn, &yprime[1], &savr[1],
		       &v[ll * v_dim1 + 1], &wght[1], &z__[1],  res,
		       ires,  psol, &v[(ll + 1) * v_dim1 + 1], &wk[1],
		       &wp[1], &iwp[1], cj, eplin, &ier, nre, npsl, &rpar[1],
		       &ipar[1]);
      if (*ires < 0)
	{
	  return 0;
      ...
      if (ier != 0)
	{
      ...
      nsp_ode_dorth (&v[(ll + 1) * v_dim1 + 1], &v[v_offset],
			&hes[hes_offset], neq, &ll, maxlp1, kmp, &snormw);
      hes[ll + 1 + ll * hes_dim1] = snormw;
      nsp_ode_dheqr (&hes[hes_offset], maxlp1, &ll, &q[1], &info, &ll);
      if (info == ll)
	{
	  goto L120;
	}
      ...
      if (ll > *kmp && *kmp < *maxl)
	{
	  if (ll == *kmp + 1)
	    {
	      C2F (dcopy) (neq, &v[v_dim1 + 1], &c__1, &dl[1], &c__1);
	      i__2 = *kmp;
	      for (i__ = 1; i__ <= i__2; ++i__)
		{
		  ip1 = i__ + 1;
      ...
      if (ll == *maxl)
	{
       ...
       */
      tem = 1. / snormw;
      C2F (dscal) (neq, &tem, &v[(ll + 1) * v_dim1 + 1], &c__1);
      /* L90: */
    }
 L100:
  if (rho < rnrm)
 ...
 L200:
  ...
  r__[1] = rnrm;
  nsp_ode_dhels (&hes[hes_offset], maxlp1, &ll, &q[1], &r__[1]);
  i__1 = *neq;
  for (k = 1; k <= i__1; ++k)
    {
      /* L220: */
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      C2F (daxpy) (neq, &r__[i__], &v[i__ * v_dim1 + 1], &c__1, &z__[1],
		   &c__1);
      /* L230: */
    }
  i__1 = *neq;
...
int
nsp_ode_dorth (double *vnew, double *v, double *hes, int *n, int *ll,
		  int *ldhes, int *kmp, double *snormw)
{
  /* System generated locals */
  ...
  for (i__ = i0; i__ <= i__1; ++i__)
    {
      hes[i__ + *ll * hes_dim1] =
	C2F (ddot) (n, &v[i__ * v_dim1 + 1], &c__1, &vnew[1], &c__1);
      tem = -hes[i__ + *ll * hes_dim1];
      C2F (daxpy) (n, &tem, &v[i__ * v_dim1 + 1], &c__1, &vnew[1], &c__1);
      /* L10: */
    }
  /*----------------------------------------------------------------------- 
   ...
   *----------------------------------------------------------------------- 
   */
  *snormw = C2F (dnrm2) (n, &vnew[1], &c__1);
  ...
  for (i__ = i0; i__ <= i__1; ++i__)
    {
      tem = -C2F (ddot) (n, &v[i__ * v_dim1 + 1], &c__1, &vnew[1], &c__1);
...
int
nsp_ode_dheqr (double *a, int *lda, int *n, double *q, int *info,
		  int *ijob)
{
  /* System generated locals */
...
int nsp_ode_dhels (double *a, int *lda, int *n, double *q, double *b)
{
  /* System generated locals */
  int a_dim1, a_offset, i__1, i__2;
