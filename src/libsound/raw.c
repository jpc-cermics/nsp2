...

{
  ...
  switch(ft->info.size) 
	...
	case UNSIGNED:
	  ...
	  return done;
	case ULAW:
	  Sciprintf("No U-Law support for shorts\n");
	  ft->ierr=1;
	  return done;
	case ALAW:
	  Sciprintf("No A-Law support for shorts\n");
	  ft->ierr=1;
	  return done;
	}
    case FLOAT:
      while(done < nsamp) 
	{
	  ...
	  *buf++ = datum =  LEFT(datum, 16);
      ...
      return done;
    default:
      Sciprintf("Drop through in rawread!\n");
      ft->ierr=1;
    }
  Sciprintf("Sorry, don't have code to read %s, %s\n",
...
void rawwrite(ft_t ft, long int *buf, long int nsamp)
{
  ...
  switch(ft->info.size) 
	...
	wshort(ft, datum);
      ...
      return;
    case ULAW:
      Sciprintf("No U-Law support for shorts (try -b option ?)\n");
      ft->ierr=1;
      return;
    case ALAW:
      Sciprintf("No A-Law support for shorts (try -b option ?)\n");
      ft->ierr=1;
      return;
    }
    case FLOAT:
    ...
    default: 
      {
	Sciprintf("Drop through in rawwrite!\n");
	ft->ierr=1;
      }
      
    }
  Sciprintf("Sorry, don't have code to write %s, %s\n",
	   styles[ft->info.style], sizes[ft->info.size]);
  ft->ierr=1;
}
