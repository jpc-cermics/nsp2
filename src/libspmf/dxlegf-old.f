...
c$$$      IF(NUDIFF.LT.0) GO TO 400
c$$$      IF(DNU1.LT.-.5D0) GO TO 400
c$$$      IF(MU2.LT.MU1) GO TO 400
c$$$      IF(MU1.LT.0) GO TO 400
c$$$*      IF(THETA.LE.0.D0.OR.THETA.GT.PI2) GO TO 420
c$$$*      IF(X.LT.0.D0.OR.X.GE.1.d0) GO TO 420
c$$$      IF(ID.LT.1.OR.ID.GT.4) GO TO 400
c$$$      IF((MU1.NE.MU2).AND.(NUDIFF.GT.0)) GO TO 400
C
         ...
         if ( MU1 .eq. 0 ) then
            if ( MU2 .eq. 0 ) then ! variable NU
               if ( id .eq. 3 ) then
                  ...
                  do I=1,L
                     PQA(I) = sqrt( 0.5*(2.d0*(DNU1+I-1)+1.0) )
                  enddo
               endif
            else  ! variable MU with MU1 = 0
               if ( id .eq. 3 ) then
                  PQA(1) = 1.0
               else  ! id = 4
                  PQA(1) = sqrt( 0.5*(2.d0*DNU1+1.0) )
               endif
            endif
         endif 
         RETURN
