...
*----------------------------------------------------------------------|
*
*     a few modifs by Bruno Pincon <bruno.pincon@iecn.u-nancy.fr> for
*     inclusion in nsp :
*      (i)  return identity matrix if the matrix H is nul
*      (ii) all errors are returned through iflag and stop statement
*           have been removed. 
*
...
*---  check restrictions on input parameters ...
      ...
      if ( iflag.ne.0 ) return ! stop 'bad sizes (in input of DGPADM)'
*
*---  initialise pointers ...
*
      icoef = 1
...
***      if ( hnorm.eq.0.0d0 ) stop 'Error - null H in input of DGPADM.'
      if ( hnorm.eq.0.0d0 ) then  ! return identity matrix
         iexph = 1
         do j = 1,mm
            wsp(j) = 0.d0
         enddo
...
*
     ...
     .                H,ldh, 0.0d0,wsp(ifree),m )
      ...
      if ( iflag.ne.0 ) return ! stop 'Problem in DGESV (within DGPADM)'
