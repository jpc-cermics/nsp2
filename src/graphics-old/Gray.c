...
static void GraySquare (int *x,int *y,double *z,int n1,int n2);
static void GraySquare1 (int *x,int *y,double *z, int n1,int n2);
...
int C2F(xgray)(double *x, double *y, double *z, int *n1, int *n2, char *strflag, double *brect, int *aaint, long int l1)
{
  int N = Max((*n1),(*n2));
  double xx[2],yy[2];
  ...
  if (nsp_gengine1.get_driver()=='R') store_Gray(x,y,z,n1,n2,strflag,brect,aaint);
...
static void GraySquare_base(int *x, int *y, double *z, int n1, int n2)
{
  ...
  for (i = 0 ; i < (n1)-1 ; i++)
	...
	nsp_gengine->xset_pattern(fill[0]);
	...
	if ( w != 0 && h != 0 && x[i] < xz[0] && y[j+1] < xz[1] && x[i]+w > 0 && y[j+1]+h > 0 )
	  {
		...
		nsp_gengine->fillrectangle(rect);
	      }
	    else 
	      {
...
static void GraySquare(int *x, int *y, double *z, int n1, int n2)
{
  if ( nsp_gengine1.get_driver_id() == 0 ) 
    ...
    nsp_gengine->fill_grid_rectangles(x, y, z, n1, n2);
  else 
...
int C2F(xgray1)(double *z, int *n1, int *n2, char *strflag, double *brect, int *aaint, long int l1)
{
  int N = Max((*n1+1),(*n2+1));
  double xx[2],yy[2];
  ...
  /** If Record is on **/
  if (nsp_gengine1.get_driver()=='R') store_Gray1(z,n1,n2,strflag,brect,aaint);
...
int C2F(xgray2)(double *z, int *n1, int *n2, double *xrect)
{
  ...
  int *xm,*ym,  j;
  /** If Record is on **/
  if (nsp_gengine1.get_driver()=='R') store_Gray2(z,n1,n2,xrect);
  xx[0]=xrect[0];xx[1]=xrect[2];
  yy[0]=xrect[1];yy[1]=xrect[3];
  /** Boundaries of the frame **/
  scale_f2i(xx,yy,xx1,yy1,2);
...
static void GraySquare1_base(int *x, int *y, double *z, int n1, int n2)
{
...
static void GraySquare1(int *x, int *y, double *z, int n1, int n2)
{
  if ( nsp_gengine1.get_driver_id() == 0 ) 
    /** accelerated version for X11 or Windows **/
    nsp_gengine->fill_grid_rectangles1(x, y, z, n1, n2);
  else 
