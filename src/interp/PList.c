...
static int _nsp_plist_print_asread(PList List, int indent, int pos, int posret);

void nsp_plist_pretty_print(PList L, int indent)
{
#ifdef PRINT_AS_READ
  _nsp_plist_print_asread(L,indent,0,indent);
#else
...
static int _nsp_plist_pretty_print_arg(PList L, int i, int pos, int posret)
{
  ...
  switch (L->type) 
    ...
    case CD_COMMAND :
      ...
      break;
    case LS_COMMAND :
      return pos+NSP_PRINTF1_COLOR(i,p_blue,"ls");
      break;
    case PWD_COMMAND :
      return pos+NSP_PRINTF1_COLOR(i,p_blue,"pwd");
      break;
    case WHAT :
...
static int _nsp_plist_print_asread_args (PList L,int Larity,int indent,int pos,int posret, char *sep);
static int _nsp_plist_print_asread_opname(int type, int indent, int pos);
static int _nsp_plist_print_asread_args(PList List, int Larity, int indent, int pos, int posret, char *sep);
static int _nsp_plist_print_asread_arg(PList L, int i, int pos, int posret);
static int _nsp_plist_equalop_mlhs_length(PList L);
static int _nsp_plist_print_asread_arg_ret(PList L, int i, int pos, int posret, int *ret);

/**
 * nsp_plist_print_asread:
 * @L: 
 * @indent: the curent indentation to use.
 * 
 * pretty printing of a #PList. 
...
void nsp_plist_print_asread(PList L, int indent)
{
  _nsp_plist_print_asread(L,indent,0,indent);
}

#define PRINTTAG_ASREAD(tag)							\
  if (pos != posret ) {							\
    Sciprintf("\n");newpos = NSP_PRINTF1_COLOR(posret,p_blue,tag) ;}	\
  else { newpos = pos+ NSP_PRINTF_COLOR(p_blue,tag) ; }
...
static int _nsp_plist_print_asread(PList List, int indent, int pos, int posret)
{
  ...
  if ( L->type > 0 ) 
	...
	case 0:
	  /* : can be a 0 arity operator **/
	  return _nsp_plist_print_asread_opname(L->type,indent,pos);
	  break;
	case 1:
	  switch ( L->type ) 
	    {
	    ...
	    case  COMMA_RET_OP : 
	    case  SEMICOLON_RET_OP  :
	      newpos =_nsp_plist_print_asread_arg(List,indent,pos,posret);
	      newpos =_nsp_plist_print_asread_opname(L->type,0,newpos);
	      Sciprintf("\n");
	      return 0;
	      break;
	    case QUOTE_OP : 
	    case DOTPRIM:
	      ...
	      break;
	    case RETURN_OP : 
	      _nsp_plist_print_asread_arg(List,indent,pos,posret);
	      Sciprintf("\n");
	      return 0;
	      break;
	    case TILDE_OP : 
	    default:
	      newpos =_nsp_plist_print_asread_opname(L->type,indent,pos);
	      newpos =_nsp_plist_print_asread_arg(List,0,newpos,posret);
	      return newpos;
	    }
	  break;
	case 2:
	  ...
	  if ( newpos > CMAX) 
	    {
	      Sciprintf("\n");
	      newpos =_nsp_plist_print_asread_arg(List->next,posret,0,posret);
	    }
	  else
	    {
	      newpos =_nsp_plist_print_asread_arg(List->next,0,newpos,posret);
	    }
	  return newpos;
	  break;
	default :
	  ...
	  for ( j = 0 ; j <  L->arity ; j++ )
	    {
	      newpos =_nsp_plist_print_asread_arg(List,(j == 0) ? indent : 1,
					   newpos,posret);
	      if ( j != L->arity -1 ) 
		newpos =_nsp_plist_print_asread_opname(L->type,1,newpos);
	      List = List->next;
	    }
	  break;
	}
	...
	case EQUAL_OP:
	  {
	    ...
	    newpos =_nsp_plist_print_asread_arg(List,indent,pos,posret);
	    if ( len > 0)   newpos += Sciprintf("=");
	    newpos =_nsp_plist_print_asread_arg(List->next,0,newpos,newpos);
	  }
	  return newpos;
	  break;
	case MLHS  :
	  ...
	  if ( L->arity > 1) 
	    ...
	    newpos = pos +  Sciprintf1(indent,"");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  if ( L->arity > 1) 
	    newpos += Sciprintf("]");
	  return newpos;
	  break;
	...
	case FEVAL :
	  ...
	  newpos =_nsp_plist_print_asread_arg(List,indent,pos,posret);
	  newpos += Sciprintf("(");
	  newpos = _nsp_plist_print_asread_args(List->next,L->arity-1,0,newpos,newpos,",");
	  newpos += Sciprintf(")");
	  return newpos;
	  break;
	case PLIST :
	  if (L->next == NULLPLIST )
	    newpos=_nsp_plist_print_asread_arg(L,indent,pos,posret);/* XXXXXXX */
	  else
	    newpos = 0;
	  return newpos;
	  break;
	...
	case UNUMBER32 :
	case UNUMBER64 :
	  return _nsp_plist_print_asread_arg(L,indent,pos,posret);
	  break;
	case OBJECT: 
	  return Sciprintf1(indent,"{object}")+ pos;
	  /* ignore */
	...
	case P_MATRIX :
	  newpos = pos + Sciprintf1(indent,"[");
	  newpos =_nsp_plist_print_asread_arg(List,0,newpos,posret+1);
	  newpos += Sciprintf("]");
	  return newpos;
	  break;
	case P_CELL :
	...
	case DIAGCONCAT:
	  ...
	  for ( j = 0 ; j < L->arity ; j++)
		...
		{
		  Sciprintf("\n");
		  newpos =_nsp_plist_print_asread_arg_ret(List,posret,0,posret,&ret);
		}
	      else
		{
		  newpos =_nsp_plist_print_asread_arg_ret(List,0,newpos,posret,&ret);
		}
	      if ( j < L->arity-1 )
		{
		  newpos =_nsp_plist_print_asread_opname(L->type,0,newpos);
	  ...
	  return newpos;
#else
	  newpos =_nsp_plist_print_asread_arg_ret(List,indent,pos,posret,&ret);
	  if ( newpos == 0) 
	    {
	      newpos =_nsp_plist_print_asread_opname(L->type,posret,newpos);
	    }
	  else 
	    {
	      newpos =_nsp_plist_print_asread_opname(L->type,0,newpos);
	  ...
	  if ( newpos > CMAX )
	    {
	      if ( L->type == COLCONCAT ) Sciprintf("...");
	      Sciprintf("\n");
	      newpos =_nsp_plist_print_asread_arg_ret(List->next,posret,0,posret,&ret);
	    }
	  else 
	    {
	      newpos =_nsp_plist_print_asread_arg_ret(List->next,0,newpos,posret,&ret);
	    }
	  return newpos;
#endif 
	  break;
	...
	case CELLDIAGCONCAT:
	  ...
	  break;
	case WHILE:
	  PRINTTAG_ASREAD("while");
	  newpos =_nsp_plist_print_asread_arg(List,1,newpos,posret);
	  newpos += NSP_PRINTF1_COLOR(1,p_blue,"do\n");
	  newpos =_nsp_plist_print_asread_arg(List->next,posret+2,0,posret+2);
	  newpos += NSP_PRINTF1_COLOR(posret,p_blue,"end");
	  return newpos;
	  break;
	case FUNCTION:
	  /* Sciprintf("function arity %d\n",L->arity); */
	  PRINTTAG_ASREAD("function");
	  _nsp_plist_print_asread_arg(List,1,pos,newpos+1);
	  Sciprintf("\n");
	  newpos =_nsp_plist_print_asread_arg(List->next,posret+2,pos,posret+2);
	  if ( newpos != 0)  Sciprintf("\n");
	  if ( L->arity == 3 ) 
	    {
	      newpos =_nsp_plist_print_asread_arg(List->next->next,posret+2,pos,posret+2);
	    }
	  return NSP_PRINTF1_COLOR(posret,p_blue,"endfunction");
	  break;
	case FOR:
	  ...
	  newpos =_nsp_plist_print_asread_arg(List,1,newpos,posret);
	  newpos += Sciprintf("=") ;
	  newpos =_nsp_plist_print_asread_arg(List->next,0,newpos,newpos+1);
	  newpos += Sciprintf(" ");
	  PRINTTAG_ASREAD("do");
	  newpos += Sciprintf("\n");
	  newpos =_nsp_plist_print_asread_arg(List->next->next,posret+2,0,posret+2);
	  if ( newpos != 0)  Sciprintf("\n");
	  return NSP_PRINTF1_COLOR(posret,p_blue,"end");
	  break;
	case IF :
	  ...
	  for ( j = 0 ; j < L->arity  ; j += 2 )
	      ...
	      if ( j == L->arity-1 ) 
		{
		  ...
		  if ( newpos != 0) Sciprintf("\n");
		  NSP_PRINTF1_COLOR(posret,p_blue,"else\n");
		  newpos =_nsp_plist_print_asread_arg(List,posret+2,0,posret+2);
		}
	      else 
		{ 
		  if ( j != 0) 
		  ...
		  newpos =_nsp_plist_print_asread_arg(List,1,newpos+1,newpos+1);
		  NSP_PRINTF1_COLOR(1,p_blue,"then\n");
		  List = List->next ;
		  newpos =_nsp_plist_print_asread_arg(List,posret+2,0,posret+2);
		  List = List->next ;
	...
	case SELECT :
	   ...
	   */
	  if (0) Sciprintf1(indent,"select{%d,%d,%d\n",indent,pos,posret) ;
	  PRINTTAG_ASREAD("select");
	  for ( j = 0 ; j < L->arity ; j++)
	    {
	      if ( j==0) 
		{
		  _nsp_plist_print_asread_arg(List,1,newpos,posret);
		  Sciprintf("\n");
		  newpos = 0;
		}
	      else
		{
		  newpos=_nsp_plist_print_asread_arg(List,posret+2,newpos,posret+2);
		}
	      List = List->next;
	    }
	  if ( newpos != 0) Sciprintf("\n");
	...
	case STATEMENTS :
	  if (0) Sciprintf1(indent,"st{%d,%d,%d\n",indent,pos,posret) ;
	  newpos = pos +  Sciprintf1(indent,"");
	  newpos= _nsp_plist_print_asread_args(List,L->arity,0,newpos,posret,"");
	  if (0) Sciprintf1(indent,"}\n");
	  return newpos;
	  break;
	case STATEMENTS1 :
	...
	case CASE :
	  ...
	  if ( List->type == COMMENT )
	    {
	      newpos =_nsp_plist_print_asread_arg(List,posret,0,posret);
	    }
	  else
	    {
	      newpos = NSP_PRINTF1_COLOR(posret,p_blue,"case") ;
	      newpos =_nsp_plist_print_asread_arg(List,1,newpos,newpos+1);
	      newpos += NSP_PRINTF1_COLOR(1,p_blue,"then\n") ;
	      newpos =_nsp_plist_print_asread_arg(List->next,posret+2,0,posret+2);
	    }
	  return newpos;
	...
	case GLOBAL:
	  /* n-ary global */
	  PRINTTAG_ASREAD("global");Sciprintf(" ");
	  /* newpos = NSP_PRINTF1_COLOR(posret,p_blue,"global ") ; */
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case CLEAR:
	  /* n-ary global */
	  PRINTTAG_ASREAD("clear");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case CLEARGLOBAL:
	  /* n-ary global */
	  PRINTTAG_ASREAD("clearglobal");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case PAUSE:
	  /* 1-ary pause */
	  PRINTTAG_ASREAD("pause");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case HELP:
	  /* 1-ary help */
	  PRINTTAG_ASREAD("help");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case WHO:
	  /* 1-ary who */
	  PRINTTAG_ASREAD("who");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case EXEC:
	  /* 1-ary exec */
	  PRINTTAG_ASREAD("exec");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case APROPOS:
	  /* 1-ary apropos */
	  PRINTTAG_ASREAD("apropos");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case CD_COMMAND:
	  /* 1-ary cd */
	  PRINTTAG_ASREAD("cd");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case LS_COMMAND:
	  /* 1-ary ls */
	  PRINTTAG_ASREAD("ls");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	case PWD_COMMAND:
	  /* 1-ary pwd */
	  PRINTTAG_ASREAD("pwd");Sciprintf(" ");
	  newpos = _nsp_plist_print_asread_args(List,L->arity,0,newpos,newpos,",");
	  return newpos;
	  break;
	default:
...
static int _nsp_plist_print_asread_args(PList List, int Larity, int indent, int pos, int posret, char *sep)
{
  ...
  for ( j = 0 ; j < Larity ; j++)
    {
      newpos =_nsp_plist_print_asread_arg(List,indent1,newpos,posret);
      if ( j != Larity -1 ) newpos += Sciprintf(sep);
      /* reset indent for next argument of necessary **/
      if ( indent1 != indent ) indent1 = indent; 
      /* newpos==0 if the previous ArgPrettyPrint has inserted a \n **/
...
static int _nsp_plist_print_asread_arg(PList L, int i, int pos, int posret)
{
  ...
  switch (L->type) 
    ...
    case BREAK:
      return pos+NSP_PRINTF1_COLOR(i,p_blue,"break");break;
    case PLIST :
      return _nsp_plist_print_asread((PList) L->O,i,pos,posret);
      break;
    case PRETURN: 
      return pos+NSP_PRINTF1_COLOR(i,p_blue,"return");
      break;
...
/* similar to _nsp_plist_print_asread_arg_ 
 * but add a newline if ar is a comment 
 */
static int _nsp_plist_print_asread_arg_ret(PList L, int i, int pos, int posret, int *ret)
{
  int newpos= _nsp_plist_print_asread_arg(L,i,pos,posret);
  if ( L != NULLPLIST  &&  L->type == COMMENT )
    {
      Sciprintf("\n");
      newpos = 0;
