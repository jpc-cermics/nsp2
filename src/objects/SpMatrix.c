...
{ 	
 ...
 nsp_mat_set_rval(A,0.0);
  if ( A->rc_type == 'i') if (nsp_mat_set_ival(A,0.00) == FAIL ) return(NULLMAT);
  for ( i = 0 ; i < Sp->m ; i++ ) 
    {
      SpRow *Ri = Sp->D[i] ;
      switch ( A->rc_type ) 
...
{ 
  ...
  *flag = 0;
  if ( B->rc_type == 'i' || A->rc_type == 'i' ) type = 'i';
