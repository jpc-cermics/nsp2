...
typedef NspSpRowMatrix *(*SuPro) (NspSpRowMatrix *A, int dim);
...
static int int_sprowmatrix__sum(Stack stack, int rhs, int opt, int lhs, SuPro F)
{
  ...
  if ((Res= (*F)(HMat,dim)) == NULLSPROW ) return RET_BUG;
  MoveObj(stack,1,(NspObject *) Res);
  return 1;
}
...
typedef NspSpRowMatrix *(*SpMiMax) (NspSpRowMatrix *A,int dim,NspMatrix **Imax,int lhs);
...
static int int_sprowmatrix__maxi(Stack stack, int rhs, int opt, int lhs, SpMiMax F,int minmax)
{
  ...
  if ( rhs == 1 || (rhs - opt ) == 1 || ( rhs == 2 && IsSMatObj(stack,2)  ))
      ...
      if (rhs == 2 )
	{
      ...
      if (( M= (*F)(A,dim,&Imax,lhs)) == NULLSPROW) return RET_BUG;
...
static int int_sprowmatrix_find(Stack stack, int rhs, int opt, int lhs)
{
  ...
  CheckLhs(1,2);
  if ((A = GetSpRow(stack,1)) == NULLSPROW)  return RET_BUG;
  if (nsp_sprowmatrix_find(A,Max(lhs,1),&Rr,&Rc,&V) == FAIL) return RET_BUG;
...
static OpTab SpRowMatrix_func[]={
  ...
  {"reshape_sprow",int_sprowmatrix_redim},
