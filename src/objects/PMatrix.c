...
int nsp_pmatrix_latex_print(NspPMatrix *Mat,  int use_math, const char *name, int rec_level)
{
  int i,j, fw;
  nsp_num_formats fmt;
  const char *pname = (name != NULL) ? name : NSP_OBJECT(Mat)->name;
  ...
  for ( i=0 ; i < m->mn ; i++) 
      ...
      else
	{
	  if ( ( m->C[i].r != 0.00 || m->C[i].i != 0.00)  || m->mn == 1  )
	    {
	      int negative_real = (  m->C[i].r < 0 &&  m->C[i].i == 0.00);
	      if ( leading == FALSE && negative_real == FALSE) 
		{
		  if (do_print) Sciprintf("+");
		  count++;
		}
      ...
      if ( m->rc_type == 'r') 
	{
	  if (  m->R[i] != 0.00 || m->mn == 1  )
	      ...
	      if ( leading == FALSE && m->R[i] >= 0.00 ) 
		{
	      ...
	      if ( m->R[i] > 0 )
		{
		  nsp_pr_any_float_latex(fmt->curr_real_fmt, m->R[i], (i==0) ? FALSE: TRUE);
		}
	      else
		{
		  Sciprintf("-");
