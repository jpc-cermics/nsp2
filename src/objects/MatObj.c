 ...
 * Copyright (C) 2005-2019 Bruno Pinçon Esial/Iecn
 *
...
new_type_matrix (type_mode mode)
{
  ...
  top->convert = (convert_func *) nsp_matrix_convert;
...
static int matrix_xdr_save(XDR *xdrs, NspMatrix * M)
{
  ...
  if (nsp_xdr_save_string(xdrs,type_get_name(nsp_type_matrix)) == FAIL) return FAIL;
#else
...
#define NSP_MATRIX_INLINED 
#include "nsp/matrix-inlined.h"
...
int int_mx2latextab (Stack stack, int rhs, int opt, int lhs)
{
  ...
  if ((HMat = GetMat (stack, 1)) == NULLMAT)
    return RET_BUG;
  nsp_matrix_latex_tab_print (HMat, 0, NULL,0);
  return 0;
}
...
int int_mxfind (Stack stack, int rhs, int opt, int lhs)
{
  ...
  if ( nsp_mat_find (A, Max (lhs, 1), &Rr, &Rc, itype) == FAIL )
    return RET_BUG;

  if ( lhs >= 3 )
    {
      int i, *indr, *indc;
      if ((val = nsp_matrix_create(NVOID,A->rc_type, ((NspMatrix *)Rc)->m,((NspMatrix *)Rc)->n))== NULLMAT)
	{
	...
	for ( i = 0 ; i < ((NspMatrix *)Rr)->mn; i++)
	  val->R[i]= A->R[indr[i]-1 + A->m*(indc[i]-1)];
      else
	for ( i = 0 ; i < ((NspMatrix *)Rr)->mn; i++)
	  val->C[i]= A->C[indr[i]-1 + A->m*(indc[i]-1)];
    }
...
static int int_mxmfind (Stack stack, int rhs, int opt, int lhs)
{
  ...
  if ( nsp_mat_mfind(x, m, ops, scalars, ind, itype) == FAIL )
    goto err;
  
  for ( j = 0 ; j <= m ; j++ )
    {
      if ( itype == 'd' ) /* back convert */
	ind[j] = (NspObject *) Mat2double( (NspMatrix *) ind[j]);
