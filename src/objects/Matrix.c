...
NspMatrix *nsp_matrix_create_logspace(const double first[],const double last[],int r,int n)
{
  ...
  if ( n == 1 )
    for ( i = 0 ; i < r ; i++)
      Loc->R[i] = pow(10.0,last[i]);  /* just as in Scilab ! */
  else
    for ( j=0, k=0 ; j < n ; j++)
      {
	double inc = (double) j/ (double) (n-1);
	for ( i = 0 ; i < r ; i++, k++)
	  Loc->R[k]= pow(10.0,(last[i]-first[i])*inc + first[i]);
      }
  return Loc;
}
	...
	for ( i=init; i < last ; i++) 
	  {
	    count++;
	    Sciprintf1(indent+1,"");
	    nsp_pr_float(&fmt,Mat->R[i], FALSE);
	    if ( i != last-1)
	      {
		Sciprintf(";");
		if ( count == 10 )
	...
	for ( i=init; i < last; i++) 
	  {
	    count++;
	    Sciprintf1(indent+1,"");
	    nsp_pr_complex (&fmt,Mat->C[i], FALSE);
	    if ( i != last-1)
	      {
		Sciprintf(";");
		if ( count == 10 )
...
int nsp_matrix_latex_tab_print(NspMatrix *Mat, int indent,const char *name, int rec_level)
{
  ...
  for (i=0; i < Mat->m; i++)
      ...
      for (j=0; j < Mat->n - 1; j++)
	{
	  Sciprintf("$");
	  if ( Mat->rc_type == 'r' ) 
	    nsp_pr_float(&fmt,Mat->R[i+j*Mat->m], TRUE);
	  else
	    nsp_pr_complex(&fmt,Mat->C[i+j*Mat->m], TRUE);
	  Sciprintf("$ & ");
	}
      Sciprintf("$");
      if ( Mat->rc_type == 'r' ) 
	nsp_pr_float(&fmt,Mat->R[i+(Mat->n-1)*Mat->m], TRUE);
      else
	nsp_pr_complex(&fmt,Mat->C[i+(Mat->n-1)*Mat->m], TRUE);
      Sciprintf(" \\\\ \\hline\n");
    }
...
static void Mij_float(const nsp_num_formats *fmt,const void *m, int i, int j)
{
  const NspMatrix *M=m;
  Sciprintf("  ");
  nsp_pr_float (fmt,M->R[i+(M->m)*j], FALSE);
}
...
static void CMij_float(const nsp_num_formats *fmt,const void *m, int i, int j)
{
  const NspMatrix *M=m;
  Sciprintf("  ");
  nsp_pr_complex (fmt, M->C[i+(M->m)*j], FALSE);
}
