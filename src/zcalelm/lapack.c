...
{
  if (A->rc_type == 'r' ) 
    return  intdgeqrpf(A,q,r,E,rank,tol,'x');
  else 
    return  intzgeqrpf(A,q,r,E,rank,tol,'x');
}
		      ...
		      NspMatrix **rank,double *tol,char flag)
{
  ...
  if (rank != NULL ) {
    ...
    double Tol;
    /* XXXX Attention eps est a controler ca doit etre le eps machine */
    double eps = 1.e-16;
    if (tol == NULL ) 
		      ...
		      NspMatrix **rank,double *tol,char flag)
{
  ...
  if (m > n) {
    ...
    int j,ix,ij;
    /* Q = [ R,0 ] */ 
    C2F(zlacpy)("F", &m, &n,(*R)->I, &m,(*Q)->I, &m, 1L);
    if ( flag != 'e' ) {
      for (j = n + 1; j <= m ; ++j) {
	for (ix = 1; ix <= m ; ++ix) {
	  ij = ix + (j - 1) * m;
    ...
    /* Q = R(1:m,1:m) */ 
    C2F(zlacpy)("F", &m, &m, (*R)->I, &m, (*Q)->I, &m, 1L);
  }
...
NspMatrix *lsq(NspMatrix *A,NspMatrix *B,char flag)
{
...
int intdgesvd(NspMatrix *A,NspMatrix **S,NspMatrix **U,NspMatrix **V,char *flag,NspMatrix **Rank,double *tol)
{
  ...
  if ( U != NULL ) {
    ...
    int nU  = ( flag[0]== 'S') ? Minmn : m;
    int mVt = ( flag[0]== 'S') ? Minmn : n;
  ...
  ix1 = Minmn * 3 + Max(m,n), ix2 = Minmn * 5 - 4;
...
int intzgesvd(NspMatrix *A,NspMatrix **S,NspMatrix **U,NspMatrix **V,char *flag,NspMatrix **Rank,double *tol)
{
 ...
 nsp_mat_abs(SV); 
	...
	{
	  /* compute the singular values and Min(m,n) cols of u and rows of vt */ 
	  C2F(zgesvd)("S","S", &m, &n,A->I, &m,SV->R,(*U)->I,&m,Vt->I,&Vt->m,
		      dwork->I, &lworkMin,rwork->R, &info, 1L, 1L);
	}
      else 
	{
	  /* compute the singular values u and vt */ 
	  C2F(zgesvd)("A","A", &m, &n,A->I, &m,SV->R,(*U)->I, &m,Vt->I, &n,
		      dwork->I, &lworkMin,rwork->R, &info, 1L, 1L);
	}
    }
  if (info != 0) {
...
int intddet(NspMatrix *A,NspMatrix **det,NspMatrix **det_em) 
{
  int info,ix;
  NspMatrix *ipiv;
  int m = A->m,n=A->n; 
  ...
  if ( A->mn == 0 ) {
    if (( *det =nsp_matrix_create(NVOID,A->rc_type,m,n)) == NULLMAT) return FAIL;
    return OK ; 
  }
  ...
  /* int matrix XXX */ 
  if (( ipiv =nsp_matrix_create(NVOID,'r',1,m)) == NULLMAT) return FAIL;
  Ipiv = (int *) ipiv->R  ;
  ...
  if (det != NULL ) {    /* det(A) */
    ...
    if (( *det =nsp_matrix_create(NVOID,A->rc_type,1,1)) == NULLMAT) return FAIL;
    (*det)->R[0] = dx ;
  } else {    /*  [e,m]=det(A) */
    ...
    if (( *det_em =nsp_matrix_create(NVOID,A->rc_type,1,2)) == NULLMAT) return FAIL;
    (*det_em)->R[0]= e; 
    (*det_em)->R[1]= m; 
  } 
  return OK ;
...
int intzdet(NspMatrix *A,NspMatrix **det,NspMatrix **det_em) 
{
  NspMatrix *ipiv ; 
  int info,ix;
  int m = A->m,n=A->n; 
  ...
  if (det != NULL ) {    /* det(A) */
    ...
    if (( *det_em =nsp_matrix_create(NVOID,A->rc_type,1,2)) == NULLMAT) return FAIL;
    (*det_em)->I[0].r  = e;    (*det_em)->I[0].i  = 0.0;
    (*det_em)->I[1] = dx; 
  }
  return OK;
} 
...
int idl_spec(NspMatrix *A, NspMatrix **d) {
  return  intdgeev(A,d,NULL); 
}


int idl_spec2(NspMatrix *A, NspMatrix **d,NspMatrix **v) {
...
static int intdgeev(NspMatrix *A,NspMatrix **d,NspMatrix **v) 
{
  ...
  if ( v == NULL ) 
	...
	for (i = 0 ; i < n ; ++i) { (*d)->I[i].r = wr->R[i]; (*d)->I[i].i = wi->R[i]; }
      }
  else 
    {
	...
	{
	  C2F(zlaset)("F", &n, &n, &zero, &zero,(*d)->I, &n, 1L);
	  for ( i = 0;  i < n ; ++i) 
...
static int intzgeev(NspMatrix *A,NspMatrix **d,NspMatrix **v) 
{
  ...
  if (v == NULL ) 
  ...
  else 
    {
      ...
      C2F(zgeev)("N","V", &n,A->I, &n,(*d)->I,NULL,&n,(*v)->I,&n,dwork->I,&lworkMin,rwork->R,
		 &info, 1L, 1L);
    }
  ...
  for (i = 0 ; i < n ; ++i) { if ((*d)->I[i].i != 0.0) { type = 'i'; break;}}
  ...
  if ( v == NULL ) 
      ...
      if ( type == 'r' ) 
	{
	  ...
	  for ( i = 0;  i < n ; ++i) { md->R[i*(m+1)] =  (*d)->I[i].r; }
	} 
      else 
	{
	  C2F(zlaset)("F", &n, &n, &zero, &zero,md->I, &n, 1L);
	  for ( i = 0;  i < n ; ++i)  md->I[i*(m+1)] =  (*d)->I[i]; 
	}
...
static int intdgecon(NspMatrix *A) 
{
  double rcond=0.0,anorm;
...
static int intzgecon(NspMatrix *A) 
{
  double rcond=0.0,anorm;
  ...
  anorm = C2F(zlange)("1", &n, &n,A->I, &n,NULL, 1L);
  C2F(zgetrf)(&n, &n, A->I, &n, Iwork , &info);
  if (info == 0) {
  ...
  if ( m == n ) 
    {
      ...
      NspMatrix *Ac ;
      if (( Ac =nsp_matrix_copy(A))== NULLMAT) return FAIL;
      anorm =  C2F(zlange)("1", &m, &n,Ac->I, &m, NULL , 1L);
      C2F(zgetrf)(&n, &n, Ac->I , &n, Ijpvt, &info);
      if (info == 0) {
	C2F(zgecon)("1", &n, Ac->I, &n,&anorm, &Rcond, dwork->I, rwork->R,&info, 1L);
	if ( Rcond > sqrt(eps)) {
	  /* we solve the linear systems */
	  C2F(zgetrs)("N", &n, &nrhs, Ac->I, &n, Ijpvt,B->I, &n, &info, 1L);
	  /* XXXXXX clean workspace here */ 
	  return OK;
	}
      }
...
int intzgees0(NspMatrix *A,NspMatrix **U,int (*F)(doubleC *w), NspMatrix **Sdim) 
{
  ...
  if ( U != NULL ) 
    {
      if (( *U =nsp_matrix_create(NVOID,A->rc_type,m,n)) == NULLMAT) return FAIL;   
      C2F(zgees)("V",sort, F, &n,A->I,&n, &sdim,W->I,(*U)->I,
		 &n, dwork->I, &lworkMin,rwork->R, Iwork, &info, 4L, 4L);
    }
  else 
    { 
      C2F(zgees)("N",sort, F, &n,A->I,&n, &sdim,W->I,NULL,
		 &n, dwork->I, &lworkMin,rwork->R, Iwork, &info, 4L, 4L);
    }
  if (info > 0) {
    Scierror("schur: the QR algorithm failed to compute all the eigenvalues;\n");
...
static int intdggev(NspMatrix *A,NspMatrix *B,NspMatrix **Vl,NspMatrix **Vr,NspMatrix **alpha,NspMatrix **beta)
{
  ...
  if ( type == 'r' ) 
    ...
    { 
      ...
      for (i = 0 ; i < n ; ++i) { (*alpha)->I[i].r = alphar->R[i]; (*alpha)->I[i].i = alphai->R[i]; }
    }
...
static int intzggev(NspMatrix *A,NspMatrix *B,NspMatrix **Vl,NspMatrix **Vr,NspMatrix **alpha,NspMatrix **beta)
{
  ...
  C2F(zggev)(jobVl,jobVr,&n,A->I, &n,B->I, &n,(*alpha)->I,(*beta)->I,
	     VlI,&n,VrI, &n,dwork->I,&lworkMin,rwork->R,&info, 1L, 1L);
