...
deff('[y]=Norm(x)','if x==[] then y=0; else y=maxi(abs(x));end');
...
for i=1:3, m=Mde(i);n=Nde(i);
    for j=-(m+1):m+1;
     d=rand(m,n,'uniform');
     b=0*ones_new(mini(m,n),1);for k=1:mini(m,n), b(k)=d(k,k);end
     if test(diag(d),b) then pause,end
     if test(diag(d+[]),b) then pause,end
    end 
end 
...
//limit XXXX : bizarre la definition a encore change
//m=10;
...
for i=1:6, m=M(i);n=N(i);
     ...
     if testN(sum(A,2),MsumG(A,'c'),0.1) then pause,end
     if testN(sum(A+[],'c'),MsumG(A,'c'),0.1) then pause,end     
     if testN(sum(A,'r'),MsumG(A,'r'),0.1) then pause,end
     if testN(sum(A,1),MsumG(A,'r'),0.1) then pause,end
     if testN(sum(A+[],'r'),MsumG(A,'r'),0.1) then pause,end
end 
...
deff('[p]=Mprod(x,j)',['[m,n]=size(x);';
    'if j==''c'' then ;p=0*ones_new(m,1);for i=1:m,p(i)=Vprod(x(i,:));end;';
    'elseif j==''r'' then p=0*ones_new(1,n);for i=1:n,p(i)=Vprod(x(:,i));end;'
    'else p= Mprod(Mprod(x,''r''),''c'');end;']);

deff('[y]=MprodG(x,j)',['if and([x==[],j==''f'']) then y=1;else y=Mprod(x,j);end']);


deff('[vp]=Vprod(x)','m=size(x,''*'');vp=x(1);for i=2:m,vp=vp*x(i);end');
