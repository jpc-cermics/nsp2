...
for i=1:6, m=M(i);n=N(i);
  b=eye(m,n);
  if test(ieye(m,n),m2i(b)) then pause,end
  if test(ieye(size(b)),m2i(b)) then pause,end
end 
...
for i=1:6, m=M(i);n=N(i);
  a=rand(m,n,'uniform');
  a=m2i(a);
  if test(iones(m,n),Mones(m,n)) then pause,end
  if test(iones(size(a)),Mones(m,n)) then pause,end
end 
...
function [a]=Mdiag(d)
  m=size(d,'*');a=izeros(m,m);for j=1:m, a(j,j)=d(j);end
endfunction

for i=1:6, m=M(i);
  d=m2i(1:m);
  b=izeros(m,m);for j=1:m, b(j,j)=d(j);end
  if test(diag(d),b) then pause,end
end 

function [a]=MFdiag(d,j)
  if isempty(d) then 
    a=izeros(max(abs(j)-1,0),max(abs(j)-1,0));
  else 
    a=Mdiag(d);m=size(a,'r');
    if ~isempty(a) then 
      if j>=0; a=[izeros(m+j,j),[a;izeros(j,m)]];
      else a=[izeros(-j,m-j);[a,izeros(m,-j)]];end
    end
  end
endfunction
...
for i=1:3, m=Mde(i);n=Nde(i);
  d=rand(m,n,'uniform');
  mm=min(m,n);
  b=0*ones(mm,min(1,mm));for j=1:min(m,n), b(j)=d(j,j);end
  if test(diag(d),b) then pause,end
end 
// diag(a,j) 
...
for i=1:3, m=Mde(i);n=Nde(i);
  for j=-(m+1):m+1;
    d=rand(m,n,'uniform');
    mm=min(m,n);
    b=0*ones(mm,min(1,mm));for k=1:min(m,n), b(k)=d(k,k);end
    if test(diag(d),b) then pause,end
  end 
end 
...
function [y]=Msum(x,j)
  [m,n]=size(x);
  if j==2, y= x*ones(n,min(1,n));
  elseif j==1,y=ones(min(1,m),m)*x;
...
function [p]=Mprod(x,j)
  [m,n]=size(x);
  if j==2 then ;p=0*ones(m,min(1,m));for i=1:m,p(i)=Vprod(x(i,:));end;
  elseif j==1 then p=0*ones(min(1,n),n);for i=1:n,p(i)=Vprod(x(:,i));end;
...
function [y]=Mcumsum(x,j)
  [m,n]=size(x);
  if j==2, y=0*x; for i=1:n; y(:,i)= x(:,1:i)*ones(i,min(1,i));end;
