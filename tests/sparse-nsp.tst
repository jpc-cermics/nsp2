...
sp=sparse(ij,v,[6,6]);
[ij_1,v_1]=spget(sp);[ij_2,k_2]=gsort(ij(:,[2,1]),'ldr','i'); ij_2 = ij_2(:,[2,1]);
if or([ij_1<>ij_2,v_1<>v(k_2)]) then pause,end

zer=sparse([],[],[6,6]);[ij_t,v_t]=spget(zer);
if ~isempty(ij_t) || ~isempty(v_t) then pause,end

sp=sparse(ij,vc,[8,6]);
[ij_1,v_1,mn1]=spget(sp);[ij_2,k_2]=sort(ij(:,[2,1]),'ldr','i'); ij_2 = ij_2(:,[2,1]);
if ~( ij_1.equal[ij_2] && v_1.equal[vc(k_2)] ) then pause,end
if ~mn1.equal[[8,6]] then pause,end
...
for i=1:5;
	...
	[ij_t,k]=sort(ij_t(:,[2,1]),'ldr','i'); ij_t = ij_t(:,[2,1]);
	vals_t=vals_t(k);
	// using spget 
...
//AFAIREif or(full(a+2*eye)<>full(a)+2*eye) then pause,end
//AFAIREif or(full(2*eye+a)<>full(a)+2*eye) then pause,end
if or(full(a-1)<>full(a)-1) then pause,end
if or(full(1-a)<>1-full(a)) then pause,end
if or(full(a-ones_new(6,6))<>full(a)-ones_new(6,6)) then pause,end
if or(full(ones_new(6,6)-a)<>ones_new(6,6)-full(a)) then pause,end
//AFAIREif or(full(a-2*eye)<>full(a)-2*eye) then pause,end
//AFAIREif or(full(2*eye-a)<>2*eye-full(a)) then pause,end
...
//AFAIREif or(full(ac+2*eye)<>full(ac)+2*eye) then pause,end
//AFAIREif or(full(2*eye+ac)<>full(ac)+2*eye) then pause,end
if or(full(ac-1)<>full(ac)-1) then pause,end
if or(full(1-ac)<>1-full(ac)) then pause,end
if or(full(ac-ones_new(6,6))<>full(ac)-ones_new(6,6)) then pause,end
if or(full(ones_new(6,6)-ac)<>ones_new(6,6)-full(ac)) then pause,end
//AFAIREif or(full(ac-2*eye)<>full(ac)-2*eye) then pause,end
//AFAIREif or(full(2*eye-ac)<>2*eye-full(ac)) then pause,end
...
//AFAIREif or(full(a*2)<>full(a)*2) then pause,end
//AFAIREif or(full(2*a)<>full(a)*2) then pause,end
//AFAIREif  a*[]<>[] then pause,end
//AFAIREif  []*a<>[] then pause,end
//AFAIREc=rand(6,6);
//AFAIREif norm(a*c-full(a)*c) >100*%eps then pause,end
//AFAIREif norm(c*a-c*full(a)) >100*%eps then pause,end
// sparse sparse
...
//AFAIREif  norm(full(a*a)-full(a)*full(a),1)>100*%eps then pause,end
//AFAIREif  norm(full(b*b)-full(b)*full(b),1)>100*%eps then pause,end
//AFAIREif  norm(full(a*b)-full(a)*full(b),1)>100*%eps then pause,end
//AFAIREif  norm(full(b*a)-full(b)*full(a),1)>100*%eps then pause,end
// complex x real real x complex
// sparse scalar , scalar sparse
//AFAIREif or(full(ac*2)<>full(ac)*2) then pause,end
//AFAIREif or(full(2*ac)<>full(ac)*2) then pause,end
//AFAIREif norm(full((2+%i)*a)-(2+%i)*full(a),1) >100*%eps then pause,end
//AFAIREif norm(full(a*(2+%i))-(2+%i)*full(a),1) >100*%eps then pause,end
//AFAIREif  ac*[]<>[] then pause,end
//AFAIREif  []*ac<>[] then pause,end
//AFAIREc=rand(6,6);
//AFAIREcc=c+rand(6,6)*%i;
//AFAIREif norm(ac*c-full(ac)*c) >100*%eps then pause,end
//AFAIREif norm(cc*a-cc*full(a)) >100*%eps then pause,end
// sparse sparse
if or(full(ac*zer)<>full(zer)) then pause,end
if or(full(zer*ac)<>full(zer)) then pause,end
//AFAIREif  norm(full(ac*a)-full(ac)*full(a),1)>100*%eps then pause,end
//AFAIREif  norm(full(bc*b)-full(bc)*full(b),1)>100*%eps then pause,end
//AFAIREif  norm(full(a*bc)-full(a)*full(bc),1)>100*%eps then pause,end
//AFAIREif  norm(full(b*ac)-full(b)*full(ac),1)>100*%eps then pause,end
// // complex x complex
//AFAIREif norm(ac*cc-full(ac)*cc) >100*%eps then pause,end
//AFAIREif norm(cc*ac-cc*full(ac)) >100*%eps then pause,end
// sparse sparse
//AFAIREif  norm(full(ac*ac)-full(ac)*full(ac),1)>100*%eps then pause,end
//AFAIREif  norm(full(bc*bc)-full(bc)*full(bc),1)>100*%eps then pause,end
//AFAIREif  norm(full(bc*ac)-full(bc)*full(ac),1)>100*%eps then pause,end
//----------------------------------------------------------
...
// sparse sparse
//AFAIREif or(full(zer.*zer)<>full(zer)) then pause,end
//AFAIREif or(full(a.*zer)<>full(zer)) then pause,end
//AFAIREif or(full(zer.*a)<>full(zer)) then pause,end
//AFAIREif  norm(full(a.*a)-full(a).*full(a),1)>100*%eps then pause,end
//AFAIREif  norm(full(b.*b)-full(b).*full(b),1)>100*%eps then pause,end
//AFAIREif  norm(full(a.*b)-full(a).*full(b),1)>100*%eps then pause,end
//AFAIREif  norm(full(b.*a)-full(b).*full(a),1)>100*%eps then pause,end
...
//AFAIREc=rand(6,6);
//AFAIREcc=c+rand(6,6)*%i;
//AFAIREif norm(full(ac.*c)-full(ac).*c) >100*%eps then pause,end
//AFAIREif norm(full(cc.*a)-cc.*full(a)) >100*%eps then pause,end
// sparse sparse
//AFAIREif or(full(ac.*zer)<>full(zer)) then pause,end
//AFAIREif or(full(zer.*ac)<>full(zer)) then pause,end
//AFAIREif  norm(full(ac.*a)-full(ac).*full(a),1)>100*%eps then pause,end
//AFAIREif  norm(full(bc.*b)-full(bc).*full(b),1)>100*%eps then pause,end
//AFAIREif  norm(full(a.*bc)-full(a).*full(bc),1)>100*%eps then pause,end
//AFAIREif  norm(full(b.*ac)-full(b).*full(ac),1)>100*%eps then pause,end
...
//AFAIREif or(full(v(:))<>vf(:)) then pause,end
...
//AFAIREif or(full(vc(:))<>vcf(:)) then pause,end
...
//AFAIRE if or(full(a1)<>A) then pause,end
...
//AFAIRE if or(full(v1)<>V) then pause,end
...
//AFAIRE if or(full(v1)<>V) then pause,end
//AFAIRE v1=v;v1(:)=[];
//AFAIRE if or(full(v1)<>[]) then pause,end
...
// Comment next lines because Boolean matrix is not supported by nsp
// $$$ a=sparse([1 6;1 5;1 3;2 4;2 1;4 4;4 3;5 1;6 6],1:9,[6 6]);
