...
\begin{mandesc}
  \shortunder{linear\_interpn}{linear_interp}{n dimensional linear interpolation}
\end{mandesc}
...
\begin{program}\HCode{x = linspace(0,2*}\verb+%pi+\HCode{,11);\Hnewline
y = sin(x);\Hnewline
xx = linspace(-2*}\verb+%pi+\HCode{,4*}\verb+%pi+\HCode{,400)';\Hnewline
...
xx = linspace(0,2*}\verb+%pi+\HCode{, 40);\Hnewline
...
z = 0.4*cos(2*}\verb+%pi+\HCode{*X).*cos(}\verb+%pi+\HCode{*Y);\Hnewline
