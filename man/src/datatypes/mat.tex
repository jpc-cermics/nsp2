...
\item \verb+A.blas_axpy[alpha, x [i1,i2 [,j1,j2]]]+ constant times a vector plus a vector i.e \verb!A = A + alpha*X!  (see \manlink{blas_axpy}{blas_axpy})
...
\item \verb+A.scale_rows[x]+ multiplies or divides each row i of A with a different scalar:
  \verb+A(i,:) = x(i) A(i,:)+.  (see \manlink{scale_rows}{scale_rows}) 
\item \verb+A.scale_cols[x]+ multiplies or divides each column j of A with a different scalar:
  \verb+A(:,j) = x(j) A(:,j)+ (see \manlink{scale_cols}{scale_cols}).
