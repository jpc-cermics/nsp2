...
The function \manlink{size}{size} lets to retrieve the two dimensions
of any nsp array.
...
but directly using \manlink{isvector}{isvector} and \manlink{isscalar}{isscalar}
...
\item An {\bf important feature} is that any matrix could be adressed with
 a one way indexing working like in the fortran langage 
 and called ``column major order'' as illustrated by the
 following figure:    
$$
...
\end{Verbatim}
Conversion between usual indexing and linear indexing could be done 
...
\verb+v($-1)+ correspond to the last but one vector element.
