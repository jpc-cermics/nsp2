...
function []=demo_spinbutton()
  ...
  frame = gtk_frame_new(label="Not accelerated");
  ...
  label = gtk_label_new (str="Day :");
  // label.set_alignment[ 0, 0.5];
  vbox2.pack_start[ label, expand=%f,fill= %t,padding=0];

  adj =  gtk_adjustment_new (value=1.0,lower= 1.0,upper= 31.0,step_incr= 1.0,page_incr= 5.0, page_size=0.0);
  ...
  label = gtk_label_new (str="Month :");
  //label.set_alignment[ 0, 0.5];
  vbox2.pack_start[ label, expand=%f,fill= %t,padding=0];

  adj =  gtk_adjustment_new (value=1.0,lower= 1.0,upper= 12.0,step_incr= 1.0,page_incr=  5.0, page_size=0.0);
  spinner = gtk_spin_button_new (adjustment=adj,climb_rate= 0,digits= 0);
  spinner.set_wrap[%t];
  ...
  label = gtk_label_new (str="Year :");
  //label.set_alignment[ 0, 0.5];
  vbox2.pack_start[ label, expand=%f,fill= %t,padding=0];

  adj =  gtk_adjustment_new (value=1998.0,lower= 0.0,upper= 2100.0,step_incr=  1.0,page_incr= 100.0, page_size=0.0);
  spinner = gtk_spin_button_new (adjustment=adj,climb_rate= 0,digits= 0);
  spinner.set_wrap[%t];
  ...
  frame = gtk_frame_new(label="Accelerated");
  ...
  label = gtk_label_new (str="Value :");
  //label.set_alignment [ 0, 0.5];
  vbox2.pack_start[ label, expand=%f,fill= %t,padding=0];

  adj =  gtk_adjustment_new (value=0.0,lower= -10000.0,upper= 10000.0,step_incr= 0.5,page_incr= 100.0, page_size=0.0);
  spinner1 = gtk_spin_button_new (adjustment=adj,climb_rate= 1,digits= 2);
  spinner1.set_wrap[%t];
  ...
  label = gtk_label_new (str="Digits :");
  //label.set_alignment[ 0, 0.5];
  vbox2.pack_start[ label, expand=%f,fill= %t,padding=0];

  adj =  gtk_adjustment_new (value=2,lower= 1,upper= 5,step_incr= 1,page_incr= 1, page_size=0);
  spinner2 = gtk_spin_button_new (adjustment=adj,climb_rate= 0,digits= 0);
  spinner2.set_wrap[%t];
  ...
  button = gtk_check_button_new(label="Snap to 0.5-ticks");
  button.connect[ "clicked",toggle_snap,list(spinner1)];
  vbox.pack_start[ button,   expand=%t,fill= %t,padding=0];
  button.set_active [ %t];

  button = gtk_check_button_new(label="Numeric only input mode");
  button.connect[ "clicked",toggle_numeric,list(spinner1)];
  vbox.pack_start[ button,  expand=%t,fill= %t,padding=0];
  button.set_active [ %t];

  val_label = gtk_label_new (str="");

  hbox = gtk_box_new("horizontal",spacing= 0);
  vbox.pack_start[ hbox,   expand=%f,fill= %t,padding=5];

  button = gtk_button_new(label="Value as Int");
  ...
  button = gtk_button_new(label="Value as Float");
  button.set_data[user_data= val_label];
