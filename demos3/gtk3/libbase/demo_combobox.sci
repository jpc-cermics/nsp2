...
function window = demo_combobox(do_widget)
  ...
  mainbox = gtkbox_new("vertical",spacing=2);
  window.add[mainbox]

  //1-- GtkCellView
  //---------------

  hbox = gtkbox_new("horizontal");
  ...
  boom =gtkbox_new("vertical",spacing=0);
  ...
  tmp = gtkframe_new (label="GtkComboBoxText (with entry)");
  hbox.pack_start[ tmp,expand=%f,fill=%f,padding=0]
  boom =gtkbox_new("vertical",spacing=0);
  boom.set_border_width[5];
  tmp.add[boom];
  // the function gtkcombobox_new
  ...
  combobox =gtkcombobox_new(with_entry=%t,text=text);
  boom.add[combobox];
