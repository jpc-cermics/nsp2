...
function y= draw_color (drawingarea, cr, color_name)
  ok = execstr('color= gdk_rgba_new(color_name);',errcatch=%t);
  if ok then 
...
function demo_flowbox (do_widget)
  colors=["AliceBlue",
	  "AntiqueWhite",
	 "AntiqueWhite1",
	 "AntiqueWhite2",
  ...
  for i =1:size(colors,'*')
    flowbox.add[color_swatch_new (colors(i))];
  end
  scrolled.show_all[];
  window.show[];
endfunction 
