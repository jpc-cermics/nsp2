function [xx,yy,zz]=spaghetti(x,y,z,w,nf, head=%t)
  ...
  if nargin==0 then
    printf("%s\n",'demo of spaghetti(x,y,z,w)_n');printf('');
    comm=['h=(-25:25)''/20; s=(tanh(h.^2/2)+1)/2; alpha=(-%pi:0.8:%pi);';
	  ...
	  'end';
	  'xbasc(); [xx,yy,zz]=spaghetti(x,y,z,0.08); plot3d1(xx,yy,zz);'];
    printf("%s\n",comm)
    execstr(comm)
  ...
  if nargin<5 then nf=11; end

  if nargin<3 then 
    printf("%s\n",'spaghetti() wants one or more trajectories x,y,z !\n')
    return
  end

  if nargin==3 then w=(max(y)-min(y))/20; end  //just a given value
  ...
  if np==1 then 
    printf("%s\n",'cannot make spaghetti of single points!\n')
    return
  end
  ...
  for j=1:nv
    ...
    nN1=sqrt(sum(N1.^2,'c'));
    if isempty(find(abs(nN1)>10*%eps)) then
