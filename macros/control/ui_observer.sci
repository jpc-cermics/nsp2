function [UIobs,J,N]=ui_observer(Sys,reject,C1,D1,flag,Alfa,Beta)
  ...
  function N=lowlevel()
    ...
    Kleft=K(:,1:size(K,1))
    if size(Kleft,'*')==1 & abs(Kleft) < 1.E-8 then
      N=[];error('Error: Bad conditioning!');return;end
    if rcond(Kleft) <= 1.E-10 then
  ...
  if size(C1,2) <> size(Sys.A,1) then
    error('Error: dims of C1 and A are not compatible');
  end
  if size(D1,2) <> size(Sys.B,2) then
    error('Error: dims of D1 and B are not compatible');
  end
  not_reject=1:size(Sys,'c');not_reject(reject)=[];

  // Sys1=Sys(:,reject);      //A,B1,C2,D21
  Sys1=linear_system(Sys.A,Sys.B(:,reject),Sys.C,Sys.D(:,reject),Sys.X0,dom = Sys.dom, ...
                     sample = Sys.dt);
  ...
  Sys22=syslin(Sys.dom,A22,B22,C22,D22);
  ...
  N=lowlevel();
  printf('residual norm = %5.2f\n',norm([eye(size(N,1),size(N,1)),N]*[C11,D11;C21,D21]));
  D12=D1(:,not_reject);C12=C1new(:,ns+1:$);

  UIobs=linear_system(UIobs.A,UIobs.B,[C12+N*C22],[D12+N*D22,-N],UIobs.X0,dom = UIobs.dom, ...
                      sample = UIobs.dt);
endfunction
