...
//
  ...
  if size(B1,1) ~= size(Sys.A,1) then error('Error: dims of B1 and A are not compatible');end
  if size(D1,2) ~= size(B1,2) then error('Error: dims of D1 and B1 are not compatible');end
  // Sys1=Sys(zeroed,:);
  Sys1=linear_system(Sys.A,Sys.B,Sys.C(zeroed,:),Sys.D(zeroed,:),Sys.X0,dom=Sys.dom,sample=Sys.dt);
