...
//                and  Series = minss(S*Sys)
  if flag=='post' then
    S= linear_system(Sys.A+Sys.B*Gain, Sys.B, Gain, eye(size(Gain*Sys.B)), ...
		     Sys.X0, dom= Sys.dom,sample = Sys.dt);
    //Series=minss(Sys*S);
    D=Sys(5);
    Series= linear_system(Sys.A+Sys.B*Gain, Sys.B, Sys.C+D*Gain, Sys.D, Sys.X0,...
			  dom='c');
  end
  if flag=='pre' then
    S= linear_system(Sys.A+Gain*Sys.C, Gain, Sys.C, eye(Sys.C*Gain),...
		     Sys.X0,dom=Sys.dom,sample=Sys.dt)
    //Series=minss(S*Sys);
    D=Sys(5);
    Series=linear_system(Sys.A+Gain*Sys.C, Sys.B+Gain*D, Sys.C, Sys.D, ...
			 Sys.X0,dom='c');
  end
