function [L,M,T]=gfrancis(Plant,Model)
  ...
  [F,G,H,J]=abcd(Plant);
  [A,B,C,D]=abcd(Model);
  [nf,vnf]=size(F);[ny,nu]=size(J);
  [na,vna]=size(A);[lc,num]=size(D);
  Ia=eye(na,na);Inf=eye(nf,nf);Iu=eye(num,num);
  Mat=[Ia .*.F-A' .*.Inf,Ia .*.G,zeros(nf*na,nu*num);Ia .*.H,Ia .*.J,zeros(ny*na,nu*num);
       -B' .*.Inf,zeros(nf*num,nu*na),Iu .*.G;
       zeros(ny*num,nf*na),zeros(ny*num,nu*na),Iu .*.J];

  nargin=[zeros(nf*na,1);matrix(C,size(C,'*'),1);zeros(nf*num,1);matrix(D,size(D,'*'),1)];
  TLM=pinv(Mat)*nargin;
  ...
  Wmodel=[A,B;C,D];
  //check
  err=norm(Wplant*[T,zeros(nf,num);L,M]-[T,zeros(nf,lc);zeros(lc,na),eye(lc,lc)]*Wmodel,1);
  if err > 1.E-5 then printf("Warning: Francis equations not satisfied!\n");end
endfunction
