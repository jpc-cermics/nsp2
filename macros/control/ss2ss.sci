...
// Copyright INRIA
  ...
  select nargin
   case 2 then 
    Sl1 = linear_system(inv(T)*Sl.A*T,inv(T)*Sl.B,Sl.C*T,Sl.D,inv(T)*Sl.X0,...
   ...
   case 3 then 
    ...
    right=linear_system(Sl.A+Sl.B*F,Sl.B,F,eye(size(F*Sl.B)),Sl.X0,dom=Sl.dom,sample=Sl.dt);
   ...
   case 5 then 
    if flag==1 then 
      ...
      Sl1=linear_system(A1,B1,C1,D1,Sl.X0,dom=Sl.dom,sample=Sl.dt);
      left=linear_system(Sl.A+G*Sl.C,[G,-G],Sl.C,[eye(p,p),zeros(p,p)],Sl.X0,dom=Sl.dom,sample=Sl.dt);
    ...
    if flag==2 then 
      ...
      A1=inv(T)*A1*T;B1=inv(T)*B1;C1=C1*T;D1=D1
      Sl1=linear_system(A1,B1,C1,D1,Sl.X0,dom=Sl.dom,sample=Sl.dt);
      right=linear_system(Sl.A+Sl.B*F, Sl.B,F, eye(size(F*Sl.B)), ...
			  Sl.X0,dom=Sl.dom,sample=Sl.dt);
      // Warning left is computed as [ At + G*Ct,(G,-G);
      //                             [    Ct    ,(I, 0)]
      // where [At Bt; Ct Dt] is Sl1*right 
      At=Sl.A+Sl.B*F; Ct=Sl.C+D*F
      left=linear_system(At+G*Ct,[G,-G],Ct,[eye(size(Ct*G)),zeros(size(Ct*G))],...
			 Sl.X0,dom=Sl.dom,sample=Sl.dt);
    end 
  end
endfunction
