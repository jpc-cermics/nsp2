...
//!
  ...
  if ns<>na then printf('Warning:  P22 not stabilizable\n');end
  if ns==na then printf('P22 is stabilizable\n');end

  [nd,Ud,Sd]=dt_ility(P22,1.d-10)

  if nd <> 0 then printf('Warning: P22 not detectable\n');end
  if nd==0 then printf('P22 is detectable\n');end
  ...
  if size(nt,'*')>0 then
    if min(abs(rzt)) < sqrt(%eps) then 
      printf('Warning: P21 has a zero on/close the imaginary axis\n'),
    end,
  end,
  ...
  [T1,r1]=rowcomp(D12),
  if r1<>m2 then error('Error: D12 not full column rank'),end,
  T1=[T1(r1+1:p1,:);T1(1:r1,:)],
  D12=T1*D12,
  //Column compression of D21 (right)
  [S1,r2]=colcomp(D21),
  if r2<>p2 then error('Error: D21 not full row rank'),end,
  D21=D21*S1,
  //Updating 
  B1=B1*S1,C1=T1*C1,
  D11=T1*D11*S1,
...
//   [B1;D21]*[B1;D21]'=R=[R1 L';L R2]
  ...
  if err==0 then
    ...
    Kinf=-(D1122+D1121*inv(mu1*eye(size(D1111'*D1111))-D1111'*D1111)*D1111'*D1112);
  end
  ...
  if norm(D11) >= gama then printf('Warning: gamma too small\n');
    ...
    Teta11=gam2*D11;
    Teta22=gam2*D11';
    W12=-gam2*D11*D11';W12=eye(size(W12))+W12;
    W21=-gam2*D11'*D11;W21=eye(size(W21))+W21;
    Teta12=(1/gama)*sqrtm(0.5*(W12+W12'));
    Teta21=-(1/gama)*sqrtm(0.5*(W21+W21'));

    //Teta*Teta'=(1/gama*gama)*eye()

    M=inv(eye(size(D11*Teta22))-D11*Teta22);
    N=inv(eye(size(Teta22*D11))-Teta22*D11);
    ...
    [T1,r1]=rowcomp(D12);
    if r1<>m2 then error('Error: D12 not full rank! '),end
    T1=[T1(r1+1:p1,:);T1(1:r1,:)],
    D12=T1*D12,
    //Column compression of D21
    [S1,r2]=colcomp(D21),
    if r2<>p2 then error('Error: D21 not full rank! '),end,
    D21=D21*S1,
    //Updating
    B1=B1*S1,C1=T1*C1,
    D11=T1*D11*S1,
    ...
    R1=B1*B1';
    S=D12'*C1;
    C1#=(eye(size(D12*D12'))-D12*D12')*C1;
    Ax=A-B2*S;
    Qx=-C1#'*C1#;
	...
	printf('Riccati solution inaccurate error=%f\n',errx);
      ...
      Q1=C1'*C1;
      L=B1*D21';
      B1#=B1*(eye(size(D21'*D21))-D21'*D21);
      Ay=A-L*C2;
      Qy=-B1#*B1#';
...
// 
  ...
  [A,B1,B2,C1,C2,D11,D12,D21,D22]=smga(P,r);
  if norm(D11,1) > %eps then error('Error: D11 <> 0'),return;end
