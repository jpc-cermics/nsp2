...
// Copyright INRIA
  ...
  if type(p,'short') =='m' then p=syslin([],[],[],[],p);end
  if type(p,'short') =='linearsys' then dom=p(7);end 
  if type(p,'short') =='r' then dom= p.dom ;end
  ...
  pssdflag=%f;
  if type(p,'short')=='linearsys' then
    if type(p(5),'short')=='p' then
      pssdflag=%t;
    end
  end
  if type(p#,'short')=='linearsys' then
  ...
  if nargin==3 then
	...
	if type(p#,'short')=='r' then
	  p1=d11+d12*p#*inv(eye(size(d22*p#))-d22*p#)*d21;// invr
                                                          // replaced by inv
	end
      end
    end   //type(p,'short')=1
    ...
    if type(p,'short') =='r' then
      ...
      p1=p11+p12*p#*inv(eye(size(p22*p#))-p22*p#)*p21// invr replaced by inv
    end  //p(1)=='linearsys'
  end  //nargin=3
  if nargin==4 then
    if type(p,'short')=='m' then
	...
	g=inv(eye(size(d22*d#11))-d22*d#11);
	g#=inv(eye(size(d#11*d22))-d#11*d22);
	...
	g=inv(eye(size(d22*j11))-d22*j11);// invr replaced by inv
	g#=inv(eye(size(j11*d22))-j11*d22);// invr replaced by inv
	
	ptfg11=d11+d12*j11*g*d21;
	ptfg12=d12*g#*j12;
	//ptfg21=j21*g*p21;
	ptfg21=j21*(eye(size(d22*g#*j11))+d22*g#*j11)*d21;
    ...
    if type(p,'short')=='r' then
      ...
      g=inv(eye(size(p22*j11))-p22*j11);// invr replaced by inv
      g#=inv(eye(size(j11*p22))-j11*p22);// invr replaced by inv
      
      ptfg11=p11+p12*j11*g*p21;
      ptfg12=p12*g#*j12;
      //ptfg21=j21*g*p21;
      ptfg21=j21*(eye(size(p22*g#*j11))+p22*g#*j11)*p21;
