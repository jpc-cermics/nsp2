...
case 3 then if type(RELTOL,'short')==10 then options=RELTOL; RELTOL=1.0e-10;
                               else options='nul'; end,
end
...
if part(options,1)=='t' then
  printf([scale*lower,scale*ga,scale*upper],..
...
// Compute all the frequencies achieving ||G||
   ...
   if isempty(frequ) then
      printf('The computed value of || G || may be inaccurate');
   end
