function [gopt]=gamitg(g,r,PREC,options)
  ...
  select nargin,
    ...
    case 3 then
      if type(PREC,'short')==10 then options=PREC; PREC=1.0e-3;
      else options='nul';
      end
  end
  ...
  if m2 > p1 then
    printf('WARNING: the dimensions of D12 are inadequate');
  end
  
  if p2 > m1 then
    printf('WARNING: the dimensions of D21 are inadequate');
  end
  ...
  if s12(m2,m2)/s12(1,1) <= 100*%eps then
    printf('WARNING: D12 is not full rank at the machine precision');
  end
  
  if s21(p2,p2)/s21(1,1) <= 100*%eps then
    printf('WARNING: D21 is not full rank at the machine precision');
  end
  ...
  for iterations=1:30,   //max iterations=30
    ...
    if str_member('t',options) then
      printf('min=3e20.10, cur=3e20.10,max=3e20.10)\n',lower,ga,upper);
    end
    ...
    if lower > INFTY then
      printf('controllable & observable mode(s) of A near the imaginary axis');
      gopt=lower;
      return;
    else
      if upper < 1.0e-10 then
	gopt=upper;
	printf('All modes of A are nearly nonminimal so that || G || is almost 0');
	return;
      else
